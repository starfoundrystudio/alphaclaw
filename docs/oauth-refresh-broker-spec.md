...
- **Phase A — protocol + gateway service** (clawctl): **DONE 2026-08-26** (main 93ac3fe, 437/437 + tsc clean). Broker forced command (`alphaclaw-gateway-oauth-broker`: status/deposit/access_token/revoke, deny marker), PKI keypair, staging both sides (workload install graceful when unstaged), installer script + KEK reuse + restricted authorized_keys entry, verification checks on both hosts. Grant record seals ALL secrets (refresh token + cached access token) under the Phase E KEK — only non-secret timestamps at rest in cleartext (a design fix caught during impl: the first cut left the cached access token in plaintext). Subprocess test drives the real shipping script (rotation containment, caching, at-rest sealing, secret-free status, deny/revoke). Live-validated on the enforced gateway: script runs sharing the Phase E KEK, deposit → ciphertext-only on disk, TLS cert verification enforced (fail-closed on untrusted endpoint — the D6 posture), status secret-free, revoke; manual drill artifacts cleaned up, gateway restored. Provenance-stamped bundle → beta pin → soak still pending the seam.
