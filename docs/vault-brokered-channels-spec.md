...
**Status:** Phases A (verification, §8), **B+C implemented on `main` 2026-08-25** (0150048 server, 175610e wizard) per owner approval and decisions D1–D6, and **Phase D shipped 2026-08-25** in `0.9.18-starfoundry.18-beta.16`: AGENTS.md gains a "Clawbridge-managed credentials" section routing model/gateway keys to the Models screen and channel tokens to the dashboard channel setup, stating the Tier C custody position (pairing crypto material is neither vault-brokered nor env-written), and teaching that `__agent_vault_*__` values are working placeholders. **Phase E implemented + live-validated 2026-08-26** (clawctl main 811ae01, 432/432 tests): gateway forced-command custody channel (wrap/unwrap/status, AES-256-GCM, KEK root-only on the security gateway), per-instance custody keypair in the connectivity PKI, workload `alphaclaw-keystore-custody` seal/unseal CLI + `alphaclaw-keystore-unseal.service` boot unit in the host asset bundle, provisioning verification on both hosts. Live drill on the enforced test pair passed all criteria: seal (plaintext → tmpfs symlink, ciphertext-only on disk), boot-simulation re-unseal, KEK-denial (`custody_denied`, no plaintext recoverable, instance keeps running), and recovery. Custody channel left deployed on the test pair. Remaining integration point: when a Tier C channel (WhatsApp/Signal) is first offered, alphaclaw's channel-add flow calls `alphaclaw-keystore-custody seal <name> <keystore-path>` after pairing completes — the registry already enumerates the paths. Note for existing instances: the custody keypair ships at provision time, so pre-Phase-E instances need the one-time key/script install (documented by the live-apply steps) before sealing. v1 excluded WhatsApp/Signal; owner rejected exclusion-by-fiat; v2 classifies **every** channel in the OpenClaw catalog (26 plugins + the 4 core channels) into protection tiers — including the classes substitution cannot serve.
