...
1. Discord: `channels.discord.proxy` → shim URL; WSS+REST transit confirmed in flow logs; `websocket` substitution completes IDENTIFY with a vaulted token; `header` carries REST. *(Needs test credential; also observe plugin behavior when the proxy value is an unexpanded `${…}` literal — see item 4 result.)*
2. Slack: plugin Web API + `apps.connections.open` calls transit confirmed; WSS ticket confirmed secret-free. *(Needs test credential.)*
3. Telegram: long-poll transit + `path` substitution end-to-end. **Transit half DONE 2026-08-25**: gateway telegram runtime with a dummy token ran its getMe/401 auto-restart loop entirely via shim `127.0.0.1:14323` → tunnel `:14322` (46/79 packets), zero direct external 443. Substitution half needs the real token.
...
Owner dependency: items 1–3 (remaining halves) need **throwaway test credentials** (a scratch Discord bot, Slack app, Telegram bot) — the enforced test instance deliberately has no external accounts.
