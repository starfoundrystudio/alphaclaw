...
const kDirectProviderCredentialKeys = new Set([
  "ANTHROPIC_API_KEY",
  "DEEPGRAM_API_KEY",
  "GEMINI_API_KEY",
  "GROQ_API_KEY",
...
const kCredentialKeyPattern =
  /\b[A-Z][A-Z0-9]*(?:_(?:API_KEY|ACCESS_KEY|AUTH_TOKEN|CLIENT_SECRET|CREDENTIALS?|PASSWORD|PRIVATE_KEY|SECRET|TOKEN))\b/g;
const kCredentialWordPattern =
  /\b(?:api key|access key|auth token|client secret|credential|password|private key|secret|token)\b/i;
const kEnvDestinationPattern =
  /(?:\b(?:environment(?: variable)?|env(?:ironment)? var|envar|runtime configuration)\b|\.env\b)/i;
const kFallbackActionPattern =
  /\b(?:add|configure|create|define|export|provide|put|set|store|supply)\b/i;
const kNegatedFallbackPattern =
  /\b(?:do not|don't|never|instead of|without)\b.{0,40}\b(?:add|configure|create|define|export|provide|put|set|store|supply)\b/i;
...
const setupRequiredResult = (entryUrl) => ({
...
const isCredentialEnvFallback = (value) =>
  String(value || "")
    .split(/\r?\n/)
    .some((line) => {
      if (
        !kEnvDestinationPattern.test(line) ||
        !kFallbackActionPattern.test(line) ||
        kNegatedFallbackPattern.test(line)
      ) {
        return false;
      }
      const keys = line.match(kCredentialKeyPattern) || [];
      if (keys.some((key) => !kDirectProviderCredentialKeys.has(key))) {
        return true;
      }
      return keys.length === 0 && kCredentialWordPattern.test(line);
    });

const retryCredentialEnvFallback = (event) => {
  if (!getEntryUrl() || !isCredentialEnvFallback(event?.lastAssistantMessage)) {
    return;
  }
  return {
    action: "revise",
    ...
    retry: {
      ...
      idempotencyKey: `agent-vault-credential-policy:${String(
        event?.runId || event?.turnId || event?.sessionId || "current",
      )}`,
      maxAttempts: 2,
    },
  };
...
const replaceCredentialEnvFallback = (event) => {
  const text = event?.payload?.text;
  const entryUrl = getEntryUrl();
  if (!entryUrl || !isCredentialEnvFallback(text)) return;
  const runtimeReady =
    String(process.env.AGENT_VAULT_ADDR || "").trim() &&
    String(process.env.AGENT_VAULT_TOKEN || "").trim() &&
    String(process.env.AGENT_VAULT_VAULT || "").trim();
  return {
    payload: {
      ...event.payload,
      text: runtimeReady
        ? "I need to request this service credential through Agent Vault instead of asking you to configure an environment variable. Please ask me to retry so I can create the secure Agent Vault approval request."
        : `Agent Vault needs to be initialized before I can request this service credential. Open ${entryUrl}, then ask me to retry. I will use the Agent Vault approval workflow instead of an environment variable.`,
    },
  };
};
...
const validateConfig = () => {
  ...
  try {
    ...
    throw new Error("Agent Vault managed tunnel address is unavailable");
  }
  if (
      ...
      try {
        ...
        if (!responseBody.length) {
          if (status < 200 || status >= 300) {
            const error = new Error(`Agent Vault returned HTTP ${status}`);
...
const request = async ({ config, pathname, method = "GET", body }) => {
...
const createPlugin = () => ({
  id: kPluginId,
  name: "Agent Vault",
  description: "Service access and credential proposal broker for Clawbridge",
  register(api) {
    api.on?.("before_agent_finalize", retryCredentialEnvFallback, {
      priority: 1000,
    });
    api.on?.("reply_payload_sending", replaceCredentialEnvFallback, {
      priority: 1000,
    });
    api.registerTool(
      {
        ...
        description:
          "MANDATORY before an authenticated third-party API call: ensure Agent Vault has both a service rule and every credential it references. Call this even when no API key is configured; never tell the user to add a credential environment variable. If Agent Vault needs initialization, return its setup URL. If access is missing, create one atomic, value-less proposal and return the approval URL. Never ask for or accept credential values.",
        parameters: {
          type: "object",
        ...
        async execute(_toolCallId, params) {
          try {
            ...
            const runtimeIdentityPresent =
              ...
              String(process.env.AGENT_VAULT_TOKEN || "").trim() &&
              String(process.env.AGENT_VAULT_VAULT || "").trim();
            if (!runtimeIdentityPresent && entryUrl) {
              return jsonToolResult(setupRequiredResult(entryUrl));
            }
            const config = await runStage("configuration", async () =>
              validateConfig(),
            );
            const discovered = await runStage("discovery", async () =>
              request({ config, pathname: "/discover" }),
            );
            ...
            const proposal = await runStage("proposal", async () =>
              request({
                config,
            ...
            return jsonToolResult(
              await runStage("approval_link", async () =>
            ...
            return jsonToolResult(
              {
                status: "error",
                stage: error?.agentVaultStage || "request_validation",
