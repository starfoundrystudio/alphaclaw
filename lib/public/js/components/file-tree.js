...
import {
  kLockedBrowsePaths,
  isBrowseDropTargetRestricted,
  isBrowsePathMoveRestricted,
...
const TreeContextMenu = ({
  ...
  const canDelete = !isDeleteBlocked && (isFolder || isFile) && targetPath;
      ...
      ${isLocked
        ? html`
            <div class="tree-context-menu-item is-disabled">
              ...
              <span>Managed by Clawbridge</span>
            </div>
          `
        : isDeleteBlocked
          ? html`
              <div class="tree-context-menu-item is-disabled">
                <${LockLineIcon} className="tree-context-menu-icon" />
                <span>Cannot be moved or deleted</span>
              </div>
            `
        : null}
    </div>
...
const TreeNode = ({
  ...
  if (node.type === "file") {
    ...
    const normalizedPolicyPath = normalizeBrowsePolicyPath(node.path || "");
    const isMoveRestricted = isBrowsePathMoveRestricted(normalizedPolicyPath);
        ...
        <a
          ...
          oncontextmenu=${(e) => {
            ...
            onContextMenu({
              ...
              targetType: "file",
              isLocked,
              isDeleteBlocked: isMoveRestricted,
            });
          }}
          draggable=${!isMoveRestricted}
          onDragStart=${(e) => {
            if (isMoveRestricted) { e.preventDefault(); return; }
            e.dataTransfer.setData(kBrowseDragDataType, node.path);
        ...
        >
          ...
          ${isMoveRestricted
            ? html`<${LockLineIcon}
                ...
                title=${isLocked ? "Managed by Clawbridge" : "Cannot be moved or deleted"}
              />`
            : hasDraft
  ...
  const normalizedFolderPolicyPath = normalizeBrowsePolicyPath(folderPath);
  const isFolderMoveRestricted = folderPath && isBrowsePathMoveRestricted(
    normalizedFolderPolicyPath,
    { isDirectory: true },
  );
  const isFolderDropRestricted = folderPath && isBrowseDropTargetRestricted(
    normalizedFolderPolicyPath,
    { isDirectory: true },
  );
  ...
  return html`
    <li class="tree-item${dragSourcePath === folderPath ? " is-dragging" : ""}">
      <div
        ...
        oncontextmenu=${(e) => {
          ...
          onContextMenu({
            ...
            isDeleteBlocked: isFolderMoveRestricted,
          });
        }}
        draggable=${!!folderPath && !isFolderMoveRestricted}
        onDragStart=${(e) => {
          if (!folderPath || isFolderMoveRestricted) { e.preventDefault(); return; }
          e.dataTransfer.setData(kBrowseDragDataType, folderPath);
          e.dataTransfer.effectAllowed = "move";
        ...
        onDragEnter=${(e) => {
          ...
          e.preventDefault();
          e.stopPropagation();
          if (isFolderDropRestricted) return;
        ...
        onDrop=${(e) => {
          ...
          const sourcePath = getBrowseDragSource(e.dataTransfer, dragSourcePath);
      ...
      >
        ...
        <span class="tree-label">${node.name}</span>
        ${isFolderMoveRestricted
          ? html`<${LockLineIcon}
              className="tree-lock-icon"
              title=${isFolderLocked ? "Managed by Clawbridge" : "Cannot be moved or deleted"}
            />`
          : null}
      </div>
...
export const FileTree = ({
  ...
  const requestDelete = (targetPath) => {
    ...
    if (isBrowsePathMoveRestricted(normalizedTargetPath, {
      isDirectory: folderPaths.has(targetPath),
    })) {
  ...
  const handleDragDrop = async (action, sourcePath, targetFolder) => {
    ...
    if (action === "drop") {
      if (!sourcePath || sourcePath !== dragSourcePathRef.current) return;
      dragSourcePathRef.current = "";
      setDragSourcePath("");
      const destination = resolveBrowseMoveDestination(sourcePath, targetFolder);
  ...
  return html`
    <div class="file-tree-wrap">
      ...
      <ul
        ...
        onDragOver=${(e) => {
          if (e.target !== e.currentTarget || !dragSourcePath) return;
          e.preventDefault();
          e.dataTransfer.dropEffect = "move";
        }}
        onDrop=${(e) => {
      ...
      ${contextMenu
        ? html`
            <${TreeContextMenu}
              ...
              isDeleteBlocked=${!!contextMenu.isDeleteBlocked}
