...
const kGroupDescriptions = {
  networking:
    "Use these when the Setup UI stays private and webhook/OAuth callbacks use a separate public hostname.",
};

const kGroupOrder = ["ai", "github", "networking", "channels", "tools", "custom"];
...
const kHintByKey = {
  ANTHROPIC_API_KEY: html`from${" "}
    ${docLink("https://console.anthropic.com", "console.anthropic.com")}`,
  ANTHROPIC_TOKEN: html`from
    <code class="text-xs bg-field px-1 rounded">claude setup-token</code>`,
  OPENAI_API_KEY: html`from${" "}
    ${docLink("https://platform.openai.com", "platform.openai.com")}`,
  GEMINI_API_KEY: html`from${" "}
    ${docLink("https://aistudio.google.com", "aistudio.google.com")}`,
  ELEVENLABS_API_KEY: html`from${" "}
    ${docLink("https://elevenlabs.io", "elevenlabs.io")}${" "} · ${" "}
  ...
  GITHUB_WORKSPACE_REPO: html`use
    ...
    >`,
  ALPHACLAW_SETUP_URL: html`private Setup UI URL, for example
    <code class="text-xs bg-field px-1 rounded"
      >https://alphaclaw.tail123.ts.net</code
    >`,
  ALPHACLAW_PUBLIC_BASE_URL: html`public callback URL for webhooks and OAuth, for example
    <code class="text-xs bg-field px-1 rounded"
      >https://callbacks.example.com</code
    >`,
  ALPHACLAW_PUBLIC_EXTRA_PATH_PREFIXES: html`comma-separated extra public paths, for example
    <code class="text-xs bg-field px-1 rounded">/googlechat,/api/messages</code>`,
  ALPHACLAW_BASE_URL: html`legacy private Setup UI fallback. Prefer
    <code class="text-xs bg-field px-1 rounded">ALPHACLAW_SETUP_URL</code>`,
  TELEGRAM_BOT_TOKEN: html`from${" "}
    ${docLink("https://t.me/BotFather", "@BotFather")}
    ·
    ${docLink("https://docs.openclaw.ai/channels/telegram", "full guide")}`,
  DISCORD_BOT_TOKEN: html`from${" "}
    ...
    ·
    ${docLink("https://docs.openclaw.ai/channels/discord", "full guide")}`,
  MISTRAL_API_KEY: html`from${" "}
    ${docLink("https://console.mistral.ai", "console.mistral.ai")}`,
  VOYAGE_API_KEY: html`from${" "}
    ${docLink("https://dash.voyageai.com", "dash.voyageai.com")}`,
  GROQ_API_KEY: html`from${" "}
    ${docLink("https://console.groq.com", "console.groq.com")}`,
  DEEPGRAM_API_KEY: html`from${" "}
    ${docLink("https://console.deepgram.com", "console.deepgram.com")}`,
  BRAVE_API_KEY: html`from${" "}
    ${docLink("https://brave.com/search/api/", "brave.com/search/api")}${" "} —
    free tier available`,
};
...
export const Envars = ({ onRestartRequired = () => {} }) => {
  ...
  const [agentVaultManaged, setAgentVaultManaged] = useState(false);
  const [legacyCredentialKeys, setLegacyCredentialKeys] = useState([]);
  ...
  const applyEnvPayload = useCallback(
    (data) => {
      ...
      setAgentVaultManaged(data.agentVaultManaged === true);
      setLegacyCredentialKeys(
        Array.isArray(data.legacyCredentialKeys)
          ? data.legacyCredentialKeys
          : [],
      );
  ...
  const handleAddVar = () => {
    ...
    if (
      agentVaultManaged &&
      /(?:^|_)(?:API_KEY|ACCESS_KEY|AUTH_TOKEN|CLIENT_SECRET|CREDENTIALS?|PASSWORD|PRIVATE_KEY|SECRET|TOKEN)$/.test(
        key,
      )
    ) {
      showToast(
        `${key} is a credential. Request it from the Agent Vault page.`,
        "error",
      );
      return;
    }
  ...
  const visibleVars = vars.filter((v) => {
    if (isManagedChannelTokenKey(v?.key)) return false;
    if (!agentVaultManaged) return true;
    if (v?.group === "ai") return false;
    return !/(?:^|_)(?:API_KEY|ACCESS_KEY|AUTH_TOKEN|CLIENT_SECRET|CREDENTIALS?|PASSWORD|PRIVATE_KEY|SECRET|TOKEN)$/.test(
  ...
  const renderGroupCard = (groupKey) => {
    ...
    return html`
      <div class="bg-surface border border-border rounded-xl overflow-hidden">
        ...
        ${kGroupDescriptions[groupKey]
          ? html`
              <p class="px-4 pb-2 text-xs text-fg-dim">
                ${kGroupDescriptions[groupKey]}
              </p>
            `
          : null}
  ...
  if (envPayloadLoading && !vars.length) {
    return html`
      <${PaneShell}
        header=${html`<${PageHeader} title="Runtime Configuration" />`}
      >
  ...
  return html`
    ...
    >
      ${agentVaultManaged
        ? html`
            <div class="rounded-xl border border-status-warning-border bg-status-warning-bg p-4 text-sm text-status-warning-muted">
              Credentials are managed in Agent Vault. This page is only for
              non-secret runtime configuration.
              ${legacyCredentialKeys.length
                ? html`
                    <p class="mt-2 text-xs">
                      Legacy credential variables remain preserved but hidden
                      until they are migrated: ${legacyCredentialKeys.join(", ")}.
                    </p>
                  `
                : null}
            </div>
