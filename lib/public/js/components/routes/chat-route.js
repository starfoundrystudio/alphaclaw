...
const kEmptyThreadPollIntervalMs = 5000;
...
export const ChatRoute = ({
  ...
  // A brand-new instance's agent greets first, server-side; an already-open
  // empty thread would never learn about it. Poll quietly until the first
  // message exists, over the socket when available, else via HTTP fallback.
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      ...
      setHistoryPollTick((tick) => tick + 1);
    }, kEmptyThreadPollIntervalMs);
    return () => clearInterval(intervalId);
  }, [selectedSessionKey, messages.length]);
