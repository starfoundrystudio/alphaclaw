...
import { mergeHistoryMessages } from "../../lib/chat-history-merge.js";
import { shouldPollChatHistory, createChatHistoryPollTracker } from "../../lib/chat-history-polling.js";
import { kChatSessionDraftsStorageKey } from "../../lib/storage-keys.js";
import { getSessionDisplayLabel } from "../../lib/session-keys.js";
import { ChannelSetupCallout } from "../channel-setup-callout.js";
import { showToast } from "../toast.js";

const html = htm.bind(h);
const kConnectionErrorRetryDelayMs = 4000;
const kBackgroundRunPollIntervalMs = 5000;
...
export const ChatRoute = ({
  ...
  const [browserRunSessionKey, setBrowserRunSessionKey] = useState("");
  const [isConnected, setIsConnected] = useState(false);
  const [rawHistoryBySession, setRawHistoryBySession] = useState({});
  const [historyNeedsReconciliation, setHistoryNeedsReconciliation] = useState({});
  const historyPollTracker = useMemo(createChatHistoryPollTracker, []);
  const sendHistoryRequest = useCallback((ws, sessionKey, reconcile = true) => {
    ws.send(JSON.stringify({
      type: "history", sessionKey,
      historyRequestId: historyPollTracker.beginRequest(sessionKey, { reconcile }),
    }));
  }, [historyPollTracker]);
  const recordHistorySnapshot = useCallback((sessionKey, requestId, payload) => {
    if (!historyPollTracker.acceptsResponse(sessionKey, requestId)) return false;
    const needsPoll = historyPollTracker.observeResponse(sessionKey, requestId, payload);
    setHistoryNeedsReconciliation(current => ({ ...current, [sessionKey]: needsPoll }));
    setRawHistoryBySession(current => ({ ...current, [sessionKey]: payload.rawHistory || null }));
    return true;
  }, [historyPollTracker]);
  const startHistoryCycle = useCallback((sessionKey) => {
    if (!sessionKey) return;
    historyPollTracker.startCycle(sessionKey);
    setHistoryNeedsReconciliation(current => ({ ...current, [sessionKey]: true }));
  }, [historyPollTracker]);
  useEffect(() => {
    startHistoryCycle(selectedSessionKey);
  }, [selectedSessionKey, startHistoryCycle]);
  ...
  const [historyPollTick, setHistoryPollTick] = useState(0);
  const httpHistoryPendingRef = useRef(false);
    ...
    const connect = () => {
      ...
      ws.onopen = () => {
        ...
        if (currentSessionKey) {
          ...
          sendHistoryRequest(ws, currentSessionKey);
        }
      };

      ws.onclose = () => {
        if (!mounted) return;
        // The gateway run can outlive its browser stream. HTTP fallback must
        // confirm fresh idle history even if this session was idle before it.
        // Failed upgrades own no stream and must not invalidate HTTP reads.
        if (opened) startHistoryCycle(selectedSessionKeyRef.current);
        setBrowserRunSessionKey("");
        ...
        if (realtimeDisabledRef.current) return;
        // Never give up permanently: a freshly provisioned instance restarts
        // its gateway (and this server) during the first minutes, and the
        // user may load the page mid-boot. Back off, but keep trying so the
        // composer enables itself once the runtime is reachable.
        const delayMs = Math.min(
          1000 * 2 ** Math.min(reconnectAttemptsRef.current, 4),
          15000,
        );
        reconnectAttemptsRef.current += 1;
        setConnectionError("Connecting to your agent…");
      ...
      ws.onerror = () => {
        ...
        setIsConnected(false);
        setHistoryLoading(false);
        // onclose follows and owns retry scheduling + the banner text.
      };
      ...
      ws.onmessage = (event) => {
        ...
        if (payload.type === "history") {
          ...
          if (!recordHistorySnapshot(historySessionKey, payload.historyRequestId, payload)) return;
          ...
          setMessagesBySession((currentMap) => ({
            ...currentMap,
            [historySessionKey]: mergeHistoryMessages(
              currentMap[historySessionKey],
              historyMessages,
            ),
          }));
          setHistoryLoading(false);
        ...
        if (payload.type === "chunk") {
          ...
          // A replace chunk carries the whole visible text so far (OpenClaw
          // 2026.9 re-sends it when the stream no longer extends the previous
          // text); it must overwrite, never append.
          const replaceChunk = payload.replace === true;
            ...
            if (
              ...
              return {
                ...
                [chunkSessionKey]: [
                  ...
                  {
                    ...lastMessage,
                    content: replaceChunk
                      ? chunkText
                      : `${String(lastMessage.content || "")}${chunkText}`,
                    debugPayload: {
        ...
        if (payload.type === "done") {
          ...
          if (doneSessionKey && ws && ws.readyState === 1) {
            ...
            sendHistoryRequest(ws, doneSessionKey);
          }
          return;
        }

        if (payload.type === "interrupted") {
          // The Gateway went away mid-run (restart). OpenClaw resumes the
          // turn itself under a new run, which this socket does not stream,
          // so read history until the session is idle (G3 finding #24).
          const interruptedSessionKey = String(
            payload.sessionKey || selectedSessionKeyRef.current || "",
          );
          if (interruptedSessionKey) {
            setActiveRunBySession((currentMap) => {
              const nextMap = { ...currentMap };
              delete nextMap[interruptedSessionKey];
              return nextMap;
            });
          }
          setBrowserRunSessionKey("");
          ...
          setHistoryLoading(false);
          if (interruptedSessionKey) {
            startHistoryCycle(interruptedSessionKey);
            appendDebugEvent(interruptedSessionKey, "ws:run-interrupted", {
        ...
        if (payload.type === "error") {
          ...
          if (String(payload.scope || "") === "connection") {
            // The bridge could not reach the gateway (booting/restarting).
            // Surface it in the banner and retry shortly — never render it
            // as an agent message in the thread.
            setConnectionError(
              String(payload.message || "Connecting to your agent…"),
            );
            if (errorSessionKey && !historyRetryTimerRef.current) {
              historyRetryTimerRef.current = setTimeout(() => {
                ...
                if (retryWs && retryWs.readyState === 1) {
                  sendHistoryRequest(retryWs, errorSessionKey);
                }
              }, kConnectionErrorRetryDelayMs);
            }
            return;
          }
          if (errorSessionKey) {
  ...
  useEffect(() => {
    ...
    if (ws && ws.readyState === 1) {
      ...
      sendHistoryRequest(ws, selectedSessionKey);
      return;
    }
    ...
    let cancelled = false;
    httpHistoryPendingRef.current = true;
    const loadHistory = async () => {
      try {
        setHistoryLoading(true);
        const historyRequestId = historyPollTracker.beginRequest(selectedSessionKey);
        ...
        if (!recordHistorySnapshot(selectedSessionKey, historyRequestId, payload)) return;
        const historyMessages = (
          Array.isArray(payload.messages) ? payload.messages : []
        )
          .map((messageRow) =>
        ...
        setMessagesBySession((currentMap) => ({
          ...currentMap,
          [selectedSessionKey]: mergeHistoryMessages(
            currentMap[selectedSessionKey],
            historyMessages,
          ),
        }));
        if (!isConnected) {
          // History is visible over HTTP while the realtime socket keeps
          // retrying in the background (transient during instance boots).
          // Only a hard "runtime unavailable" error disables realtime.
          setConnectionError(
            "Showing history; connecting live chat in the background…",
          );
        }
      } catch (err) {
        if (cancelled) return;
        ...
        if (!cancelled) {
          httpHistoryPendingRef.current = false;
          setHistoryLoading(false);
        }
      }
    };
  ...
  }, [isConnected, selectedSessionKey, historyPollTick]);

  const pollBackgroundHistory = shouldPollChatHistory({
    messages,
    sessionKey: selectedSessionKey,
    browserRunSessionKey,
    rawHistory: rawHistoryBySession[selectedSessionKey],
    needsReconciliation: historyNeedsReconciliation[selectedSessionKey],
    sending,
    streaming,
  });
  // A greeting or resumed run may first emit tool rows or partial text. Keep
  // polling until it is idle, while browser-owned runs use their live stream.
  useEffect(() => {
    if (!selectedSessionKey || !pollBackgroundHistory) return undefined;
    const intervalId = setInterval(() => {
      if (document.hidden) return;
      const ws = wsRef.current;
      if (ws && ws.readyState === 1) {
        sendHistoryRequest(ws, selectedSessionKey, false);
        return;
      }
      // A poll tick tears down the HTTP effect. Let slow reads finish before
      // requesting another snapshot; session/socket changes still cancel them.
      if (!httpHistoryPendingRef.current) setHistoryPollTick((tick) => tick + 1);
    }, kBackgroundRunPollIntervalMs);
    return () => clearInterval(intervalId);
  }, [selectedSessionKey, pollBackgroundHistory]);
  ...
  const handleSend = useCallback(() => {
    ...
    setAssistantStreamStarted(false);
    setBrowserRunSessionKey(selectedSessionKey);
    setSending(true);
    setMessagesBySession((currentMap) => ({
      ...currentMap,
      [selectedSessionKey]: [
      ...
      <div class="chat-thread" ref=${threadRef} onscroll=${handleThreadScroll}>
        ${!selectedSessionKey
          ...
          : historyLoading && messages.length === 0
            ...
            : messages.length === 0
              ? bootstrapPending
                ? html`<div class="chat-agent-waking">
                    <div class="chat-empty-state">
                      Your agent is starting up — it'll introduce itself in a
                      moment.
                    </div>
...
${JSON.stringify(
      ...
      <div class="chat-composer">
        ...
        <div class="chat-composer-actions">
          ...
          <button
            type="button"
            class="ac-btn-primary chat-composer-send"
