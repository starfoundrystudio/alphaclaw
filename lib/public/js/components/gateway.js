...
import {
  ...
  kGatewayHopDotClasses,
} from "../lib/gateway-hop.js";
...
const kRowTooltips = {
  ...
  securityGateway:
    "Reachability of the security gateway host over SSH, as measured by the host probe every two minutes. Clawbridge cannot repair this; if it stays unreachable the gateway host needs a power cycle.",
};
...
export const Gateway = ({
  ...
  // Hide a known disabled vault on self-hosted instances. A failed status
  // request must remain visible instead of implying the feature disappeared.
  const showVaultRow = vaultStatus
    ? vaultStatus.mode !== "disabled" || vaultStatus.ownerEnrollmentPending
    : !!vaultStatusError;
  const vaultHealth = vaultStatusError
    ? "status unavailable"
  ...
  const gatewayHop = useMemo(
    () => describeGatewayHopStatus(watchdogStatus?.gatewayHop, nowMs),
    [watchdogStatus?.gatewayHop, nowMs],
  );
  const gatewayHopDotClass =
    kGatewayHopDotClasses[gatewayHop.tone] || kGatewayHopDotClasses.unknown;
  ...
  return html` <div class="bg-surface border border-border rounded-xl p-4">
    <div class="space-y-2">
      ...
      ${gatewayHop.visible
        ? html`
            <div class="flex items-center justify-between gap-3">
              <${Tooltip} text=${kRowTooltips.securityGateway} delay=${300}>
                <div class="min-w-0 inline-flex items-center gap-2 text-sm">
                  <span
                    class=${gatewayHopDotClass.startsWith("ac-status-dot")
                      ? gatewayHopDotClass
                      : `w-2 h-2 rounded-full ${gatewayHopDotClass}`}
                  ></span>
                  <span class="font-semibold">Security gateway:</span>
                  <span class="text-fg-muted">${gatewayHop.label}</span>
                </div>
              </${Tooltip}>
              ${gatewayHop.detail
                ? html`
                    <span
                      class="text-xs text-fg-muted text-right truncate"
                      title=${gatewayHop.detail}
                      >${gatewayHop.detail}</span
                    >
                  `
                : null}
            </div>
