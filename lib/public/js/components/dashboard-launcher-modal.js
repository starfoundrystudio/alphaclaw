  ...
  if (status === kDashboardLauncherStatuses.LOADING) {
    return html`
      <div class="flex items-center gap-2 rounded-lg border border-border bg-field p-3 text-sm text-fg-muted">
        ...
        Preparing secure access. This can take up to a minute.
      </div>
    `;
  }
...
const FooterActions = ({ state, actions }) => {
  ...
  const canRetryPrepare = status === kDashboardLauncherStatuses.ERROR;
  ...
  >
    <div class="space-y-3">
      ...
      <div class="rounded-lg border border-status-warning-border bg-status-warning-bg p-3">
        ...
        <p class="mt-1 text-xs text-status-warning-muted">
          This upstream interface exposes controls outside the managed experience. Set up channels, models, and secrets in Clawbridge: changes made here are not managed, and TeamYou may not support workflows or settings found only here.
        </p>
      </div>
