      ...
      <div class="space-y-3">
        <div class="rounded-lg border border-status-warning-border bg-status-warning-bg p-3">
          ...
          <p class="mt-1 text-xs text-status-warning-muted">
            Clawbridge could not resolve the OpenClaw gateway token, so the dashboard may ask for manual authentication instead of opening cleanly.
          </p>
        </div>
      </div>
    `;
  ...
  return html`
    <div class="space-y-3">
      ...
      <p class="text-sm text-fg-muted">
        Clawbridge will open OpenClaw in a new tab. The first time this browser reaches OpenClaw, OpenClaw may ask Clawbridge to approve it here.
      </p>
    </div>
  `;
};
      ...
      ${canOpen
        ? html`
            <${ActionButton}
              tone="primary"
              size="sm"
              idleLabel=${status === kDashboardLauncherStatuses.APPROVED ? "Launch OpenClaw again" : "Launch OpenClaw"}
