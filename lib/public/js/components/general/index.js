...
export const GeneralTab = ({
  ...
  // Shares the vault page's cache key, so switching between the two screens
  // reuses the fetched status instead of re-claiming against the vault.
  const { data: vaultStatusPayload } = useCachedFetch(
    "/api/agent-vault/status",
    fetchAgentVaultStatus,
    { maxAgeMs: 30000, staleWhileRevalidate: true },
  );
    ...
    <div class="space-y-4">
      ...
      <div class="bg-surface border border-border rounded-xl p-4">
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0">
            <h2 class="font-semibold text-sm">OpenClaw dashboard access</h2>
            <p class="mt-1 text-xs text-fg-muted">
              The primary OpenClaw launcher is now always available in the left sidebar.
            </p>
          </div>
          <${ActionButton}
