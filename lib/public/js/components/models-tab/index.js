...
export const Models = ({ onRestartRequired = () => {}, agentId, embedded = false }) => {
  ...
  const providerRefreshFailures = useMemo(
    () =>
      (catalogStatus.providerOutcomes || []).filter((outcome) => {
        const status = String(outcome?.status || outcome?.outcome || "")
          .trim()
          .toLowerCase();
        return status !== "ready" || !!outcome?.error;
      }),
    [catalogStatus.providerOutcomes],
  );

  const handleCatalogRefresh = async () => {
    const result = await refreshCatalog();
    if (result?.restartRequired) onRestartRequired();
  };

  const headerActions = html`
    <${ActionButton}
      onClick=${handleCatalogRefresh}
      disabled=${saving || catalogRefreshing}
      loading=${catalogRefreshing}
      loadingMode="inline"
      tone="secondary"
      size="sm"
      idleLabel="Refresh catalog"
  ...
  const bodyContent = html`
    ...
    <div class="bg-surface border border-border rounded-xl p-4 space-y-3">
                  ...
                  >
                    ...
                    <div class="flex items-center gap-2 shrink-0">
                      ...
                      <button
                        ...
                        class="text-xs text-fg-dim hover:text-status-error-muted px-1 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        Remove
                      </button>
                    </div>
      ...
      ${loading
        ...
        : error
          ...
          : null}
      ${catalogStatus.restartRequired
        ? html`
            <div class="bg-status-warning-bg border border-status-warning-border rounded-lg p-3 text-xs text-status-warning">
              A newer hosted model catalog has been downloaded. Restart OpenClaw to activate its models and pricing.
            </div>
          `
        : null}
      ${catalogStatus.warning || providerRefreshFailures.length > 0
        ? html`
            <div class="bg-status-warning-bg border border-status-warning-border rounded-lg p-3 text-xs text-status-warning">
              ${catalogStatus.warning ||
              `${providerRefreshFailures.length} provider catalog refresh${providerRefreshFailures.length === 1 ? "" : "es"} failed. The last compatible inventory remains available.`}
            </div>
          `
        : null}
    </div>
            ...
            ${requiredAuthRoutes.map(
              (route) => html`
                <${ProviderAuthCard}
                  ...
                  vaultKeyPending=${vaultKeyPending}
                  onRequestVaultKey=${requestVaultKey}
    ...
    <${AddModelModal}
      ...
      onAdd=${({ modelKey, modelConfig }) => addModel(modelKey, modelConfig)}
    />
  `;
