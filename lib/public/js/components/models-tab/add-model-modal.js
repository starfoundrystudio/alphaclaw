...
export const getInitialAddModelState = ({ catalog = [] } = {}) => {
  const accessMode = "subscription";
  const provider = getDefaultProvider({ models: catalog, accessMode });
  const modelKey = getInitialModelKeyForAccessModeProvider({
    models: catalog,
    accessMode,
    provider,
  });
  return { accessMode, provider, modelKey };
};

export const isAddModelSelectionConfigured = ({
  selection = {},
  configuredModels = {},
  providerRuntimeIds = {},
  modelRuntimeIds = {},
} = {}) => {
  const modelKey = String(selection.modelKey || "").trim();
  if (!modelKey || !hasOwn(configuredModels, modelKey)) return false;
  const selectedRuntimeId = getAgentRuntimeId(selection.modelConfig || {});
  const modelRuntimeId = String(modelRuntimeIds[modelKey] || "").trim();
  const configuredRuntimeId =
    modelRuntimeId ||
    getAgentRuntimeId(configuredModels[modelKey] || {}) ||
    String(providerRuntimeIds[getModelProvider(modelKey)] || "").trim();
  if (selectedRuntimeId) return configuredRuntimeId === selectedRuntimeId;
  return !configuredRuntimeId || configuredRuntimeId === "openclaw";
};

export const getAddModelProgressMessage = (selection = {}) => {
  const runtimeId = getAgentRuntimeId(selection.modelConfig || {});
  if (runtimeId === "codex") {
    return "Installing the Codex runtime plugin. This can take a minute the first time.";
  }
  if (runtimeId === "claude-cli") {
    return "Installing the Claude CLI runtime plugin. This can take a minute the first time.";
  }
  return "Saving model settings...";
};

export const shouldShowAlreadyConfiguredMessage = ({
  alreadyConfigured = false,
  adding = false,
  busy = false,
} = {}) => alreadyConfigured && !adding && !busy;
...
export const AddModelModal = ({
  ...
  const selectedAlreadyConfigured = isAddModelSelectionConfigured({
    selection: selected,
    configuredModels,
    providerRuntimeIds,
    modelRuntimeIds,
  });
  const progressMessage = getAddModelProgressMessage(selected);
  const showAlreadyConfiguredMessage = shouldShowAlreadyConfiguredMessage({
    alreadyConfigured: selectedAlreadyConfigured,
    adding,
    busy,
  });
  ...
  useEffect(() => {
    if (!visible) return;
    const initial = getInitialAddModelState({ catalog });
    setAccessMode(initial.accessMode);
  ...
  const handleAdd = async () => {
    if (!selected.modelKey || selectedAlreadyConfigured || adding || busy) return;
    setAdding(true);
    setError("");
    try {
  ...
  const controlsDisabled = adding || busy;
    ...
    >
      ...
      >
        ...
        ${error
          ...
          : null}
        ${adding || busy
          ? html`<p class="text-xs text-fg-muted">${progressMessage}</p>`
          : null}
        ...
        <div class="flex items-center justify-end gap-2 pt-2 border-t border-border">
          ...
          <${ActionButton}
            onClick=${handleAdd}
            disabled=${!selected.modelKey || selectedAlreadyConfigured || controlsDisabled}
            loading=${adding}
            tone="primary"
            size="sm"
            idleLabel="Add model"
            loadingLabel=${getAgentRuntimeId(selected.modelConfig || {})
              ? "Installing..."
              : "Adding..."}
            className="text-xs"
          />
