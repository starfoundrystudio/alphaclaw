...
export const buildProviderHasAuth = ({
  ...
  if (codexStatus?.connected) {
    if (openAiAuthMode === "codex") {
      result.openai = true;
    }
  }
  if (anthropicAuthMode === "claude-cli") {
    const cliStatusKnown = !!claudeCliStatus && claudeCliStatus.ok !== false;
    const cliLoggedIn = !!claudeCliStatus?.loggedIn;
    const cliConfigured = !!claudeCliStatus?.configured || hasClaudeCliProfile;
    if ((cliStatusKnown && cliLoggedIn && cliConfigured) || (!cliStatusKnown && hasClaudeCliProfile)) {
