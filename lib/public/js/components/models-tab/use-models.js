...
export const useModels = (agentId) => {
  ...
  const applyCatalogResult = useCallback(
    (catalogResult) => {
      ...
      const nextCatalogStatus = {
        ...
        providerOutcomes: Array.isArray(catalogResult?.providerOutcomes)
          ? catalogResult.providerOutcomes
          : [],
        refreshFailed: Boolean(catalogResult?.refreshFailed),
        warning: String(catalogResult?.warning || ""),
        restartRequired: Boolean(catalogResult?.restartRequired),
      };
  ...
  const refreshCatalog = useCallback(async () => {
    ...
    try {
      const result = await refreshModels("all");
      invalidateCache(kModelCatalogCacheKey);
      applyCatalogResult(result);
      if (result.restartRequired) {
        showToast(
          "A newer hosted model catalog was downloaded. Restart OpenClaw to activate it.",
          "warning",
        );
      } else {
      ...
      const message = err.message || "Failed to refresh model catalog";
      showToast(message, "error");
      return {
        ok: false,
        error: message,
        restartRequired: Boolean(err.payload?.restartRequired),
      };
    } finally {
      setCatalogRefreshing(false);
    }
  }, [applyCatalogResult, catalogRefreshing]);
