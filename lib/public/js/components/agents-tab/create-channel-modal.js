...
const isVaultPlaceholderValue = (value) =>
  /^__agent_vault_[a-z0-9_]+__$/.test(String(value || "").trim());
...
export const CreateChannelModal = ({
  ...
  const handleRequestVaultToken = async () => {
    if (vaultState.status === "requesting") return;
    setVaultState({ status: "requesting" });
    try {
      const result = await requestChannelVaultToken({ provider, accountId });
  ...
  useEffect(() => {
    ...
    const timer = setInterval(async () => {
      ...
      try {
        ...
        if (slotKeys.every((key) => availableKeys.has(key))) {
          await handleRequestVaultToken();
        } else {
          setVaultState({
            status: "idle",
