...
import {
  ...
  requestChannelVaultToken,
} from "../../lib/api.js";
import { VaultChannelTokenSetup } from "./create-channel-modal/vault-token-setup.js";
import { ChannelPolicyEditor } from "./create-channel-modal/channel-policy-editor.js";
...
const isVaultPlaceholderValue = (value) =>
  /^__(?:agent_vault|av)_[a-z0-9_]+__$/.test(String(value || "").trim());
const kVaultPollIntervalMs = 7000;
export const CreateChannelModal = ({
  ...
  const [slackAppName, setSlackAppName] = useState("Clawbridge");
  const [policy, setPolicy] = useState({});
  // Id of the account this modal is creating. The parent reloads the channel
  // list before the modal moves on, so without this the new account briefly
  // reads as a duplicate of itself.
  const [submittedAccountId, setSubmittedAccountId] = useState("");
  useEffect(() => {
    if (visible) setSubmittedAccountId("");
  }, [visible]);
  ...
  useEffect(() => {
    ...
    setSlackAppName("Clawbridge");
    setPolicy(
      isEditMode && account?.policy && typeof account.policy === "object"
        ? JSON.parse(JSON.stringify(account.policy))
        : {},
    );
  }, [
    visible,
  ...
  const vaultMode =
    ...
    (vaultBrokering?.providers || []).includes(normalizedProvider);
  const [vaultState, setVaultState] = useState({ status: "idle" });
  // The vault request is scoped to the account id, so once it is filed the
  // name that derives the id is locked for this attempt.
  const vaultNameLocked =
    vaultMode && providerHasAccounts && vaultState.status !== "idle";
  ...
  const accountId = useMemo(() => {
    ...
    // In vault mode the account id scopes the Agent Vault request, so it
    // comes from the name alone: the Telegram bot identity only arrives
    // after approval and must not re-derive (and reset) the account.
    return slugifyChannelAccountId(
      isTelegramCreate && !vaultMode ? telegramBot?.username || name : name,
    );
  }, [
    name,
    providerHasAccounts,
  ...
  const accountExists = useMemo(
    () =>
      !(submittedAccountId && submittedAccountId === accountId) &&
      Array.isArray(selectedChannel?.accounts) &&
      selectedChannel.accounts.some(
        (entry) =>
          String(entry?.id || "").trim() === String(accountId || "").trim(),
      ),
    [selectedChannel, accountId, submittedAccountId],
  );
  // Additional accounts need their own name before the vault step: the
  // name derives the account id that scopes the vault credential keys.
  const vaultBlockedReason = !vaultMode
    ? ""
    : !String(accountId || "").trim()
      ? "Name this account first."
      : accountExists
        ? "That name is already used by another account."
        : "";
  useEffect(() => {
  ...
  // The vault request is account-scoped; changing provider or the derived
  // account id invalidates any placeholders already obtained.
  useEffect(() => {
  ...
  }, [visible, provider, accountId, vaultMode]);

  const applyVaultSlots = (slots = []) => {
    // Slots are keyed by env name, never by position: the server orders them
    // for approval-page presentation (Slack lists the app token first).
    const findSlot = (matcher) =>
      slots.find((slot) => matcher.test(String(slot?.envKey || "")));
    const appSlot = findSlot(/_APP_TOKEN/);
    const tokenSlot =
      slots.find((slot) => slot !== appSlot) || null;
    setToken(String(tokenSlot?.placeholder || ""));
    setAppToken(String(appSlot?.placeholder || ""));
  };

  const handleRequestVaultToken = async ({ approvedProposalId } = {}) => {
    if (vaultState.status === "requesting") return;
    setVaultState({ status: "requesting" });
    try {
      // After an approval, naming the applied proposal tells the server the
      // service update already landed, so it confirms instead of re-filing.
      const result = await requestChannelVaultToken({
        provider,
        accountId,
        approvedProposalId,
      });
      if (result.status === "active") {
        applyVaultSlots(result.slots);
        setVaultState({ status: "active", slots: result.slots });
        ...
        setVaultState({
          status: "idle",
          error: result.error || "Agent Vault request failed",
        });
      }
    } catch (requestError) {
      setVaultState({
        status: "idle",
        error: requestError.message || "Agent Vault request failed",
      });
    }
  };

  // While the proposal is pending, watch it; once it leaves "pending",
  // confirm the slots actually exist before re-requesting (a re-request
  // after a decline would file a duplicate proposal).
  useEffect(() => {
    if (!visible || vaultState.status !== "pending") return undefined;
    const proposalId = vaultState.proposal?.id;
    const slotKeys = (vaultState.slots || []).map((slot) =>
      String(slot.envKey || ""),
    );
    ...
    const timer = setInterval(async () => {
      ...
      try {
        ...
        const available = await fetchAgentVaultCredentials().catch(() => null);
        if (cancelled) return;
        ...
        if (
          ...
          slotKeys.every((key) => availableKeys.has(key))
        ) {
          await handleRequestVaultToken({ approvedProposalId: proposalId });
        } else {
          setVaultState({
            status: "idle",
            error: `The Agent Vault request was ${proposalStatus}.`,
          });
        }
      } finally {
        polling = false;
      }
    }, kVaultPollIntervalMs);
    return () => {
      cancelled = true;
      clearInterval(timer);
    };
  }, [visible, vaultState.status, vaultState.proposal?.id]);

  // Once the vault is active for Discord, fetch the bot identity through the
  // vault-aware inspect so the original post-verify experience — live intents
  // status, recheck, and the preconfigured "Add bot to a server" install URL —
  // renders exactly as in the raw-token flow.
  useEffect(() => {
    if (!visible || !vaultMode || !isDiscordCreate) return undefined;
    if (vaultState.status !== "active" || discordBot) return undefined;
    const placeholderToken = String(token || "").trim();
    if (!placeholderToken) return undefined;
    let cancelled = false;
    (async () => {
      try {
        const result = await inspectDiscordBotToken(placeholderToken);
        if (!cancelled) setDiscordBot(result?.bot || null);
      } catch {
        // The server-side probe still gates the connect; the setup section
        // simply stays in its guidance-only state.
      }
    })();
    return () => {
      cancelled = true;
    };
  }, [visible, vaultMode, isDiscordCreate, vaultState.status, discordBot, token]);
  ...
  const canSubmit =
    ...
    (!vaultMode || vaultState.status === "active") &&
    (!isTelegramCreate ||
      vaultMode ||
      (!!telegramBot &&
  ...
  const handleSubmit = async () => {
    ...
    if (!isEditMode && !String(token || "").trim()) {
      setError(
        vaultMode
          ? "Store the token in Agent Vault before connecting the channel"
          : "Token is required",
      );
      return;
    }
    if (vaultMode && vaultState.status !== "active") {
      setError("Complete the Agent Vault approval before connecting the channel");
      return;
    }
    if (
      !vaultMode &&
    ...
    if (!isEditMode) setSubmittedAccountId(accountId);
    try {
      await onSubmit({
        provider,
      ...
      if (vaultMode) {
        // Vault mode skipped client-side verification, so the connected
        // screens have no bot identity yet — fetch it now with the
        // placeholders (the server routes these inspections through the
        // vault proxy) so the deep-link buttons render. Best-effort: the
        // channel is already created either way.
        try {
          if (isTelegramCreate) {
            const result = await inspectTelegramBotToken(trimmedToken);
            setTelegramBot(result?.bot || null);
          } else if (isDiscordCreate) {
            const result = await inspectDiscordBotToken(trimmedToken);
            setDiscordBot(result?.bot || null);
          } else if (isSlackCreate) {
            const result = await inspectSlackCredentials({
              botToken: trimmedToken,
              appToken: trimmedAppToken,
            });
            setSlackIdentity(result?.slack || null);
          }
        } catch {
          // Connected screen falls back to its generic copy.
        }
      }
      if (isTelegramCreate) {
      ...
      // The parent also toasts, but toasts vanish — keep the failure visible
      // in the modal (with the Agent Vault link when relevant) until the
      // user acts on it.
      setError(
        String(submitError?.message || "Could not configure the channel"),
      );
    }
  };
  ...
  return html`
    ...
    >
      ...
      ${telegramCreated
        ...
        <label class="block space-y-1">
          ...
          <input
            ...
            class=${`w-full bg-field border border-border rounded-lg px-3 py-2 text-sm outline-none focus:border-fg-muted ${
              vaultNameLocked ? "text-fg-muted" : "text-body"
            }`}
          />
        </label>
        ...
        `
          : null}
        ${vaultMode && isGuidedBotCreate
          ? html`<${VaultChannelTokenSetup}
              provider=${normalizedProvider}
              vaultState=${vaultState}
              blockedReason=${vaultBlockedReason}
              onRequest=${() => handleRequestVaultToken()}
            />`
          : null}
        ...
        ${!isGuidedBotCreate
          ...
          ${isWhatsApp
            ? html`
                ...
                />
              `
            : isEditMode && isVaultPlaceholderValue(token)
              ? html`
                  <div class="ac-surface-inset rounded-lg p-3">
                    <p class="text-xs text-body">
                      Managed in Agent Vault — rotate or revoke it there.
                    </p>
                  </div>
                `
              : html`
          ...
          <p class="text-xs text-fg-muted">
            ${isWhatsApp
              ...
              : html`Saved behind the scenes as${" "}<code
                  class="font-mono text-fg-muted"
                >${envKey || "CHANNEL_TOKEN"}</code>.`}
          </p>
        </label>`
          : null}
          ...
          needsAppToken && !isSlackCreate
            ? html`
                <label class="block space-y-1">
                  ...
                  />
                  <p class="text-xs text-fg-muted">
                    Saved behind the scenes as${" "}<code class="font-mono text-fg-muted">
        ...
        ${isEditMode && !isWhatsApp
          ? html`<${ChannelPolicyEditor}
              provider=${normalizedProvider}
              policy=${policy}
              agents=${agents}
              onChange=${setPolicy}
        ...
        ${error
          ? html`<div
              class="rounded-xl border border-status-error-border bg-status-error-bg p-3 space-y-2"
            >
              <p class="text-xs leading-5 text-status-error-muted">${error}</p>
              ${vaultMode && vaultBrokering?.consoleUrl
                ? html`<a
                    href=${vaultBrokering.consoleUrl}
                    target="_blank"
                    rel="noreferrer"
                    class="inline-flex h-8 items-center justify-center rounded-lg bg-white px-3 text-xs font-medium text-black hover:bg-gray-100"
                  >
                    Open Agent Vault
                  </a>`
                : null}
