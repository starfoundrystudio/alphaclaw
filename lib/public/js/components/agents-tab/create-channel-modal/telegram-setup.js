...
export const TelegramChannelSetup = ({
  ...
  return html`
    <div class="space-y-4">
      ...
      ${vaultMode
        ? html`<div class="rounded-xl border border-border bg-field p-3">
            <p class="text-xs leading-5 text-fg-muted">
              Keep BotFather's token handy — you will enter it on the Agent
              Vault approval page, never on this instance.
            </p>
          </div>`
      ...
      <div class="rounded-xl border border-border bg-field p-3">
        <p class="text-xs leading-5 text-fg-muted">
          AlphaClaw keeps direct messages in private pairing mode. In groups,
          anyone can talk to the bot by mentioning it; unmentioned conversation
          is ignored.
        </p>
      </div>
    </div>
...
export const TelegramChannelConnected = ({ bot = null, onDone = () => {} }) =>
  html`
    <div class="space-y-5 py-1">
      ...
      <div class="rounded-xl border border-border bg-field p-3">
        <p class="text-xs leading-5 text-fg-muted">
          Add the bot to a Telegram group and anyone there can interact by
          mentioning it. Direct messages still require pairing.
        </p>
      </div>
