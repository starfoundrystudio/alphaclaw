...
export const buildSlackConversationUrl = ({ appId = "", teamId = "" } = {}) => {
  const normalizedAppId = String(appId || "").trim();
  const normalizedTeamId = String(teamId || "").trim();
  if (!normalizedAppId) return "";

  const url = new URL("https://slack.com/app_redirect");
  url.searchParams.set("app", normalizedAppId);
  if (normalizedTeamId) url.searchParams.set("team", normalizedTeamId);
  return url.toString();
};
...
export const SlackChannelSetup = ({
  ...
  const normalizedAppName = String(appName || "").trim() || "Clawbridge";
  ...
  return html`
    <div class="space-y-4">
        ...
        <div class="flex items-start gap-3">
          ...
          <div class="min-w-0 space-y-1">
            ...
            <p class="text-xs leading-5 text-fg-muted">
              Clawbridge pre-configures everything the app needs — you just
      ...
      ${slack
        ? null
        : html`<div class="rounded-xl border border-border bg-field p-3 space-y-3">
        <div>
        ...
        ${vaultMode
          ? html`<p class="text-xs leading-5 text-fg-muted">
              Paste each token into a notes app as you copy it — the approval
              page asks for both at once. There,${" "}
              <code>SLACK_BOT_TOKEN</code> takes the <code>xoxb-</code> token
              and <code>SLACK_APP_TOKEN</code> takes the <code>xapp-</code>${" "}
              token.
            </p>`
          : null}
      </div>`}

      ${vaultMode ? null : html`<div class="space-y-3">
        <label class="block space-y-1">
        ...
        <p class="text-xs leading-5 text-fg-dim">
          Clawbridge verifies both tokens directly with Slack and confirms they
      ...
      <div class="rounded-xl border border-border bg-field p-3">
        <p class="text-xs leading-5 text-fg-muted">
          Direct messages require pairing. In channels, the app only responds
          when mentioned.
        </p>
      </div>
    </div>
...
export const SlackChannelConnected = ({ slack = null, onDone = () => {} }) => {
  const conversationUrl = buildSlackConversationUrl({
    appId: slack?.appId,
    teamId: slack?.workspace?.id,
  });
  const botName = slack?.bot?.name || "your app";
    ...
    <div class="space-y-5 py-1">
      ...
      <div class="ac-surface-inset rounded-xl p-4 space-y-2">
        ...
        <ol class="list-decimal space-y-1.5 pl-5 text-xs leading-5 text-fg-muted">
          <li>Open the app's conversation with the button below.</li>
          <li>Send any message to get a pairing code.</li>
          <li>
            Approve the request here under${" "}<strong>Pending Pairings</strong>.
          </li>
        </ol>
        <p class="text-[11px] leading-4 text-fg-dim">
          To use it in a channel, invite the app and mention it.
        </p>
      </div>
