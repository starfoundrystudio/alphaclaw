      ...
      ${slack
        ...
        ${vaultMode
          ? html`<p class="text-xs leading-5 text-fg-muted">
              The approval page asks for both tokens in this same order:${" "}
              <code>SLACK_APP_TOKEN</code> takes the <code>xapp-</code> token,
              then <code>SLACK_BOT_TOKEN</code> takes the${" "}
              <code>xoxb-</code> token.
            </p>`
          : null}
      </div>`}
