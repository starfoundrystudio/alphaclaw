...
export const AgentBindingsSection = ({
  ...
  // Deleting a channel leaves its vault tokens in place (another account or
  // a re-add may still use them), so say so for vault-brokered providers.
  const deletingVaultTokens =
    !!vaultBrokering?.enforced &&
    (vaultBrokering?.providers || []).includes(
      String(deletingAccount?.provider || "").trim(),
    );
  ...
  return html`
    <div class="space-y-3">
      ...
      <${ConfirmDialog}
        ...
        details=${deletingVaultTokens
          ? html`
              <p class="text-xs text-fg-muted">
                Its tokens stay in Agent Vault. Delete them there if you no
                longer use this app.
              </p>
            `
          : null}
