    ...
    <div class="bg-surface border border-border rounded-xl p-4 space-y-3">
        ...
        : html`
            <div class="border-t border-border pt-3 space-y-2">
              <div class="flex items-center justify-between gap-3">
                <div>
                  <p class="text-xs font-medium text-body">Speed</p>
                  <p class="text-xs text-fg-muted">Use fast mode when the selected model supports it.</p>
                </div>
                <${RowAccessorySelect}
                  ariaLabel="Agent speed preference"
                  title="Agent speed preference"
                  value=${fastModeValue}
                  disabled=${saving || updatingFastMode}
                  onChange=${handleSelectFastMode}
                >
      ...
      ${loading
        ...
        : html`
            <div class="border-t border-border pt-3 space-y-2">
              <div>
                <p class="text-xs font-medium text-body">Fallback order</p>
                <p class="text-xs text-fg-muted">OpenClaw tries these models if the primary model is unavailable.</p>
              </div>
                    ...
                    ${fallbackEntries.map(
                      (entry, index) => html`
                        <div class="flex items-center justify-between gap-3 py-1">
                          <span class="text-sm text-body truncate">${index + 1}. ${getModelDisplayLabel(entry)}</span>
                          <button
                            type="button"
                            class="ac-btn-ghost text-xs px-2 py-1 rounded-lg"
                            disabled=${saving || updatingFallbacks}
                            onclick=${() => handleRemoveFallback(entry.key)}
                          >Remove</button>
                        </div>
                      `,
                    )}
                  </div>`
                : html`<p class="text-xs text-fg-muted">No fallback models configured.</p>`}
              ${fallbackOptions.length > 0
                ? html`<${SearchableModelPicker}
