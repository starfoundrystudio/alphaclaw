...
export const WelcomeSetupStep = ({
  ...
  if (handoff?.setupUrl) {
    const isChecking = handoff.status === "checking";
    const isRedirecting = handoff.status === "redirecting";
    return html`
      <div class="py-4 flex flex-col items-center text-center gap-3">
      ...
      <div class="bg-field border border-border rounded-xl p-3 space-y-3">
        <div class="space-y-1">
          <p class="text-xs font-medium text-fg-muted">Final Tailscale URL</p>
          <p class="text-sm text-body font-mono break-all">${handoff.setupUrl}</p>
        </div>
