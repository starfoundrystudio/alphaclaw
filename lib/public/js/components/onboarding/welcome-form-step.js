...
export const WelcomeFormStep = ({
  ...
  const openAiCodexRoute =
    vals[kOpenAiCodexRouteKey] === kOpenAiCodexRouteRuntime
      ? kOpenAiCodexRouteRuntime
      : kOpenAiCodexRoutePi;
  ...
  return html`
    ...
    html`
      <div class="bg-field border border-border rounded-lg p-3 space-y-2">
                ...
                options=${[
                  {
                    ...
                    label: "OpenClaw Pi",
                    title:
                      "Use Codex OAuth through OpenClaw's default runtime. Best when you want easy switching across providers later.",
                  },
                  {
                    value: kOpenAiCodexRouteRuntime,
                    label: "Codex runtime",
                    title:
                      "Use the native Codex harness for better OpenAI/Codex runtime behavior. Non-Codex models require changing runtime config later.",
                  },
                ]}
                value=${openAiCodexRoute}
                onChange=${(value) => setValue(kOpenAiCodexRouteKey, value)}
              />
            </div>
            <p class="text-xs text-fg-dim">
              ${openAiCodexRoute === kOpenAiCodexRouteRuntime
                ? "Codex runtime improves OpenAI/Codex runs, but this agent will be tied to Codex-compatible models until runtime config changes."
                : "OpenClaw Pi keeps this agent flexible for switching between OpenAI, Anthropic, Gemini, OpenRouter, and other providers."}
            </p>
          </div>
        `}
        ...
        html`
          <p class="text-xs text-fg-dim">
            This model route uses Codex OAuth through OpenClaw Pi, so provider
            switching remains flexible.
          </p>
        `}
        ${codexAuthStarted &&
        html`
