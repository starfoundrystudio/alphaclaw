...
// WhatsApp is intentionally omitted from the wizard: its multi-device link
// runs a Noise-protocol handshake that the Agent Vault MITM breaks, so
// pairing can never complete on managed instances (the server also denies
// it via plugins.deny). Restore it here once a per-host MITM passthrough
// exists. See docs/vault-brokered-channels-spec.md §4.
...
export const Channels = ({
  ...
  const channelAccounts = Array.isArray(channelAccountsPayload?.channels)
    ...
    : [];
  const channelVaultBrokering = channelAccountsPayload?.vaultBrokering || null;
  ...
  const handleUpdateChannel = async (payload) => {
    ...
    try {
      ...
      // The modal shows the failure in its persistent error card (with the
      // Agent Vault link when relevant); a toast on top is redundant noise.
      throw error;
    } finally {
      setSaving(false);
    }
  ...
  const handleCreateChannel = async (payload) => {
    ...
    try {
      ...
      // Surfaced by the modal's persistent error card, not a toast.
      throw error;
    } finally {
      setSaving(false);
      setCreateLoadingLabel("Creating...");
  ...
  return html`
    <div class="space-y-3">
      ...
      <${CreateChannelModal}
        ...
        vaultBrokering=${channelVaultBrokering}
