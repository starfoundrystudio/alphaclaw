...
export const ComposioPanel = () => {
  ...
  useEffect(() => {
    if (!status?.cliInstalling && !status?.cliUpdating) return undefined;
      ...
      <div class="rounded-lg border border-border bg-field px-3 py-3 space-y-2">
        <div class="flex items-center justify-between gap-2">
          <div class="flex flex-wrap items-center gap-3">
            <${StatusDot}
              ok=${Boolean(status?.cliInstalled) && !status?.cliUpdating}
              label=${status?.cliUpdating
                ? "Updating CLI…"
                : status?.cliInstalled
                  ? `CLI installed${status?.cliVersion ? ` v${status.cliVersion}` : ""}`
      ...
      ${status?.cliUpdating
        ? html`
            <p class="text-xs text-fg-muted">
              Updating the Composio CLI${status?.targetCliVersion
                ? ` to v${status.targetCliVersion}`
                : ""} — email notifications pause briefly and resume
              automatically.
            </p>
          `
        : null}
