...
export const GoogleAccountRow = ({
  ...
  const reconnectRequired = account.oauthHealth?.reconnectRequired === true;
  const temporarilyUnavailable =
    account.oauthHealth?.checked === true &&
    account.oauthHealth?.healthy === false &&
    !reconnectRequired;
  const connectionWarning = reconnectRequired || temporarilyUnavailable;
  return html`
    <div class="border border-border rounded-lg bg-field overflow-visible">
      ...
      >
        ...
        <div class="flex items-center gap-2">
          ...
          <${Badge} tone=${account.authenticated && !connectionWarning ? "success" : "warning"}>
            ${reconnectRequired
              ? "Reconnect needed"
        ...
        ? html`
            <div class="px-3 pb-3 space-y-3 border-t border-border">
              ${connectionWarning
                ? html`
                    <div
                      class="mt-3 rounded-lg border border-status-warning-border bg-status-warning-bg px-3 py-2 text-xs text-status-warning"
                    >
                      ${reconnectRequired
                        ? "This restored Google connection can no longer refresh. Sign in again to reconnect it."
                        : "Google credential validation is temporarily unavailable. Use Check APIs to retry; reconnect only if this warning persists."}
                    </div>
                  `
                : null}
              ...
              <div class="pt-1 space-y-2 sm:space-y-0 sm:flex sm:justify-between sm:items-center">
                <div class="grid grid-cols-2 gap-2 w-full sm:w-auto sm:flex sm:items-center">
                  <button
                    ...
                    disabled=${account.authenticated && !scopesChanged && !reconnectRequired}
