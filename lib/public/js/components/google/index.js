...
import { ComposioPanel } from "./composio-panel.js";
...
import { getGoogleDisconnectOutcome } from "./disconnect-outcome.js";
...
export const Google = ({
  ...
  const handleProviderChange = async (event) => {
    const provider = String(event?.target?.value || "").trim();
    if (!provider || provider === providerState.provider) return;
    setSavingProvider(true);
    try {
      const data = await saveGoogleProvider(provider);
      if (!data?.ok) throw new Error(data?.error || "Could not save provider");
      setProviderState((prev) => ({
        ...prev,
        provider: data.provider,
        source: data.source,
      }));
      showToast("Google integration provider updated", "success");
      refreshAccounts();
    } catch (err) {
      showToast(err.message || "Could not save provider", "error");
    } finally {
  ...
  const handleDisconnect = async (accountId) => {
    ...
    const outcome = getGoogleDisconnectOutcome(data);
  ...
  return html`
    <div class="bg-surface border border-border rounded-xl p-4">
      <div class="flex items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
          ${providerState.loaded
            ? html`
                <${RowAccessorySelect}
                  ariaLabel="Google integration provider"
                  title=${providerState.source === "env"
                    ? "Set by ALPHACLAW_GOOGLE_PROVIDER environment variable"
                    : "Google integration provider"}
                  value=${providerState.provider}
                  disabled=${savingProvider || providerState.source === "env"}
                  onChange=${(value) =>
                    handleProviderChange({ target: { value } })}
                >
                  <option value="composio">Composio (recommended)</option>
                  <option value="gog">gog CLI</option>
                  <option value="none">Disabled</option>
                </${RowAccessorySelect}>
              `
            : null}
        ${providerState.provider === "gog" && accounts.length
          ? html`
              <div class="relative">
                <${OverflowMenu}
                  open=${addMenuOpen}
      ...
      ${providerState.provider === "composio"
        ? html`<${ComposioPanel} />`
        : providerState.provider === "none"
          ? html`<p class="text-xs text-fg-muted pt-3">
              Google Workspace integration is disabled for this deployment.
            </p>`
          : loading
