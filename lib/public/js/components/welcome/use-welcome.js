...
// Readiness polling for the successor runtime. Managed gateway hosts expose
// the probe on the current public bootstrap origin; older and local installs
// use the final ts.net origin. The endpoint returns a decodable SVG only after
// the successor process, gateway, Vault, watchdog, and kickoff are ready.
const kInstanceReadyProbePath = "/api/onboard/runtime-ready.svg";
const kInstanceSetupUrlHeader = "x-clawbridge-setup-url";
...
export const getInstanceProbeBaseUrl = (result, currentOrigin) =>
  result?.handoffViaBootstrapOrigin === true
    ? String(currentOrigin || "")
    : String(result?.setupUrl || "");
...
export const fetchBootstrapHandoffCompletion = async ({
  fetchImpl = globalThis.fetch,
} = {}) => {
  if (typeof fetchImpl !== "function") return null;
  try {
    const response = await fetchImpl(kInstanceReadyProbePath, {
      method: "HEAD",
      cache: "no-store",
      credentials: "same-origin",
    });
    const setupUrl = String(
      response?.headers?.get?.(kInstanceSetupUrlHeader) || "",
    ).trim();
...
export const waitForOnboardingCompletion = async ({
  ...
  fetchHandoffStatus = fetchBootstrapHandoffCompletion,
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    ...
    const handoffStatus = await fetchHandoffStatus();
    if (handoffStatus?.onboarded === true) return handoffStatus;
...
export const useWelcome = ({ onComplete }) => {
  ...
  const beginSetupHandoff = useCallback(
    async (result) => {
      ...
      const probeBaseUrl = getInstanceProbeBaseUrl(
        result,
        window.location.origin,
      );
      // Managed gateway hosts expose the authenticated readiness image on the
      // public bootstrap origin. Older and local installations retain their
      // existing final-origin probe until their ingress supports that lane.
      (async () => {
        const ready = await waitForInstanceReady({
          setupUrl: probeBaseUrl,
          isCancelled,
        });
