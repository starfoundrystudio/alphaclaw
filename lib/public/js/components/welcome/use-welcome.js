...
export const useWelcome = ({ onComplete }) => {
  ...
  const handleSubmit = async () => {
    ...
    const preflightError = (() => {
      ...
      const githubError = getWelcomeGroupError(
        "github",
        normalizedVals,
        submitValidationContext,
      );
    ...
    const agentRuntimeId =
      ...
      normalizedVals[kOpenAiCodexRouteKey] === kOpenAiCodexRouteRuntime
        ? "codex"
        : null;
    const onboardingModelKey =
      ...
      !agentRuntimeId &&
      normalizedVals[kOpenAiCodexRouteKey] !== kOpenAiCodexRouteRuntime
        ? getCodexOauthModelKeyForOpenAiModel(normalizedVals.MODEL_KEY, models)
  ...
  const goNext = async () => {
    ...
    if (activeGroup.id === "github") {
      ...
      const shouldVerifyGithub =
        githubFlow === kGithubFlowImport || hasAnyGithubBackupInput(normalizedVals);
      if (!shouldVerifyGithub) {
