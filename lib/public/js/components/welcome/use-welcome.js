...
export const useWelcome = ({ onComplete }) => {
    ...
    try {
      ...
      claudeCliLoginUnsubscribeRef.current = subscribeClaudeCliLoginEvents({
        ...
        onMessage: ({ event, data }) => {
          const payload = data || {};
          setClaudeCliLoginOutput(String(payload.output || ""));
  ...
  }, [claudeCliActionLoading, refreshClaudeCliStatus]);
  ...
  const adoptClaudeCliAuth = useCallback(async () => {
    ...
    try {
      ...
      setClaudeCliStatus(result.status || { ...claudeCliStatus, configured: true });
  ...
  }, [claudeCliActionLoading, claudeCliStatus]);
  ...
  const setModelAccessMode = (nextMode) => {
    ...
    const nextAccountProvider =
      normalizedMode === "subscription"
        ? getDefaultAccountLoginProvider(onboardingCatalog)
        : "";
    const nextModelKey =
      normalizedMode === "subscription"
        ? getInitialModelKeyForAccountLoginProvider({
            models: onboardingCatalog,
            provider: nextAccountProvider,
          })
        : getInitialModelKeyForAccessMode({
            models: onboardingCatalog,
    ...
    setVals((prev) => ({
      ...
      [kAccountLoginProviderStorageKey]: nextAccountProvider,
  ...
  const setAccountLoginProvider = (nextProvider) => {
    ...
    const normalizedProvider =
      normalizeAccountLoginProvider(nextProvider, onboardingCatalog) ||
      getDefaultAccountLoginProvider(onboardingCatalog);
    if (formError) setFormError(null);
    setShowAllModels(false);
    setVals((prev) => ({
      ...prev,
      [kModelAccessModeStorageKey]: "subscription",
      [kAccountLoginProviderStorageKey]: normalizedProvider,
      MODEL_KEY: getInitialModelKeyForAccountLoginProvider({
        models: onboardingCatalog,
  ...
  const modelAccessMode =
    ...
    getDefaultModelAccessMode();
  const accountLoginOptions = getAccountLoginProviderOptions(onboardingModels);
  const selectedAccountLoginProvider =
    modelAccessMode === "subscription"
      ? normalizeAccountLoginProvider(
          vals[kAccountLoginProviderStorageKey],
          onboardingModels,
        ) ||
        getAccountLoginProviderForModelKey(vals.MODEL_KEY, onboardingModels) ||
        getDefaultAccountLoginProvider(onboardingModels)
      : "";
  const accessModeModels =
    modelAccessMode === "subscription"
      ? getOnboardingModelsForAccountLoginProvider({
