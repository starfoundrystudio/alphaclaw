...
// The recovery poll must outlast the setup→runtime service swap: an
// interrupted onboard response is typically followed by ~55s where the
// proxy answers 502 before the runtime app serves /api/onboard/status
// again. 60 × 2s = 120s covers that window with margin.
export const kOnboardCompletionPollAttempts = 60;
export const kOnboardCompletionPollIntervalMs = 2000;

export const buildOnboardingVars = (vals = {}) =>
  Object.entries(vals)
    .filter(
    ...
    .filter(([key]) => !kOnboardingChannelCredentialKeys.has(key))
    .filter(([, value]) => value)
...
export const getMissingSetupUrlError = () =>
  new Error(
    "Setup completed, but Clawbridge did not receive the final Tailscale URL. Check Tailscale finalization and retry setup.",
  );
...
// Readiness polling for the final ts.net instance. A static asset is not a
// sufficient signal: the old setup process can keep serving it before its
// delayed service transition begins. This image endpoint returns a decodable
// SVG only from the successor runtime process and 503 from the setup process.
const kInstanceReadyProbePath = "/api/onboard/runtime-ready.svg";
const kInstanceReadyPollIntervalMs = 3000;
// The runtime-only endpoint makes a fixed grace period unnecessary: the old
// setup process remains unready however long systemd delays the transition.
const kInstanceReadyInitialDelayMs = 0;
const kInstanceReadyRequiredSuccesses = 2;
const kInstanceReadySlowStartHintDelayMs = 90000;
// Past this, surface tailnet troubleshooting without claiming it is the only
// cause: a reachable successor can also remain unready while its OpenClaw
// gateway recovers.
const kInstanceReadyTailnetHintDelayMs = 210000;

export const buildInstanceProbeUrl = (setupUrl, attempt = 0) => {
  try {
    const url = new URL(String(setupUrl || ""));
    return `${url.origin}${kInstanceReadyProbePath}?ready-probe=${attempt}`;
  } catch {
    return "";
  }
};

export const probeInstanceReady = (
  setupUrl,
  {
    attempt = 0,
  ...
  new Promise((resolve) => {
    const probeUrl = buildInstanceProbeUrl(setupUrl, attempt);
    if (!probeUrl) {
      resolve(false);
      return;
    }
    ...
    if (Number(timeoutMs) > 0) {
      timer = setTimeout(() => finish(false), Number(timeoutMs));
    }
...
export const waitForInstanceReady = async ({
  setupUrl,
  probe = probeInstanceReady,
  waitMs = wait,
  initialDelayMs = kInstanceReadyInitialDelayMs,
  intervalMs = kInstanceReadyPollIntervalMs,
  requiredSuccesses = kInstanceReadyRequiredSuccesses,
  slowStartHintDelayMs = kInstanceReadySlowStartHintDelayMs,
  tailnetHintDelayMs = kInstanceReadyTailnetHintDelayMs,
  onSlowStart,
  onTailnetHint,
  nowMs = () => Date.now(),
  isCancelled = () => false,
  ...
  let successes = 0;
  let attempt = 0;
  let slowStartHinted = false;
  let hinted = false;
  const getElapsedMs = () => {
    const wallClockElapsedMs = Number(nowMs()) - startedAtMs;
    return Math.max(
      elapsedMs,
      Number.isFinite(wallClockElapsedMs) ? wallClockElapsedMs : 0,
    );
  };
  if (Number(initialDelayMs) > 0) {
    await waitMs(Number(initialDelayMs));
    elapsedMs += Number(initialDelayMs);
  }
  while (!isCancelled()) {
    const ready = await probe(setupUrl, { attempt });
    attempt += 1;
    if (isCancelled()) return false;
    successes = ready ? successes + 1 : 0;
    if (successes >= Math.max(1, Number(requiredSuccesses) || 1)) {
      return true;
    }
    ...
    if (
      !slowStartHinted &&
      !ready &&
      currentElapsedMs >= Number(slowStartHintDelayMs)
    ) {
      slowStartHinted = true;
      onSlowStart?.();
    }
    if (
      !hinted &&
      !ready &&
      currentElapsedMs >= Number(tailnetHintDelayMs)
    ) {
      hinted = true;
      onTailnetHint?.();
    }
    await waitMs(Math.max(0, Number(intervalMs) || 0));
    elapsedMs += Math.max(0, Number(intervalMs) || 0);
  }
  return false;
};
...
export const isRecoverableOnboardCompletionError = (error) => {
  ...
  return (
    ...
    // A retried submit after a completed-but-interrupted onboard reaches an
    // instance whose marker is already written; route it through the status
    // poll so it resolves to the normal handoff instead of a dead-end error.
...
export const useWelcome = ({ onComplete }) => {
  ...
  const setupHandoffPollTokenRef = useRef(0);
  ...
  const claudeCliLoginIdRef = useRef("");
  ...
  const startClaudeCliAuth = useCallback(async () => {
    ...
    try {
      ...
      claudeCliLoginIdRef.current = result.id;
        ...
        onMessage: ({ event, data }) => {
          ...
          if (event === "done" || event === "error" || nextStatus !== "running") {
            ...
            if (!shouldAutoAdopt && payload.error) {
              setClaudeCliError(String(payload.error));
            }
            claudeCliLoginUnsubscribeRef.current?.();
            ...
            if (shouldAutoAdopt && !claudeCliLoginFinalizingRef.current) {
              ...
              (async () => {
                try {
                  ...
                  setClaudeCliError(
                    err.message || "Failed to clean up Claude login",
                  );
                } finally {
                  setClaudeCliActionLoading(false);
                  await refreshClaudeCliStatus({ silent: true });
  ...
  const cancelClaudeCliAuth = useCallback(async () => {
    const id = String(
      claudeCliLoginIdRef.current || claudeCliLoginId || "",
    ).trim();
    if (!id) return;
    setClaudeCliError("");
    claudeCliLoginUnsubscribeRef.current?.();
    ...
    try {
      ...
      setClaudeCliLoginState("cancelled");
    } catch (err) {
      setClaudeCliError(err.message || "Failed to cancel Claude login");
      setClaudeCliLoginState("error");
    } finally {
      claudeCliLoginIdRef.current = "";
      setClaudeCliLoginId("");
  ...
  }, [claudeCliLoginId, refreshClaudeCliStatus]);
  ...
  useEffect(
    () => () => {
      const activeLoginId = claudeCliLoginIdRef.current;
      if (activeLoginId) {
      ...
      claudeCliLoginPopupRef.current = null;
      claudeCliLoginFinalizingRef.current = false;
      setupHandoffPollTokenRef.current += 1;
    },
    [],
  );
  ...
  const getValidationContext = (currentVals = {}) => {
    ...
    const currentCredentialError = getAiCredentialErrorForFields(
      currentVals,
      currentProviderAuthFields,
    );
    ...
    return {
      ...
      credentialError: currentCredentialError,
      codexLoading,
      tailscaleApiToken,
  ...
  const beginSetupHandoff = useCallback(
    async (result) => {
      ...
      const token = ++setupHandoffPollTokenRef.current;
      const isCancelled = () => setupHandoffPollTokenRef.current !== token;
      // Poll in the background; the handoff UI holds the link back until the
      // final instance actually answers, then auto-redirects.
      (async () => {
        const ready = await waitForInstanceReady({
          setupUrl: nextHandoff.setupUrl,
          onSlowStart: () => {
            if (isCancelled()) return;
            setSetupHandoff((current) =>
              current ? { ...current, status: "delayed" } : current,
            );
          },
          onTailnetHint: () => {
            if (isCancelled()) return;
            setSetupHandoff((current) =>
              current ? { ...current, status: "tailnet-help" } : current,
            );
          },
          isCancelled,
        });
        ...
        setSetupHandoff((current) =>
          current ? { ...current, status: "redirecting" } : current,
        );
  ...
  const handleSubmit = async () => {
    ...
    if (loading) return;
    const vars = buildOnboardingVars(normalizedVals);
