...
// Readiness polling for the final ts.net instance. After onboarding completes
// the host swaps alphaclaw-setup.service for alphaclaw.service, leaving about
// a minute where the proxy answers 502 on the final URL. A no-cors fetch
// "completes" on that 502 (opaque responses hide the status), so the probe
// loads a known static image instead: it only decodes when the runtime is
// actually serving assets, and errors on 502 pages, closed connections, and
// off-tailnet Funnel resets alike.
const kInstanceReadyProbePath = "/img/logo.svg";
const kInstanceReadyPollIntervalMs = 3000;
// Grace period so a probe answered by the still-dying setup service (it keeps
// serving the same assets for a few seconds after the response) cannot
// declare readiness before the swap actually starts.
const kInstanceReadyInitialDelayMs = 8000;
const kInstanceReadyRequiredSuccesses = 2;
// Past this, the likely cause is the browser not being on the tailnet
// (Funnel publishes public DNS, but off-tailnet connections are closed).
...
export const waitForInstanceReady = async ({
  ...
  while (!isCancelled()) {
    ...
    if (!hinted && !ready && elapsedMs >= Number(tailnetHintDelayMs)) {
      hinted = true;
      onTailnetHint?.();
    }
