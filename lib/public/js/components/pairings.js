...
export const PairingRow = ({ p, onApprove, onReject }) => {
  ...
  const handle = async (action) => {
    setBusy(action);
    try {
      if (action === "approve") await onApprove(p.id, p.channel, p.accountId, p.requesterId);
...
// WhatsApp is omitted: it is not offered in the wizard and is denied on
// managed instances (see channels.js / vault-brokered-channels-spec.md §4),
// so it can never be an unpaired channel here.
...
export function Pairings({
  ...
  const handleApprove = async (p) => {
    await onApprove(p.id, p.channel, p.accountId, p.requesterId);
    hidePairing(p);
  };
