...
const VersionSummaryRow = ({
  ...
  return html`
    <div class="ac-surface-inset border border-border rounded-lg px-3 py-2">
      <p class="text-[12px] text-fg-muted">${label}</p>
...
export const UpdateModal = ({
  ...
  const canApplyUpdate = updateStrategy?.action === "self-update";
  ...
  const strategySteps = Array.isArray(updateStrategy?.steps)
    ...
    : [];
  const showStrategyDetails = Boolean(
    strategyDescription || strategySteps.length > 0,
  );
