...
const kKnownOnboardingModels = [
  ...
  {
    key: "openrouter/openai/gpt-5.6-sol",
    provider: "openrouter",
    label: "GPT-5.6 Sol",
    accessModes: ["gateway"],
  ...
  {
    key: "vercel-ai-gateway/openai/gpt-5.6-sol",
...
export const kFeaturedModelDefs = [
  {
    label: "Opus 5",
    preferredKeys: ["anthropic/claude-opus-5"],
  },
  {
    label: "Fable 5.1",
    preferredKeys: ["anthropic/claude-fable-5-1"],
  },
  {
    label: "Sonnet 4.6",
  ...
  const catalogRecommended = available
    .filter(
      (model) =>
        ...
        (!Array.isArray(model.recommendedAccessModes) ||
          model.recommendedAccessModes.length === 0 ||
          model.recommendedAccessModes.includes(accessMode)),
    )
    .map((model) => ({ ...model, featuredLabel: model.label || model.key }))
    // The catalog ranks recommendations in the support spec's order
    // (Opus first, then Fable, then Sonnet); fall back to the versioned-name
    // order only among rows without a rank.
    .sort((left, right) => {
      const rankCompare =
        (Number.isFinite(left?.recommendationRank) ? left.recommendationRank : 1000) -
        (Number.isFinite(right?.recommendationRank) ? right.recommendationRank : 1000);
      if (rankCompare !== 0) return rankCompare;
      return compareOnboardingModelsByVersionedName(left, right);
    });
