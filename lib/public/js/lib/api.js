...
export async function approvePairing(id, channel, accountId = "", requesterId = "") {
  const res = await authFetch(`/api/pairings/${id}/approve`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ channel, accountId, requesterId }),
  });
  return res.json();
}
...
export async function fetchGoogleProvider() {
  const res = await authFetch("/api/google/provider");
  return res.json();
}

export async function saveGoogleProvider(provider) {
  const res = await authFetch("/api/google/provider", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ provider }),
  });
  return parseJsonOrThrow(res, "Could not save Google provider");
}

export async function fetchComposioStatus() {
  const res = await authFetch("/api/composio/status");
  return res.json();
}

export async function refreshComposioStatus() {
  const res = await authFetch("/api/composio/refresh", { method: "POST" });
  return parseJsonOrThrow(res, "Could not refresh Composio status");
}

export async function startComposioLogin() {
  const res = await authFetch("/api/composio/login/start", { method: "POST" });
  return parseJsonOrThrow(res, "Could not start Composio sign-in");
}

export async function enableComposioGmailWatch() {
  const res = await authFetch("/api/composio/gmail-watch/enable", {
    method: "POST",
  });
  return parseJsonOrThrow(res, "Could not enable email notifications");
}

export async function disableComposioGmailWatch() {
  const res = await authFetch("/api/composio/gmail-watch/disable", {
    method: "POST",
  });
  return parseJsonOrThrow(res, "Could not disable email notifications");
}

export async function linkComposioToolkit(toolkit, { alias = "" } = {}) {
  const res = await authFetch("/api/composio/link", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ toolkit, ...(alias ? { alias } : {}) }),
  });
  return parseJsonOrThrow(res, "Could not start Composio link");
}
...
export async function fetchAdvancedControlAccessStatus() {
  const res = await authFetch("/api/advanced-control/status");
  return parseJsonOrThrow(res, "Could not load advanced access status");
}

export async function acknowledgeAdvancedControlAccess(returnTo = "/openclaw") {
  const res = await authFetch("/api/advanced-control/acknowledge", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ returnTo }),
  });
  return parseJsonOrThrow(res, "Could not record advanced access acknowledgement");
}
...
export async function fetchAlphaclawReleaseNotes(tag = "") {
  ...
  try {
    ...
    const endpoint = normalizedTag
      ? `https://api.github.com/repos/starfoundrystudio/alphaclaw/releases/tags/${encodeURIComponent(normalizedTag)}`
      : "https://api.github.com/repos/starfoundrystudio/alphaclaw/releases/latest";
...
export async function updateOpenAiCompatApiFeature(enabled) {
  ...
  try {
    ...
    throw new Error(text || "Could not parse Clawbridge config response");
  }
  if (!res.ok) {
...
export async function runOnboard(
  ...
  if (!text.trim()) {
    const error = new Error(
      "Setup response was interrupted after initialization. Checking completion status...",
    );
    error.code = "ONBOARD_RESPONSE_EMPTY";
  ...
  try {
    ...
    error.code = "ONBOARD_RESPONSE_INVALID_JSON";
    error.status = res.status;
...
export const refreshModels = async (scope = "all") => {
  const res = await authFetch("/api/models/refresh", {
    method: "POST",
    headers: { "content-type": "application/json" },
  ...
  if (!res.ok || payload?.ok === false) {
    const error = new Error(payload?.error || "Failed to refresh model catalog");
    error.status = res.status;
...
export const fetchClaudeCliStatus = async () => {
  const res = await authFetch("/api/account-logins/claude-cli/status");
  return res.json();
};

export const startClaudeCliLogin = async () => {
  const res = await authFetch("/api/account-logins/claude-cli/login/start", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  });
  return res.json();
...
export const subscribeClaudeCliLoginEvents = ({
  id = "",
  onMessage = () => {},
  onError = () => {},
}) =>
  subscribeToSse({
    url: `/api/account-logins/claude-cli/login/${encodeURIComponent(String(id || ""))}/events`,
    onMessage,
    onError,
  });

export const submitClaudeCliLoginInput = async ({ id = "", input = "" } = {}) => {
  const res = await authFetch(
    `/api/account-logins/claude-cli/login/${encodeURIComponent(String(id || ""))}/input`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export const cancelClaudeCliLogin = async ({
  id = "",
  keepalive = false,
} = {}) => {
  const res = await authFetch(
    `/api/account-logins/claude-cli/login/${encodeURIComponent(String(id || ""))}/cancel`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      keepalive,
...
export const adoptClaudeCli = async () => {
  const res = await authFetch("/api/account-logins/claude-cli/adopt", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  });
  return res.json();
...
export const disconnectClaudeCli = async () => {
  const res = await authFetch("/api/account-logins/claude-cli/disconnect", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  });
  return res.json();
...
export const inspectTelegramBotToken = async (token) => {
  const res = await authFetch("/api/channels/telegram/inspect-token", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ token: String(token || "").trim() }),
  });
  return parseJsonOrThrow(res, "Could not verify Telegram bot token");
};

export const inspectDiscordBotToken = async (token) => {
  const res = await authFetch("/api/channels/discord/inspect-token", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ token: String(token || "").trim() }),
  });
  return parseJsonOrThrow(res, "Could not verify Discord bot token");
};

export const inspectSlackCredentials = async ({ botToken, appToken } = {}) => {
  const res = await authFetch("/api/channels/slack/inspect-credentials", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
  ...
  return parseJsonOrThrow(res, "Could not verify Slack credentials");
};

export const requestChannelVaultToken = async ({ provider, accountId } = {}) => {
  const res = await authFetch("/api/channels/vault-token", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ provider, accountId }),
  });
  return parseJsonOrThrow(res, "Could not request the Agent Vault channel token");
};
...
export const updateChannelAccountPolicy = async (payload) => {
  const res = await authFetch("/api/channels/accounts/policy", {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload || {}),
  });
  return parseJsonOrThrow(res, "Could not update channel access policy");
};
...
export const fetchAgentSkills = async (agentId) => {
  const res = await authFetch(
    `/api/agents/${encodeURIComponent(String(agentId || ""))}/skills`,
  );
  return parseJsonOrThrow(res, "Could not load agent skills");
};
...
export const startCodexDeviceAuth = async () => {
  const res = await authFetch("/api/codex/device/start", { method: "POST" });
  return res.json();
};

export const pollCodexDeviceAuth = async (sessionId) => {
  const res = await authFetch("/api/codex/device/poll", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function fetchAgentVaultStatus() {
  const res = await authFetch("/api/agent-vault/status");
  return parseJsonOrThrow(res, "Could not load Agent Vault status");
}

export async function fetchAgentVaultCredentials() {
  const res = await authFetch("/api/agent-vault/credentials");
  return parseJsonOrThrow(res, "Could not load Agent Vault credentials");
}

export async function createAgentVaultProposal(access) {
  const res = await authFetch("/api/agent-vault/proposals", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(access),
  });
  return parseJsonOrThrow(res, "Could not create Agent Vault proposal");
}

export async function fetchAgentVaultProposal(proposalId) {
  const res = await authFetch(
    `/api/agent-vault/proposals/${encodeURIComponent(String(proposalId))}`,
  );
  return parseJsonOrThrow(res, "Could not load Agent Vault proposal");
}

export async function requestModelVaultKey(provider, { agentId } = {}) {
  const query = agentId ? `?agentId=${encodeURIComponent(agentId)}` : "";
  const res = await authFetch(`/api/models/vault-key${query}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ provider }),
  });
  return parseJsonOrThrow(res, "Could not request the Agent Vault model key");
}
