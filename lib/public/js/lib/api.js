...
export async function configureGithubSync({
  ...
  const res = await authFetch("/api/github-sync/config", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ repo, token, schedule }),
  });
  const text = await res.text();
  let data;
  try {
    ...
    throw new Error(text || "Could not parse GitHub sync response");
  }
  if (!res.ok) {
    throw new Error(data.error || text || `HTTP ${res.status}`);
  }
...
export async function runOnboard(
  vars,
  modelKey,
  { agentRuntimeId = null, importMode = false, tailscaleApiToken = "" } = {},
) {
