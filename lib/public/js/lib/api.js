...
export async function linkComposioToolkit(toolkit) {
...
export async function fetchTailscaleStatus() {
  ...
  return parseJsonOrThrow(res, "Could not load Tailscale status");
}

export async function validateTailnetChange(tailscaleApiToken) {
  const res = await authFetch("/api/tailscale/change/validate", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ tailscaleApiToken }),
  });
  return parseJsonOrThrow(res, "Could not validate the new tailnet");
}

export async function startTailnetChange({
  tailscaleApiToken,
  expectedCurrentDns,
}) {
  const res = await authFetch("/api/tailscale/change", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      tailscaleApiToken,
  ...
  return parseJsonOrThrow(res, "Could not start the tailnet change");
}
