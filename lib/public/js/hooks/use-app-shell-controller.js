...
export const useAppShellController = ({ location = "" } = {}) => {
  ...
  const [initialRuntimePending, setInitialRuntimePending] = useState(false);
  ...
  const uiSandbox = sharedStatus?.uiSandbox || null;
  ...
  useEffect(() => {
    fetchOnboardStatus()
      .then((data) => {
        setInitialRuntimePending(data.initialRuntimePending === true);
        setOnboarded(data.onboarded);
        setWorkspaceBootstrap(
          data.workspaceBootstrap && typeof data.workspaceBootstrap === "object"
  ...
  useEffect(() => {
    if (!initialRuntimePending) return;
    let cancelled = false;
    let timer;
    const poll = async () => {
      try {
        ...
        if (cancelled) return;
        if (data.onboarded && data.initialRuntimePending === false) {
          setInitialRuntimePending(false);
          return;
        }
      } catch { /* Keep the setup gate closed while the service restarts. */ }
  ...
  const handleAcUpdate = useCallback(async () => {
    ...
    try {
      ...
      if (data.ok) {
        showToast("Clawbridge updated — restarting...", "success");
        setTimeout(() => window.location.reload(), 5000);
      } else {
        showToast(data.error || "Clawbridge update failed", "error");
        setAcUpdating(false);
      }
    } catch (err) {
      showToast(err.message || "Could not update Clawbridge", "error");
      setAcUpdating(false);
    }
  }, [acUpdating]);
