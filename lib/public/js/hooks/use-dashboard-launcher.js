...
export const useDashboardLauncher = ({ gatewayStatus = "" } = {}) => {
    ...
    try {
      const opened =
        typeof window !== "undefined" && typeof window.open === "function"
          ...
          : null;
      if (!opened) {
        showToast("Your browser blocked the OpenClaw tab. Copy the link or allow popups.", "warning");
      }
    } catch (err) {
  ...
  const copyDashboardLink = useCallback(async () => {
    const copied = await copyTextToClipboard(dashboardUrl || "/openclaw");
    showToast(copied ? "Dashboard link copied" : "Could not copy dashboard link", copied ? "success" : "error");
  }, [dashboardUrl]);
