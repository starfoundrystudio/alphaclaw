...
import {
  acknowledgeAdvancedControlAccess,
  approveDevice,
  fetchAdvancedControlAccessStatus,
  fetchDashboardUrl,
  fetchDevicePairings,
  rejectDevice,
} from "../lib/api.js";
...
const kActiveDevicePollMs = 3000;
...
export const useDashboardLauncher = ({ gatewayStatus = "" } = {}) => {
  ...
  const openLauncher = useCallback(async () => {
    ...
    try {
      devicePoll.refresh({ force: true });
      const [data, accessStatus] = await Promise.all([
        fetchDashboardUrl(),
        fetchAdvancedControlAccessStatus(),
      ]);
      const nextState = getDashboardUrlState(data);
      setDashboardUrl(nextState.url);
      setAccessAcknowledged(accessStatus?.acknowledged === true);
      setStatus(nextState.status);
    } catch (err) {
      setError(err.message || "Could not load advanced OpenClaw access");
  ...
  const openDashboardTab = useCallback(async () => {
    const url = dashboardUrl || "/openclaw";
    setStatus(kDashboardLauncherStatuses.OPENING);
    let opened = null;
    try {
      opened = openDashboardWindow(accessAcknowledged ? url : "about:blank");
      if (!opened) {
        showToast(
          "Your browser blocked the OpenClaw tab. Allow popups for Clawbridge and try again.",
          "warning",
        );
      ...
      if (!accessAcknowledged) {
        ...
        const acknowledgement = await acknowledgeAdvancedControlAccess(url);
        setAccessAcknowledged(true);
        const acknowledgedUrl = String(acknowledgement?.url || url);
        if (opened.location?.replace) opened.location.replace(acknowledgedUrl);
        else opened.location = acknowledgedUrl;
      }
    } catch (err) {
      try {
        opened?.close?.();
      } catch {}
      showToast(err.message || "Could not open advanced OpenClaw controls", "error");
      setStatus(kDashboardLauncherStatuses.READY);
      return;
    } finally {
      setAcknowledging(false);
    }
    if (readOpenClawBrowserAuthState().hasOperatorToken) {
      closeLauncher();
      return;
    }
    startWaitingForPairing();
  }, [accessAcknowledged, closeLauncher, dashboardUrl, startWaitingForPairing]);
