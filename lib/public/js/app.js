...
import { DashboardLauncherModal } from "./components/dashboard-launcher-modal.js";
...
import { useDashboardLauncher } from "./hooks/use-dashboard-launcher.js";
import {
  getHashRouterPath,
  useHashLocation,
...
const App = () => {
  ...
  const {
    state: dashboardLauncherState,
    actions: dashboardLauncherActions,
  } = useDashboardLauncher({
    gatewayStatus: controllerState.sharedStatus?.gateway || "",
  });
  ...
  const isCredentialsRoute = location.startsWith("/credentials");
  ...
  const agentDetailTab = (() => {
    ...
    return ["tools", "skills"].includes(tab) ? tab : "overview";
  })();
  ...
  const footerVersion = (() => {
    ...
    if (openclawVersion && alphaclawVersion) {
      return `OpenClaw ${openclawVersion} / Clawbridge ${alphaclawVersion}`;
    }
    if (openclawVersion) {
      return `OpenClaw ${openclawVersion}`;
    }
    if (alphaclawVersion) {
      return `Clawbridge ${alphaclawVersion}`;
    }
    return null;
  })();

  // First-run onboarding: until the agent's bootstrap ritual has completed,
  // a bare base URL should land on the chat screen so the user meets their
  // agent immediately. Only fires when the URL carries no explicit hash, so
  // deep links and user navigation always win.
  useEffect(() => {
    if (controllerState.onboarded !== true) return;
    const bootstrap = controllerState.workspaceBootstrap;
    if (!bootstrap || bootstrap.complete !== false) return;
    if (window.location.hash) return;
    setLocation("/chat");
  }, [
    controllerState.onboarded,
    controllerState.workspaceBootstrap,
    setLocation,
  ]);
  ...
  if (controllerState.initialRuntimePending) {
    return html`
      <div class="min-h-screen flex flex-col items-center justify-center px-4 text-center gap-3">
        <${LoadingSpinner} className="h-7 w-7 text-body" />
        <h2 class="text-lg font-semibold text-body">Your instance is starting...</h2>
        <p class="text-sm text-fg-muted">Clawbridge is waiting for chat and Agent Vault to be ready. Your dashboard will open automatically.</p>
      </div>
    `;
  }
  ...
  return html`
    ...
    >
      ...
      <${AppSidebar}
        ...
        dashboardLauncherState=${dashboardLauncherState}
        onOpenDashboardLauncher=${dashboardLauncherActions.openLauncher}
      ...
      <div class="app-content">
        ...
        >
          ...
          <span class="mobile-topbar-title">
            <span style="color: var(--accent)">claw</span>bridge
          </span>
        </div>
          ...
          ? html`
              <div class="app-content-pane chat-pane">
                <${ChatRoute}
                  sessions=${chatSessions}
                  selectedSessionKey=${selectedChatSessionKey}
                  bootstrapPending=${controllerState.workspaceBootstrap
                    ?.complete === false}
                  channelsConfigured=${controllerState.sharedStatus?.channels
                    ? Object.values(
                        controllerState.sharedStatus.channels,
                      ).some(
                        (channel) =>
                          channel &&
                          String(channel.status || "") !== "" &&
                          String(channel.status) !== "missing",
                      )
                    : null}
                />
              </div>
      ...
      <div class="app-statusbar">
        <div class="statusbar-left">
          ${controllerState.uiSandbox?.enabled
            ? html`
                <span
                  class="inline-flex items-center rounded-full border border-status-warning-border bg-status-warning-bg px-2 py-0.5 text-[11px] font-medium text-status-warning"
                  title=${`Sandbox mode: ${controllerState.uiSandbox.scenario || "healthy"}`}
                >
                  Sandbox: ${controllerState.uiSandbox.scenario || "healthy"}
                </span>
              `
            : null}
          ${footerVersion
