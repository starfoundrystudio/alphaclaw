...
const runOpenclawCommandWithConfigConflictRetry = ({
  logger = console,
  maxAttempts = 2,
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      return runOpenclawCommand(params);
    } catch (error) {
      lastError = error;
      if (
        attempt >= attempts ||
        !isOpenclawConfigMutationConflictError(error)
      ) {
        throw error;
      }
      logger.log(
        `[alphaclaw] ${retryLabel} hit an OpenClaw config write conflict; retrying with a fresh config load (${attempt + 1}/${attempts})`,
      );
    }
  }
  throw lastError;
...
const installManagedPluginWithConfigSuppression = ({
  ...
  try {
    ...
    runOpenclawCommandWithConfigConflictRetry({
      ...commandOptions,
      logger,
      retryLabel: `${key} plugin install with temporary config suppression`,
      args: [
        "plugins",
        "install",
        buildInstallSpec(definition),
