...
} = require("../server/teamyou-memory-activation");
const {
  reconcileTeamyouMemoryPlugin,
} = require("./teamyou-memory-plugin-install");
const { stripOpenclawNoise } = require("../server/openclaw-cli-output");
const { applyOpenclawPluginHotfixes } = require("./openclaw-plugin-hotfixes");
const {
  buildAgentVaultRuntimeEnv,
} = require("../server/agent-vault/runtime-store");
const {
...
const buildOpenclawEnv = ({ rootDir, openclawDir, env = process.env }) => withOpenclawMaintenanceEnv({
  ...env,
  // Once the Agent Vault handoff enables proxy mode in openclaw.json, every
  // openclaw CLI invocation needs OPENCLAW_PROXY_URL (and the MITM CA trust)
  // from the vault runtime env or the CLI refuses to start with "proxy:
  // enabled but no HTTP proxy URL is configured". Empty when no vault
  // runtime exists, so non-gateway instances are unaffected.
  ...buildAgentVaultRuntimeEnv(),
  HOME: env.HOME || rootDir,
  OPENCLAW_HOME: rootDir,
  CODEX_HOME: resolveManagedCodexHome({ rootDir, env, fsModule: fs }),
...
const firstMeaningfulLine = (value) =>
  String(value || "")
    ...
    .find(
      (line) =>
        line &&
        !/^\[?config\]?\s*warnings?:/i.test(line) &&
        !/^Config warnings:/i.test(line),
    ) || "";

// Each OpenClaw CLI start costs 13-17 s on managed hosts with 2026.9.5, and a
// Slack add took ~8 minutes (G3 finding #28) with no record of which command
// spent the time or failed. When a commandLogger is given, log every command
// with its duration and, on failure, its first meaningful error line.
const runOpenclawCommand = ({
  ...
  const command = [quoteArg(process.execPath), quoteArg(openclawCliPath)]
    ...
    .join(" ");
  const label = `openclaw ${args.join(" ")}`.slice(0, 200);
  const startedAt = Date.now();
  const seconds = () => ((Date.now() - startedAt) / 1000).toFixed(1);
  try {
    const output = execSyncImpl(command, {
      cwd,
      env,
      encoding: "utf8",
    ...
    commandLogger?.log?.(`[alphaclaw] ${label}: ok in ${seconds()}s`);
    return output;
  } catch (error) {
    const detail =
    ...
    commandLogger?.log?.(
      `[alphaclaw] ${label}: failed in ${seconds()}s${detail ? `: ${detail.slice(0, 300)}` : ""}`,
    );
    throw error;
  }
};

const isOpenclawConfigMutationConflictError = (error) => {
  const text = [error?.message, error?.stderr, error?.stdout]
    .filter(Boolean)
    .map(String)
    .join("\n");
  return /config (?:path )?changed since last load/i.test(text);
};

const isOpenclawPluginAlreadyExistsError = (error) => {
  const text = [error?.message, error?.stderr, error?.stdout]
    .filter(Boolean)
    .map(String)
    .join("\n");
  return /plugin already exists:/i.test(text);
};

const isPluginInstallCommand = (args = []) =>
  args[0] === "plugins" && args[1] === "install";

const runOpenclawCommandWithPluginRecovery = ({
  logger = console,
  maxAttempts = 3,
  retryLabel = "OpenClaw command",
  isAlreadyConverged = null,
  ...params
}) => {
  let lastError;
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      return runOpenclawCommand({ ...params, args });
    } catch (error) {
      lastError = error;
      const configConflict = isOpenclawConfigMutationConflictError(error);
      const alreadyExists = isOpenclawPluginAlreadyExistsError(error);
      // OpenClaw 2026.9 installs an enabled plugin by itself, so "already
      // exists" can mean the target version is already in place. A forced
      // reinstall then blocks on OpenClaw's pending plugin migration until the
      // command times out (G3 #34, host 08). Check before forcing.
      if (alreadyExists && typeof isAlreadyConverged === "function") {
        let converged = false;
        try {
          converged = isAlreadyConverged() === true;
        } catch {}
        if (converged) {
          logger.log(
            `[alphaclaw] ${retryLabel}: OpenClaw already has the target version installed; not forcing a reinstall`,
          );
          return "";
        }
      }
      const canForceInstall =
        isPluginInstallCommand(args) && !args.includes("--force");
      if (
        attempt >= attempts ||
        (!configConflict && !(alreadyExists && canForceInstall))
      ) {
        throw error;
      }
      if (canForceInstall) {
      ...
      logger.log(
        configConflict
          ? `[alphaclaw] ${retryLabel} hit an OpenClaw config write conflict; retrying with a fresh config load${canForceInstall ? " and forced managed replacement" : ""} (${attempt + 1}/${attempts})`
          : `[alphaclaw] ${retryLabel} found a partial managed install; retrying with forced replacement (${attempt + 1}/${attempts})`,
      );
    }
  }
  throw lastError;
...
const parsePluginList = (output) => {
  ...
  try {
    ...
    throw new Error(
      `OpenClaw plugin inventory returned invalid JSON: ${error.message}`,
    );
  }
  if (!payload || !Array.isArray(payload.plugins)) {
    throw new Error("OpenClaw plugin inventory is missing the plugins array");
  }
...
const getWebSearchProviderEnvVars = (definition) =>
  uniqueStrings(definition.webSearchProviderEnvVars || []);
...
const getPluginRelevanceReasons = ({
  ...
  // Any entry counts, including `{enabled: false}`: gated plugins (TeamYou
  // memory before activation) are installed disabled on purpose. Runtime
  // flows use `onlyPluginKeys` so a stray entry cannot hijack them.
  ...
  if (config.tools?.web?.search?.enabled !== false) {
    for (const envVar of getWebSearchProviderEnvVars(definition)) {
      if (String(env?.[envVar] || "").trim()) {
        reasons.push(`web-search-env:${envVar}`);
      }
    }
  }
...
const buildUpdateSpec = (definition) =>
  definition.install?.exactNpmSpec ||
  `${definition.package}@${definition.version}`;

const buildPluginMutationArgs = ({ definition, installed }) =>
  installed
    ? [
        "plugins",
        "update",
        buildUpdateSpec(definition),
        "--accept-capabilities",
      ]
    : [
        "plugins",
...
const isOpenclawConfigReferenceError = (error) => {
  // Classify on the real error only: OpenClaw 2026.9 prints config warnings
  // ("plugins.deny: plugin not found: …") on stderr even on success, which
  // made every install failure look like a config-reference error (G3 #21).
  const text = [
    error?.message,
    error?.stdout,
    error?.stderr,
...
const installManagedPluginWithConfigSuppression = ({
  ...
  try {
    ...
    const updateInstalled = installed || suppressedInstalled;
    runOpenclawCommandWithPluginRecovery({
      ...commandOptions,
      logger,
      retryLabel: `${key} plugin ${updateInstalled ? "update" : "install"} with temporary config suppression`,
      args: buildPluginMutationArgs({
        definition,
...
const enforcePendingTeamyouBootstrapGate = ({
  fsModule,
  logger,
  openclawDir,
}) => {
  const result = enforceTeamyouBootstrapGateIfPending({
    fsModule,
    openclawDir,
  });
  if (result.changed) {
    logger.log(
      `[alphaclaw] Re-applied pending TeamYou bootstrap gate after plugin reconciliation (${result.reason})`,
    );
  }
  return result;
};
...
const reconcileOpenclawPlugins = ({
  ...
  // Only callers that run while no Gateway is up may set this: OpenClaw 2026.9
  // hands plugin installs to a running Gateway, which refuses them under the
  // read-only config guard.
  installTeamyouMemoryPlugin = false,
  reconcileTeamyouMemoryPluginImpl = reconcileTeamyouMemoryPlugin,
  // Runtime flows (channel add, model save) pass the plugin(s) they need so
  // one unrelated plugin's install failure cannot abort them (G3 #21).
  // Startup reconciles everything relevant.
  onlyPluginKeys = null,
} = {}) => {
  ...
  const pluginKeys = Object.entries(manifest.managedPlugins)
    .filter(([key, definition]) => {
      ...
      if (Array.isArray(onlyPluginKeys)) {
        const wanted = new Set(onlyPluginKeys.map(String));
        const ids = [key, definition.pluginId, definition.channelId].filter(Boolean);
        if (!ids.some((id) => wanted.has(id))) return false;
        // A runtime flow asked for this plugin before referencing it in the
        // config. Channel add installs first and writes the channel config
        // afterwards: enabling the entry first made OpenClaw 2026.9 install
        // the package itself and collide with this install (G3 #34).
        if (reasons.length === 0) reasons.push("requested");
      }
  ...
  const pluginResults = [];
  // A targeted reconcile keeps the lock entries of plugins it did not look at.
  const nextLockPlugins = Array.isArray(onlyPluginKeys)
    ...
    if (needsInstall) {
      ...
      try {
        runOpenclawCommandWithPluginRecovery({
          ...commandOptions,
          logger,
          retryLabel: `${key} plugin ${installed ? "update" : "install"}`,
          args: buildPluginMutationArgs({ definition, installed }),
          isAlreadyConverged: () => {
            const current = findInstalledPlugin(
              listInstalledPlugins(commandOptions),
              key,
              definition,
            );
            return String(current?.version || "") === definition.version;
          },
        });
      } catch (error) {
      ...
      const reconciledPlugin = findInstalledPlugin(
        installedPlugins,
        key,
        definition,
      );
      if (!reconciledPlugin) {
        throw new Error(
          `OpenClaw plugin inventory did not contain ${key} after reconciliation`,
        );
      }
      if (String(reconciledPlugin.version || "") !== definition.version) {
        throw new Error(
          `OpenClaw plugin inventory reported ${key}@${reconciledPlugin.version || "unknown"} after reconciling ${definition.version}`,
        );
      }
      enforcePendingTeamyouBootstrapGate({
        fsModule,
        logger,
        openclawDir,
      });
  ...
  if (installTeamyouMemoryPlugin) {
    const teamyouResult = reconcileTeamyouMemoryPluginImpl({
      openclawDir,
      env,
      fsModule,
      logger,
      runOpenclaw: (args) => runOpenclawCommand({ ...commandOptions, args }),
    });
    if (teamyouResult.reason !== "not_configured") {
      pluginResults.push({ ...teamyouResult, reasons: ["clawctl_handoff"] });
    }
  }

  // Version-pinned upstream workarounds on installed plugin files (e.g. the
  // 2026.9.5 Slack Socket Mode proxy bug). Runs on every reconcile, so a
  // freshly installed plugin is patched before the Gateway's next start.
  let hotfixes = [];
  try {
    hotfixes = applyOpenclawPluginHotfixes({ openclawDir, fsModule, logger }) || [];
  } catch (error) {
    logger.warn?.(`[alphaclaw] OpenClaw plugin hotfixes failed: ${error.message}`);
  }
