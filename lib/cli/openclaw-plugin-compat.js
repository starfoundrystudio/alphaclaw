...
const collectConfigSuppressionsForManagedPlugin = ({
  ...
  const providerIds = (definition.providerIds || []).map(String);
  const webSearchProviderIds = (definition.webSearchProviderIds || []).map(String);
  ...
  const isProtectedPath = (pathParts) =>
    ...
    isProtectedProviderConfigPath(pathParts, providerIds) ||
    isPathPrefix(pathParts, ["channels"]);

  if (
    webSearchProviderIds.includes(
