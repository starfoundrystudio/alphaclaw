...
const repairConflictingOpenclawPluginIndex = {
  ...
  description:
    "OpenClaw 2026.7.1 refuses to migrate a legacy plugin install index when " +
...
const normalizeCodexPluginCompatibility = {
  ...
  target: "openclaw.json",
  description:
    "OpenClaw 2026.7.1 retired Codex plugin settings that were valid in older " +
  ...
  check({ fsModule = fs, openclawDir }) {
    ...
    if (paths.length === 0) {
      return {
        status: "ok",
        message: "Codex plugin config is compatible with OpenClaw 2026.7.1",
      };
    }
    return {
      status: "pending",
