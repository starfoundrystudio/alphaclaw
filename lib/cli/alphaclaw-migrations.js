const fs = require("fs");
const path = require("path");
const {
  ensureManagedChannelApprovalDefaults,
  ensureManagedPluginApprovalDefaults,
} = require("../server/exec-defaults-config");
const {
  inspectPluginIndexConflict,
  repairPluginIndexConflict,
} = require("./openclaw-startup-state-repair");
const {
  ensureManagedOpenclawDefaults,
} = require("../server/managed-defaults-config");

const kMigrationLedgerDir = "migrations";
const kMigrationLedgerFileName = "alphaclaw-migrations.jsonl";
const kMigrationFailureBlockThreshold = 3;

const isObject = (value) =>
  !!value && typeof value === "object" && !Array.isArray(value);

const hasOwn = (value, key) =>
  isObject(value) && Object.prototype.hasOwnProperty.call(value, key);

const normalizeString = (value) => String(value || "").trim();

const resolveMigrationLedgerPath = ({ rootDir }) =>
  path.join(rootDir, kMigrationLedgerDir, kMigrationLedgerFileName);

const writeJsonFile = ({ fsModule = fs, filePath, value }) => {
  fsModule.mkdirSync(path.dirname(filePath), { recursive: true });
  fsModule.writeFileSync(
    filePath,
    `${JSON.stringify(value, null, 2)}\n`,
    "utf8",
  );
};

const readMigrationLedger = ({ fsModule = fs, rootDir }) => {
  const ledgerPath = resolveMigrationLedgerPath({ rootDir });
  try {
    const raw = fsModule.readFileSync(ledgerPath, "utf8");
    return raw
      .split(/\r?\n/)
      .map((line) => line.trim())
      .filter(Boolean)
      .map((line) => {
        try {
          return JSON.parse(line);
        } catch {
          return null;
        }
      })
      .filter(Boolean);
  } catch {
    return [];
  }
};

const appendMigrationLedgerEntry = ({
  fsModule = fs,
  rootDir,
  entry,
  now = new Date(),
}) => {
  const ledgerPath = resolveMigrationLedgerPath({ rootDir });
  fsModule.mkdirSync(path.dirname(ledgerPath), { recursive: true });
  const payload = {
    timestamp: now.toISOString(),
    ...entry,
  };
  fsModule.appendFileSync(ledgerPath, `${JSON.stringify(payload)}\n`, "utf8");
  return payload;
};

const getRecentFailureCount = ({ ledger = [], migrationId }) => {
  let count = 0;
  for (let index = ledger.length - 1; index >= 0; index -= 1) {
    const entry = ledger[index];
    if (entry?.id !== migrationId) continue;
    if (entry.status === "completed") return count;
    if (entry.status === "failed") count += 1;
  }
  return count;
};

const repairConflictingOpenclawPluginIndex = {
  id: "2026-07-repair-openclaw-plugin-index-conflicts",
  title: "Repair conflicting OpenClaw plugin install metadata",
  scope: "state",
  target: "state/openclaw.sqlite and plugins/installs.json",
  description:
    "OpenClaw (since 2026.7.1) refuses to migrate a legacy plugin install index when " +
    "the shared SQLite registry already contains conflicting metadata. Preserve " +
    "both sources, reset only the derived SQLite row, and let OpenClaw doctor " +
    "perform the authoritative migration.",
  check({ fsModule = fs, openclawDir }) {
    const inspection = inspectPluginIndexConflict({ fsModule, openclawDir });
    if (!inspection.pending) {
      return {
        status: "ok",
        message: "OpenClaw plugin install metadata does not conflict",
      };
    }
    return {
      status: "pending",
      message:
        "Reset conflicting SQLite plugin install metadata before OpenClaw doctor",
      details: {
        conflictingPluginIds: inspection.conflictingPluginIds,
        legacyPath: inspection.legacyPath,
        databasePath: inspection.databasePath,
      },
    };
  },
  apply({ fsModule = fs, rootDir, openclawDir, now }) {
    return repairPluginIndexConflict({
      fsModule,
      rootDir,
      openclawDir,
      now,
    });
  },
};

const removeActiveMemoryModelFallbackPolicy = {
  id: "2026-06-remove-active-memory-model-fallback-policy",
  title: "Remove deprecated Active Memory modelFallbackPolicy",
  scope: "config",
  target: "openclaw.json",
  description:
    "Clawbridge used to write plugins.entries.active-memory.config.modelFallbackPolicy. " +
    "OpenClaw now ignores it and logs a warning when it is present.",
  check({ fsModule = fs, openclawDir }) {
    const configPath = path.join(openclawDir, "openclaw.json");
    if (!fsModule.existsSync(configPath)) {
      return {
        status: "ok",
        message: "openclaw.json is missing; nothing to migrate",
      };
    }
    const config = JSON.parse(fsModule.readFileSync(configPath, "utf8"));
    const activeMemoryConfig = config.plugins?.entries?.["active-memory"]?.config;
    if (
      !isObject(activeMemoryConfig) ||
      !Object.prototype.hasOwnProperty.call(activeMemoryConfig, "modelFallbackPolicy")
    ) {
      return {
        status: "ok",
        message: "Deprecated Active Memory modelFallbackPolicy is absent",
      };
    }
    return {
      status: "pending",
      message:
        "Remove plugins.entries.active-memory.config.modelFallbackPolicy from openclaw.json",
      details: {
        path: "plugins.entries.active-memory.config.modelFallbackPolicy",
      },
    };
  },
  apply({ fsModule = fs, openclawDir }) {
    const configPath = path.join(openclawDir, "openclaw.json");
    if (!fsModule.existsSync(configPath)) {
      return {
        changed: false,
        changes: ["Skipped openclaw.json because it is missing."],
      };
    }
    const config = JSON.parse(fsModule.readFileSync(configPath, "utf8"));
    const activeMemoryConfig = config.plugins?.entries?.["active-memory"]?.config;
    if (
      !isObject(activeMemoryConfig) ||
      !Object.prototype.hasOwnProperty.call(activeMemoryConfig, "modelFallbackPolicy")
    ) {
      return {
        changed: false,
        changes: ["Deprecated Active Memory modelFallbackPolicy was already absent."],
      };
    }
    delete activeMemoryConfig.modelFallbackPolicy;
    writeJsonFile({ fsModule, filePath: configPath, value: config });
    return {
      changed: true,
      changes: [
        "Removed plugins.entries.active-memory.config.modelFallbackPolicy from openclaw.json.",
      ],
    };
  },
};

const findLegacyCodexPluginConfigPaths = (config) => {
  const paths = [];
  const pluginConfig = config.plugins?.entries?.codex?.config;
  if (!isObject(pluginConfig)) return paths;

  if (hasOwn(pluginConfig, "codexDynamicToolsProfile")) {
    paths.push("plugins.entries.codex.config.codexDynamicToolsProfile");
  }

  const codexPlugins = pluginConfig.codexPlugins;
  if (isObject(codexPlugins)) {
    if (codexPlugins.allow_destructive_actions === "on-request") {
      paths.push(
        "plugins.entries.codex.config.codexPlugins.allow_destructive_actions",
      );
    }
    if (isObject(codexPlugins.plugins)) {
      for (const [pluginId, plugin] of Object.entries(codexPlugins.plugins)) {
        if (
          isObject(plugin) &&
          plugin.allow_destructive_actions === "on-request"
        ) {
          paths.push(
            `plugins.entries.codex.config.codexPlugins.plugins.${pluginId}.allow_destructive_actions`,
          );
        }
      }
    }
  }

  if (pluginConfig.appServer?.approvalPolicy === "on-failure") {
    paths.push("plugins.entries.codex.config.appServer.approvalPolicy");
  }

  return paths;
};

const normalizeCodexPluginCompatibilityConfig = (config) => {
  const pluginConfig = config.plugins?.entries?.codex?.config;
  if (!isObject(pluginConfig)) {
    return { changed: false, changes: [] };
  }

  const changes = [];
  if (hasOwn(pluginConfig, "codexDynamicToolsProfile")) {
    delete pluginConfig.codexDynamicToolsProfile;
    changes.push(
      "Removed retired plugins.entries.codex.config.codexDynamicToolsProfile.",
    );
  }

  const codexPlugins = pluginConfig.codexPlugins;
  if (isObject(codexPlugins)) {
    let rewrittenDestructivePolicies = 0;
    if (codexPlugins.allow_destructive_actions === "on-request") {
      codexPlugins.allow_destructive_actions = "auto";
      rewrittenDestructivePolicies += 1;
    }
    if (isObject(codexPlugins.plugins)) {
      for (const plugin of Object.values(codexPlugins.plugins)) {
        if (
          isObject(plugin) &&
          plugin.allow_destructive_actions === "on-request"
        ) {
          plugin.allow_destructive_actions = "auto";
          rewrittenDestructivePolicies += 1;
        }
      }
    }
    if (rewrittenDestructivePolicies > 0) {
      changes.push(
        `Replaced ${rewrittenDestructivePolicies} retired Codex plugin destructive action ${rewrittenDestructivePolicies === 1 ? "policy" : "policies"} with "auto".`,
      );
    }
  }

  if (pluginConfig.appServer?.approvalPolicy === "on-failure") {
    pluginConfig.appServer.approvalPolicy = "on-request";
    changes.push(
      'Replaced retired Codex app-server approvalPolicy "on-failure" with "on-request".',
    );
  }

  return { changed: changes.length > 0, changes };
};

const normalizeCodexPluginCompatibility = {
  id: "2026-07-normalize-codex-plugin-compatibility",
  title: "Normalize Codex plugin config for OpenClaw 2026.7.1",
  scope: "config",
  target: "openclaw.json",
  description:
    "OpenClaw 2026.7.1 (and later) retired Codex plugin settings that were valid in older " +
    "releases. Clawbridge must normalize them before target plugin reconciliation " +
    "because the new Codex doctor contract is not available while the old plugin is installed.",
  check({ fsModule = fs, openclawDir }) {
    const configPath = path.join(openclawDir, "openclaw.json");
    if (!fsModule.existsSync(configPath)) {
      return {
        status: "ok",
        message: "openclaw.json is missing; nothing to migrate",
      };
    }
    const config = JSON.parse(fsModule.readFileSync(configPath, "utf8"));
    const paths = findLegacyCodexPluginConfigPaths(config);
    if (paths.length === 0) {
      return {
        status: "ok",
        message: "Codex plugin config is compatible with OpenClaw 2026.7.1 and later",
      };
    }
    return {
      status: "pending",
      message: "Normalize retired Codex plugin settings in openclaw.json",
      details: { paths },
    };
  },
  apply({ fsModule = fs, openclawDir }) {
    const configPath = path.join(openclawDir, "openclaw.json");
    if (!fsModule.existsSync(configPath)) {
      return {
        changed: false,
        changes: ["Skipped openclaw.json because it is missing."],
      };
    }
    const config = JSON.parse(fsModule.readFileSync(configPath, "utf8"));
    const normalized = normalizeCodexPluginCompatibilityConfig(config);
    if (!normalized.changed) {
      return {
        changed: false,
        changes: ["Codex plugin config was already compatible."],
      };
    }
    writeJsonFile({ fsModule, filePath: configPath, value: config });
    return normalized;
  },
};

const hasCodexRuntime = (value) =>
  normalizeString(value?.agentRuntime?.id) === "codex";

const modelKeyUsesOpenAiProvider = (modelKey) => {
  const provider = normalizeString(modelKey).split("/")[0];
  return provider === "openai" || provider === "openai-codex";
};

const configUsesOpenAiCodexRuntime = (config) => {
  if (hasCodexRuntime(config.models?.providers?.openai)) return true;
  if (hasCodexRuntime(config.agents?.defaults)) return true;

  const defaultModels = config.agents?.defaults?.models;
  if (!isObject(defaultModels)) return false;
  return Object.entries(defaultModels).some(
    ([modelKey, modelConfig]) =>
      modelKeyUsesOpenAiProvider(modelKey) && hasCodexRuntime(modelConfig),
  );
};

const getConfiguredOpenAiProfileIds = (config) =>
  Object.entries(config.auth?.profiles || {})
    .filter(([, profile]) => normalizeString(profile?.provider) === "openai")
    .map(([profileId]) => profileId);

const buildCodexPreferredOpenAiAuthOrder = (config) => {
  const existingOrder = Array.isArray(config.auth?.order?.openai)
    ? config.auth.order.openai
    : [];
  const configuredProfiles = getConfiguredOpenAiProfileIds(config);
  const seen = new Set(["openai:codex-cli"]);
  const ordered = ["openai:codex-cli"];

  for (const profileId of [...existingOrder, ...configuredProfiles]) {
    const normalized = normalizeString(profileId);
    if (!normalized || seen.has(normalized)) continue;
    seen.add(normalized);
    ordered.push(normalized);
  }
  return ordered;
};

const preferCodexCliOpenAiAuthProfile = {
  id: "2026-06-prefer-codex-cli-openai-auth-profile",
  title: "Prefer Codex CLI OAuth for OpenAI Codex runtime",
  scope: "config",
  target: "openclaw.json",
  description:
    "When OpenAI models run through the Codex runtime, OpenClaw resolves a single " +
    "OpenAI auth profile for the Codex bridge. Clawbridge should prefer the " +
    "Codex CLI OAuth profile when it exists, while preserving other OpenAI profiles.",
  check({ fsModule = fs, openclawDir }) {
    const configPath = path.join(openclawDir, "openclaw.json");
    if (!fsModule.existsSync(configPath)) {
      return {
        status: "ok",
        message: "openclaw.json is missing; nothing to migrate",
      };
    }
    const config = JSON.parse(fsModule.readFileSync(configPath, "utf8"));
    if (!configUsesOpenAiCodexRuntime(config)) {
      return {
        status: "ok",
        message: "OpenAI Codex runtime is not configured",
      };
    }
    const codexProfile = config.auth?.profiles?.["openai:codex-cli"];
    if (
      !isObject(codexProfile) ||
      normalizeString(codexProfile.provider) !== "openai" ||
      normalizeString(codexProfile.mode) !== "oauth"
    ) {
      return {
        status: "ok",
        message: "openai:codex-cli OAuth profile is not configured",
      };
    }
    if (config.auth?.order?.openai?.[0] === "openai:codex-cli") {
      return {
        status: "ok",
        message: "openai:codex-cli is already the preferred OpenAI auth profile",
      };
    }
    return {
      status: "pending",
      message:
        "Prefer openai:codex-cli first in auth.order.openai for Codex runtime models",
      details: {
        path: "auth.order.openai",
        preferredProfileId: "openai:codex-cli",
      },
    };
  },
  apply({ fsModule = fs, openclawDir }) {
    const configPath = path.join(openclawDir, "openclaw.json");
    if (!fsModule.existsSync(configPath)) {
      return {
        changed: false,
        changes: ["Skipped openclaw.json because it is missing."],
      };
    }
    const config = JSON.parse(fsModule.readFileSync(configPath, "utf8"));
    if (!configUsesOpenAiCodexRuntime(config)) {
      return {
        changed: false,
        changes: ["OpenAI Codex runtime was not configured."],
      };
    }
    const codexProfile = config.auth?.profiles?.["openai:codex-cli"];
    if (
      !isObject(codexProfile) ||
      normalizeString(codexProfile.provider) !== "openai" ||
      normalizeString(codexProfile.mode) !== "oauth"
    ) {
      return {
        changed: false,
        changes: ["openai:codex-cli OAuth profile was not configured."],
      };
    }

    const nextOrder = buildCodexPreferredOpenAiAuthOrder(config);
    const previousOrder = Array.isArray(config.auth?.order?.openai)
      ? config.auth.order.openai
      : [];
    const alreadySatisfied =
      previousOrder.length === nextOrder.length &&
      previousOrder.every((profileId, index) => profileId === nextOrder[index]);
    if (alreadySatisfied) {
      return {
        changed: false,
        changes: ["openai:codex-cli was already first in auth.order.openai."],
      };
    }

    if (!isObject(config.auth)) config.auth = {};
    if (!isObject(config.auth.order)) config.auth.order = {};
    config.auth.order.openai = nextOrder;
    writeJsonFile({ fsModule, filePath: configPath, value: config });
    return {
      changed: true,
      changes: [
        "Updated auth.order.openai to prefer openai:codex-cli for Codex runtime models.",
      ],
    };
  },
};

const ensurePluginApprovalForwarding = {
  id: "2026-07-enable-plugin-approval-forwarding",
  title: "Enable plugin approval forwarding defaults",
  scope: "config",
  target: "openclaw.json",
  description:
    "Skill Workshop apply/reject/quarantine actions use OpenClaw's plugin " +
    "approval channel. Existing Clawbridge installs need approvals.plugin " +
    "defaults so those approval prompts can be routed without switching Skill " +
    "Workshop to auto-approval.",
  check({ fsModule = fs, openclawDir }) {
    const configPath = path.join(openclawDir, "openclaw.json");
    if (!fsModule.existsSync(configPath)) {
      return {
        status: "ok",
        message: "openclaw.json is missing; nothing to migrate",
      };
    }
    const config = JSON.parse(fsModule.readFileSync(configPath, "utf8"));
    const ensured = ensureManagedPluginApprovalDefaults(
      JSON.parse(JSON.stringify(config)),
    );
    if (!ensured.changed) {
      return {
        status: "ok",
        message: "Plugin approval forwarding defaults are already configured",
      };
    }
    return {
      status: "pending",
      message:
        "Add approvals.plugin defaults to openclaw.json for Skill Workshop approval routing",
      details: {
        path: "approvals.plugin",
        defaults: {
          enabled: true,
          mode: "session",
        },
      },
    };
  },
  apply({ fsModule = fs, openclawDir }) {
    const configPath = path.join(openclawDir, "openclaw.json");
    if (!fsModule.existsSync(configPath)) {
      return {
        changed: false,
        changes: ["Skipped openclaw.json because it is missing."],
      };
    }
    const config = JSON.parse(fsModule.readFileSync(configPath, "utf8"));
    const ensured = ensureManagedPluginApprovalDefaults(config);
    if (!ensured.changed) {
      return {
        changed: false,
        changes: ["approvals.plugin defaults were already configured."],
      };
    }
    writeJsonFile({ fsModule, filePath: configPath, value: ensured.config });
    return {
      changed: true,
      changes: [
        "Added approvals.plugin.enabled=true and approvals.plugin.mode=\"session\" defaults to openclaw.json.",
      ],
    };
  },
};

const backfillChannelPluginApprovers = {
  id: "2026-07-backfill-channel-plugin-approvers",
  title: "Backfill Discord and Telegram plugin approvers",
  scope: "config",
  target: "openclaw.json",
  description:
    "Discord and Telegram plugin approval decisions use execApprovals.approvers. " +
    "Existing Clawbridge installs may already trust users for chat access without " +
    "having those users mirrored into the plugin approval approver list.",
  check({ fsModule = fs, openclawDir }) {
    const configPath = path.join(openclawDir, "openclaw.json");
    if (!fsModule.existsSync(configPath)) {
      return {
        status: "ok",
        message: "openclaw.json is missing; nothing to migrate",
      };
    }
    const config = JSON.parse(fsModule.readFileSync(configPath, "utf8"));
    const ensured = ensureManagedChannelApprovalDefaults(
      JSON.parse(JSON.stringify(config)),
    );
    if (!ensured.changed) {
      return {
        status: "ok",
        message: "Discord and Telegram plugin approvers are already configured",
      };
    }
    return {
      status: "pending",
      message:
        "Backfill Discord and Telegram execApprovals.approvers from existing trusted users",
      details: {
        paths: [
          "channels.discord.execApprovals.approvers",
          "channels.discord.accounts.*.execApprovals.approvers",
          "channels.telegram.execApprovals.approvers",
          "channels.telegram.accounts.*.execApprovals.approvers",
        ],
      },
    };
  },
  apply({ fsModule = fs, openclawDir }) {
    const configPath = path.join(openclawDir, "openclaw.json");
    if (!fsModule.existsSync(configPath)) {
      return {
        changed: false,
        changes: ["Skipped openclaw.json because it is missing."],
      };
    }
    const config = JSON.parse(fsModule.readFileSync(configPath, "utf8"));
    const ensured = ensureManagedChannelApprovalDefaults(config);
    if (!ensured.changed) {
      return {
        changed: false,
        changes: ["Discord and Telegram plugin approvers were already configured."],
      };
    }
    writeJsonFile({ fsModule, filePath: configPath, value: ensured.config });
    return {
      changed: true,
      changes: [
        "Backfilled Discord and Telegram execApprovals.approvers from existing trusted users.",
      ],
    };
  },
};

const enforceManagedGatewayOwnershipDefaults = {
  id: "2026-09-enforce-managed-gateway-ownership-defaults",
  title: "Enforce managed Gateway restart and update defaults",
  scope: "config",
  target: "openclaw.json",
  description:
    "Clawbridge owns Gateway restarts and OpenClaw upgrades. Existing managed " +
    "instances must disable agent-requested restarts and startup update checks.",
  check({ fsModule = fs, rootDir, openclawDir }) {
    if (!fsModule.existsSync(path.join(rootDir, "onboarded.json"))) {
      return {
        status: "ok",
        message: "Managed onboarding marker is missing; ownership defaults are not applicable",
      };
    }
    const configPath = path.join(openclawDir, "openclaw.json");
    if (!fsModule.existsSync(configPath)) {
      return {
        status: "ok",
        message: "openclaw.json is missing; nothing to migrate",
      };
    }
    const config = JSON.parse(fsModule.readFileSync(configPath, "utf8"));
    const pendingPaths = [];
    if (config.commands?.restart !== false) pendingPaths.push("commands.restart");
    if (config.update?.checkOnStart !== false) pendingPaths.push("update.checkOnStart");
    if (pendingPaths.length === 0) {
      return {
        status: "ok",
        message: "Managed Gateway ownership defaults are already configured",
      };
    }
    return {
      status: "pending",
      message: "Disable agent restarts and OpenClaw startup update checks",
      details: { paths: pendingPaths },
    };
  },
  apply({ fsModule = fs, rootDir, openclawDir }) {
    if (!fsModule.existsSync(path.join(rootDir, "onboarded.json"))) {
      return {
        changed: false,
        changes: ["Skipped ownership defaults because the managed onboarding marker is missing."],
      };
    }
    const configPath = path.join(openclawDir, "openclaw.json");
    if (!fsModule.existsSync(configPath)) {
      return {
        changed: false,
        changes: ["Skipped openclaw.json because it is missing."],
      };
    }
    const config = JSON.parse(fsModule.readFileSync(configPath, "utf8"));
    if (!isObject(config.commands)) config.commands = {};
    if (!isObject(config.update)) config.update = {};
    const changes = [];
    if (config.commands.restart !== false) {
      config.commands.restart = false;
      changes.push("Set commands.restart=false.");
    }
    if (config.update.checkOnStart !== false) {
      config.update.checkOnStart = false;
      changes.push("Set update.checkOnStart=false.");
    }
    if (changes.length === 0) {
      return {
        changed: false,
        changes: ["Managed Gateway ownership defaults were already configured."],
      };
    }
    writeJsonFile({ fsModule, filePath: configPath, value: config });
    return { changed: true, changes };
  },
};

const applyManagedRuntimeDefaults = {
  id: "2026-09-apply-managed-runtime-defaults",
  title: "Apply managed OpenClaw runtime defaults",
  scope: "config",
  target: "openclaw.json",
  description:
    "Apply the approved managed runtime posture and CPX concurrency default " +
    "without replacing unrelated OpenClaw settings.",
  check({ fsModule = fs, rootDir, openclawDir }) {
    if (!fsModule.existsSync(path.join(rootDir, "onboarded.json"))) {
      return {
        status: "ok",
        message: "Managed onboarding marker is missing; runtime defaults are not applicable",
      };
    }
    const alreadyDelivered = readMigrationLedger({ fsModule, rootDir }).some(
      (entry) =>
        entry?.id === applyManagedRuntimeDefaults.id &&
        entry.status === "completed",
    );
    if (alreadyDelivered) {
      return {
        status: "ok",
        message: "Managed OpenClaw runtime defaults were already delivered",
      };
    }
    const configPath = path.join(openclawDir, "openclaw.json");
    if (!fsModule.existsSync(configPath)) {
      return {
        status: "ok",
        message: "openclaw.json is missing; nothing to migrate",
      };
    }
    const config = JSON.parse(fsModule.readFileSync(configPath, "utf8"));
    const preview = JSON.parse(JSON.stringify(config));
    const ensured = ensureManagedOpenclawDefaults(preview);
    if (!ensured.changed) {
      return {
        status: "pending",
        message: "Record the managed OpenClaw runtime defaults as delivered",
      };
    }
    return {
      status: "pending",
      message: "Apply the approved managed OpenClaw runtime defaults",
    };
  },
  apply({ fsModule = fs, rootDir, openclawDir }) {
    if (!fsModule.existsSync(path.join(rootDir, "onboarded.json"))) {
      return {
        changed: false,
        changes: ["Skipped runtime defaults because the managed onboarding marker is missing."],
      };
    }
    const configPath = path.join(openclawDir, "openclaw.json");
    if (!fsModule.existsSync(configPath)) {
      return {
        changed: false,
        changes: ["Skipped openclaw.json because it is missing."],
      };
    }
    const config = JSON.parse(fsModule.readFileSync(configPath, "utf8"));
    const ensured = ensureManagedOpenclawDefaults(config);
    if (!ensured.changed) {
      return {
        changed: false,
        changes: ["Managed OpenClaw runtime defaults were already configured."],
      };
    }
    writeJsonFile({ fsModule, filePath: configPath, value: ensured.config });
    return {
      changed: true,
      changes: [
        "Applied CPX maxConcurrent=3 and the approved managed feature, telemetry, terminal, and secret-tool defaults.",
      ],
    };
  },
};

const kAlphaclawMigrations = [
  repairConflictingOpenclawPluginIndex,
  removeActiveMemoryModelFallbackPolicy,
  normalizeCodexPluginCompatibility,
  preferCodexCliOpenAiAuthProfile,
  ensurePluginApprovalForwarding,
  backfillChannelPluginApprovers,
  enforceManagedGatewayOwnershipDefaults,
  applyManagedRuntimeDefaults,
];

const runAlphaclawMigrations = ({
  fsModule = fs,
  rootDir,
  openclawDir = path.join(rootDir, ".openclaw"),
  fix = false,
  forceRetry = "",
  migrations = kAlphaclawMigrations,
  now = new Date(),
} = {}) => {
  if (!rootDir) throw new Error("rootDir is required");
  const ledger = readMigrationLedger({ fsModule, rootDir });
  const forceRetryId = String(forceRetry || "").trim();
  const results = [];

  for (const migration of migrations) {
    const base = {
      id: migration.id,
      title: migration.title,
      scope: migration.scope,
      target: migration.target,
      description: migration.description,
    };
    let checkResult;
    try {
      checkResult = migration.check({ fsModule, rootDir, openclawDir });
    } catch (error) {
      results.push({
        ...base,
        status: "failed",
        error: error.message || String(error),
      });
      continue;
    }

    if (checkResult.status !== "pending") {
      results.push({
        ...base,
        status: "ok",
        message: checkResult.message || "No migration needed",
      });
      continue;
    }

    const recentFailureCount = getRecentFailureCount({
      ledger,
      migrationId: migration.id,
    });
    const forceThisMigration =
      forceRetryId === migration.id ||
      forceRetryId === "all" ||
      forceRetryId === "*";
    if (
      recentFailureCount >= kMigrationFailureBlockThreshold &&
      !forceThisMigration
    ) {
      results.push({
        ...base,
        status: "blocked",
        message:
          `Migration has failed ${recentFailureCount} consecutive times; rerun with ` +
          `--force-retry ${migration.id} after inspecting the failure.`,
        details: checkResult.details || null,
      });
      continue;
    }

    if (!fix) {
      results.push({
        ...base,
        status: "pending",
        message: checkResult.message || "Migration is pending",
        details: checkResult.details || null,
      });
      continue;
    }

    try {
      const applyResult = migration.apply({
        fsModule,
        rootDir,
        openclawDir,
        now,
      });
      const entry = appendMigrationLedgerEntry({
        fsModule,
        rootDir,
        now,
        entry: {
          id: migration.id,
          status: "completed",
          scope: migration.scope,
          target: migration.target,
          changed: applyResult.changed === true,
          changes: applyResult.changes || [],
        },
      });
      ledger.push(entry);
      results.push({
        ...base,
        status: applyResult.changed ? "fixed" : "ok",
        message: applyResult.changed ? "Migration applied" : "Migration already satisfied",
        changes: applyResult.changes || [],
      });
    } catch (error) {
      const entry = appendMigrationLedgerEntry({
        fsModule,
        rootDir,
        now,
        entry: {
          id: migration.id,
          status: "failed",
          scope: migration.scope,
          target: migration.target,
          error: error.message || String(error),
        },
      });
      ledger.push(entry);
      results.push({
        ...base,
        status: "failed",
        error: error.message || String(error),
      });
    }
  }

  const summary = results.reduce(
    (acc, result) => {
      acc.total += 1;
      acc[result.status] = (acc[result.status] || 0) + 1;
      return acc;
    },
    { total: 0 },
  );
  return {
    ok: !results.some(
      (result) => result.status === "failed" || result.status === "blocked",
    ),
    fix: fix === true,
    rootDir,
    openclawDir,
    ledgerPath: resolveMigrationLedgerPath({ rootDir }),
    summary,
    results,
  };
};

module.exports = {
  kAlphaclawMigrations,
  kMigrationFailureBlockThreshold,
  appendMigrationLedgerEntry,
  readMigrationLedger,
  resolveMigrationLedgerPath,
  runAlphaclawMigrations,
};
