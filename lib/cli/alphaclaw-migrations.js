...
const archiveForeignHarnessCodexSidecars = {
  id: "2026-07-archive-foreign-harness-codex-sidecars",
  title: "Archive stale Codex sidecars owned by another harness",
  scope: "state",
  target: "agents/*/sessions/*.codex-app-server.json",
  description:
    "OpenClaw 2026.7.1 retains a Codex binding sidecar when its unique session " +
    "owner is another harness, then treats that retention warning as a fatal " +
    "gateway startup blocker. Archive only sidecars with one explicit, non-Codex owner.",
  check({ fsModule = fs, openclawDir }) {
    const inspection = inspectForeignHarnessCodexSidecars({
      fsModule,
      openclawDir,
    });
    if (!inspection.pending) {
      return {
        status: "ok",
        message: "No uniquely foreign-owned Codex sidecars require repair",
      };
    }
    return {
      status: "pending",
      message: "Archive stale Codex binding sidecars owned by another harness",
      details: {
        sidecars: inspection.candidates.map((candidate) => ({
          path: candidate.sidecarPath,
          sessionKey: candidate.owner.sessionKey,
          agentHarnessId: candidate.owner.agentHarnessId,
        })),
      },
    };
  },
  apply({ fsModule = fs, openclawDir }) {
    return repairForeignHarnessCodexSidecars({ fsModule, openclawDir });
  },
};
...
const kAlphaclawMigrations = [
  ...
  archiveForeignHarnessCodexSidecars,
