...
const restoreAuthStoreSnapshots = ({ fsModule = fs, sqliteModule, snapshots }) => {
  ...
  for (const snapshot of snapshots) {
    const originals = snapshot.sources.flatMap((source) => source.originals);
    if (originals.length === 0) continue;

    let restoredToSqlite = false;
    const sqliteStore = readSqliteStore({
      sqliteModule,
      databasePath: snapshot.sqlitePath,
    });
    if (sqliteStore) {
      const result = restoreOAuthCredentialMaterial({ store: sqliteStore, originals });
      if (result.changed) {
        writeSqliteStore({
          sqliteModule,
          databasePath: snapshot.sqlitePath,
      ...
      restoredToSqlite = true;
    }

    if (!restoredToSqlite && fsModule.existsSync(snapshot.jsonPath)) {
      const jsonStore = readJsonFile({ fsModule, filePath: snapshot.jsonPath });
      const result = restoreOAuthCredentialMaterial({ store: jsonStore, originals });
      if (result.changed) {
