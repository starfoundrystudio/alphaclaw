...
const validateOnboardingInput = ({
  ...
  const selectedProviderEnvKeys =
    catalogEnvKeys.length > 0
      ? catalogEnvKeys
      : [getEnvVarForApiKeyProvider(selectedProvider)].filter(Boolean);
  const selectedProviderVars = vars.filter((entry) =>
    selectedProviderEnvKeys.includes(String(entry?.key || "")),
  );
  const providerCredentialValidation =
    validateEnvCredentialValues(selectedProviderVars);
  if (!providerCredentialValidation.ok) return providerCredentialValidation;
  ...
  const hasAi = (() => {
    ...
    if (selectedProviderEnvKeys.length > 0) {
      return selectedProviderEnvKeys.some((envKey) => !!String(varMap[envKey] || "").trim());
    }
    return hasAnyAi;
  })();
  if (!hasAi) {
