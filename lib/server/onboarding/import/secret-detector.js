...
const kManagedSystemEnvKeys = new Set([
...
const generateManagedToken = () => crypto.randomBytes(24).toString("base64url");

const collectManagedSystemEnvVars = ({ fs, baseDir, configFiles = [], envFiles = [] }) => {
  const managedVars = new Map();
  let gatewayAuthPresent = false;
  let webhookConfigPresent = false;
  ...
  for (const cfgFile of configFiles) {
    try {
      ...
      if (cfg.gateway?.auth && typeof cfg.gateway.auth === "object") {
        gatewayAuthPresent = true;
        const gatewayToken = String(cfg.gateway.auth.token || "").trim();
        if (gatewayToken && !isAlreadyEnvRef(gatewayToken)) {
          managedVars.set("OPENCLAW_GATEWAY_TOKEN", gatewayToken);
        }
      }

      if (cfg.hooks && typeof cfg.hooks === "object") {
        webhookConfigPresent = true;
        const webhookToken = String(cfg.hooks.token || "").trim();
        if (webhookToken && !isAlreadyEnvRef(webhookToken)) {
          managedVars.set("WEBHOOK_TOKEN", webhookToken);
        }
      }
    } catch {}
  }
  ...
  for (const envFile of envFiles) {
    try {
      ...
      for (const line of String(content || "").split("\n")) {
        ...
        const key = trimmed.slice(0, eqIdx).trim();
        const value = trimmed.slice(eqIdx + 1).trim();
        if (!kManagedSystemEnvKeys.has(key) || !value) continue;
        managedVars.set(key, value);
      }
    } catch {}
  }

  if (gatewayAuthPresent && !managedVars.has("OPENCLAW_GATEWAY_TOKEN")) {
    managedVars.set("OPENCLAW_GATEWAY_TOKEN", generateManagedToken());
  }
  if (webhookConfigPresent && !managedVars.has("WEBHOOK_TOKEN")) {
    managedVars.set("WEBHOOK_TOKEN", generateManagedToken());
  }

  return [...managedVars.entries()].map(([key, value]) => ({ key, value }));
};
