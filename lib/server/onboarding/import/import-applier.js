...
const copyDirRecursive = (fs, src, dest) => {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      copySymlink(fs, srcPath, destPath);
    } else {
...
const copySymlink = (fs, src, dest) => {
  let linkTarget = "";
  try {
    linkTarget = fs.readlinkSync(src);
  } catch (error) {
    console.warn(
      `[import] Skipping unreadable symlink ${src}: ${error.message}`,
    );
    return;
  }

  if (!fs.existsSync(src)) {
    console.warn(`[import] Skipping dangling symlink ${src} -> ${linkTarget}`);
    return;
  }
  ...
  try {
    ...
    console.warn(`[import] Skipping symlink ${src}: ${error.message}`);
  }
};
