...
const resolveSetupUiUrl = (baseUrl) => {
  ...
  return normalizedBaseUrl || "http://localhost:3000";
};
...
const renderGoogleAccountsMarkdown = (fs) => {
  try {
    ...
    section +=
      "Configured in Clawbridge (use `--client <client> --account <email>` for gog commands):\n\n";
    section += accounts
      .map((account) => {
...
// OpenClaw 2026.9's Birth Sequence no longer contains the former optional
// Connect section. Inject the managed channel handoff independently of any
// upstream heading so template copy can evolve without silently dropping the
// required Clawbridge step. The marker makes this idempotent. An old seeded
// Connect section is removed when present so an in-progress 7.x ritual also
// converges on the current managed contract.
const kUpstreamConnectHeading = "## Connect (Optional)";
const kBootstrapCompletionMarker = "When the four beats are complete";
const kClawbridgeConnectMarker = "<!-- alphaclaw:managed-channel-setup -->";
const kClawbridgeConnectSection = `${kClawbridgeConnectMarker}
## Managed Channel Setup

Before you finish: ask how they want to keep chatting — Slack, Telegram, or Discord — and point them to the channel setup in the Clawbridge dashboard (the Channels card on the General screen). Channel tokens are entered there, never in this chat. WhatsApp is not available on this installation; don't offer it. Don't skip this step.
`;

const patchSeededBootstrapConnectStep = ({ fs, workspaceDir }) => {
  try {
    const bootstrapPath = path.join(workspaceDir, "BOOTSTRAP.md");
    if (!fs.existsSync(bootstrapPath)) return false;
    let content = fs.readFileSync(bootstrapPath, "utf8");
    if (content.includes(kClawbridgeConnectMarker)) return false;

    const legacyHeadingIndex = content.indexOf(kUpstreamConnectHeading);
    if (legacyHeadingIndex !== -1) {
      const afterHeading = legacyHeadingIndex + kUpstreamConnectHeading.length;
      const nextHeadingIndex = content.indexOf("\n## ", afterHeading);
      const sectionEnd = nextHeadingIndex === -1 ? content.length : nextHeadingIndex;
      content = content.slice(0, legacyHeadingIndex) + content.slice(sectionEnd);
    }

    const completionIndex = content.indexOf(kBootstrapCompletionMarker);
    const insertionIndex = completionIndex === -1
      ? content.length
      : content.lastIndexOf("\n", completionIndex);
...
const syncBootstrapPromptFiles = ({ fs, workspaceDir, baseUrl }) => {
  try {
    ...
    const toolsTemplate = renderManagedCapabilityContractTokens(
      fs.readFileSync(
    ...
    const agentsContent = renderManagedCapabilityContractTokens(
      fs.readFileSync(
    ...
    const writeToWorkspace = (targetDir, toolsContent) => {
      ...
      fs.writeFileSync(path.join(bootstrapDir, "AGENTS.md"), agentsContent);
      fs.writeFileSync(path.join(bootstrapDir, "TOOLS.md"), toolsContent);
    };

    writeToWorkspace(workspaceDir, buildToolsContent());
    patchSeededBootstrapConnectStep({ fs, workspaceDir });
    ...
    for (const entry of otherWorkspaces) {
      try {
        ...
        patchSeededBootstrapConnectStep({ fs, workspaceDir: entry.workspace });
      } catch (e) {
        console.error(
