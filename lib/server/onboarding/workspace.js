...
// OpenClaw seeds BOOTSTRAP.md with a "## Connect (Optional)" step whose copy
// (optional, WhatsApp-first, guide-through-chat) doesn't fit managed
// instances — agents reliably skip it, so users finish the ritual with no
// way to keep chatting off-dashboard. The injected AGENTS.md gate says the
// step is required, but mid-ritual the agent follows BOOTSTRAP.md's own
// checklist, so the fix has to land in that document. Rewrites the section
// in place; idempotent (the marker heading is gone after the patch) and a
// no-op when BOOTSTRAP.md is absent (ritual finished) or the heading
// changed upstream.
const kUpstreamConnectHeading = "## Connect (Optional)";
const kClawbridgeConnectSection = `## Connect (Required)
...
const patchSeededBootstrapConnectStep = ({ fs, workspaceDir }) => {
  try {
    ...
    const headingIndex = content.indexOf(kUpstreamConnectHeading);
    if (headingIndex === -1) return false;
    const afterHeading = headingIndex + kUpstreamConnectHeading.length;
