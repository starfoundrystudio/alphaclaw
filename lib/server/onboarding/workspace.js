...
const kGoogleWorkspaceSections = {
  ...
  composio: [
    "### Google Workspace",
    "",
    "Google Workspace (Gmail, Calendar, Drive, Sheets, Docs, and more) is integrated through the **Composio CLI** (`composio`), which manages OAuth and token refresh — there is no Google Cloud project or `gog` CLI in this deployment. Discover linked accounts with `composio connected-accounts list`, inspect available tools with `composio tools list --toolkit <toolkit>` (e.g. `gmail`, `googlecalendar`, `googledrive`), check a tool's input schema with `composio tools info <TOOL_NAME>`, and run tools with `composio tools execute <TOOL_NAME>`. Do not use the `gog` CLI and do not ask the user for Google Cloud OAuth credentials.",
  ].join("\n"),
  none: [
    "### Google Workspace",
    "",
