...
const { resolveBootstrapCompletion } = require("../teamyou-memory-activation");
const {
  applyManagedSearxngWebSearchFallback,
} = require("../web-search-config");
const {
  ensureManagedOpenclawDefaults,
} = require("../managed-defaults-config");
...
const buildProviderApiKeyArgs = ({ provider, varMap }) => {
  ...
  return ["--auth-choice", authMeta.authChoice];
};
...
const buildOnboardArgs = ({
  ...
  if (shouldSkipAuthForCodexOauth) {
    ...
    onboardArgs.push("--auth-choice", "openai-api-key");
...
const ensureManagedConfigShell = (cfg, { openAiCompatApiEnabled = false } = {}) => {
  ...
  ensureManagedOpenclawDefaults(cfg);
...
// TeamYou artifacts are installed out-of-band after onboarding (clawctl's
// post-onboard reconcile), so this gate is written unconditionally on every
// config write while OpenClaw bootstrap is incomplete. Pre-seeding the
// disabled skill/plugin entries is harmless when the artifacts are not
// installed yet and guarantees they arrive gated when they land.
const disableManagedTeamyouMemoryUntilBootstrap = ({ cfg, fsModule } = {}) => {
  const completion = resolveBootstrapCompletion({ fsModule, cfg });
  if (completion.complete) return;
  const teamyouMemoryEntry = cfg.plugins.entries[kManagedTeamyouMemoryPluginId];
  if (teamyouMemoryEntry && typeof teamyouMemoryEntry === "object") {
  ...
  const teamyouSkillEntry = cfg.skills.entries[kManagedTeamyouSkillId] || {};
  cfg.skills.entries[kManagedTeamyouSkillId] = {
    ...teamyouSkillEntry,
    enabled: false,
  };
};
...
const writeSanitizedOpenclawConfig = ({
  ...
  disableManagedTeamyouMemoryUntilBootstrap({ cfg, fsModule: fs });
  applyManagedCodexNativeWebSearchDefault({ cfg, agentRuntimeId });
  applyManagedSearxngWebSearchFallback({ cfg });
