...
const kDefaultHeartbeatModel = "vercel-ai-gateway/google/gemini-2.5-flash-lite";
...
const buildOnboardArgs = ({
  ...
  if (selectedProvider === "openai-codex" && hasCodexOauth) {
...
const ensureManagedConfigShell = (cfg) => {
  ...
  if (!cfg.agents.defaults.heartbeat) cfg.agents.defaults.heartbeat = {};
  ...
  cfg.agents.defaults.heartbeat.model = kDefaultHeartbeatModel;
  ...
  cfg.plugins.entries["active-memory"] = {
    ...
    config: {
      ...
      modelFallbackPolicy:
        cfg.plugins.entries["active-memory"]?.config?.modelFallbackPolicy ||
        "default-remote",
      queryMode:
        cfg.plugins.entries["active-memory"]?.config?.queryMode ||
        kDefaultActiveMemoryQueryMode,
      promptStyle:
        cfg.plugins.entries["active-memory"]?.config?.promptStyle ||
        kDefaultActiveMemoryPromptStyle,
      timeoutMs:
        cfg.plugins.entries["active-memory"]?.config?.timeoutMs ||
        kDefaultActiveMemoryTimeoutMs,
      maxSummaryChars:
        cfg.plugins.entries["active-memory"]?.config?.maxSummaryChars ||
        kDefaultActiveMemoryMaxSummaryChars,
      persistTranscripts:
        cfg.plugins.entries["active-memory"]?.config?.persistTranscripts ?? false,
      logging: cfg.plugins.entries["active-memory"]?.config?.logging ?? true,
    },
  };
};
...
const applyManagedAgentRuntimeDefault = ({ cfg, agentRuntimeId }) => {
  const normalizedRuntimeId = String(agentRuntimeId || "").trim();
  if (normalizedRuntimeId !== "codex") return;
  if (!cfg.agents) cfg.agents = {};
  if (!cfg.agents.defaults) cfg.agents.defaults = {};
  cfg.agents.defaults.agentRuntime = {
    ...(cfg.agents.defaults.agentRuntime || {}),
    id: "codex",
  };
...
const writeSanitizedOpenclawConfig = ({
  ...
  applyManagedAgentRuntimeDefault({ cfg, agentRuntimeId });
