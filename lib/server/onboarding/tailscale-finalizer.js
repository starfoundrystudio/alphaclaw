...
const isTailscaleCliPermissionError = (error) => {
  ...
  return (
    ...
    (text.includes("must be root") || text.includes("sudo tailscale"))
  );
};

const withNonInteractiveSudo = (cmd) =>
  String(cmd || "").replace(/^tailscale\b/, "sudo -n tailscale");

const runTailscaleCli = async (shellCmd, cmd, opts = {}) => {
  try {
    return await shellCmd(cmd, opts);
  } catch (error) {
    if (!isTailscaleCliPermissionError(error) || !cmd.startsWith("tailscale ")) {
      throw error;
    }
    return shellCmd(withNonInteractiveSudo(cmd), opts);
  }
};
...
const ensureSshPolicy = (policy) => {
  ...
  const desiredSrc = [kCloudOpsEmail];
  const desiredDst = [kTailscaleTag];
  ...
  for (const sshRule of policy.ssh) {
    ...
    if (
      ...
      !sameStringSet(sshRule.src, desiredSrc) ||
...
const callTeamYouWriteback = async ({
  ...
  const callbackUrl = String(
    env.TEAMYOU_FINALIZE_CALLBACK_URL ||
      env.ALPHACLAW_TEAMYOU_FINALIZE_CALLBACK_URL ||
      "",
  ).trim();
  if (!callbackUrl) return { skipped: true };
  const callbackToken = String(
    env.TEAMYOU_FINALIZE_CALLBACK_TOKEN ||
      env.ALPHACLAW_TEAMYOU_FINALIZE_CALLBACK_TOKEN ||
      "",
  ).trim();
  const res = await fetchImpl(callbackUrl, {
    ...
    body: JSON.stringify({
      ...
      tailscaleDeviceId: deviceId,
      tailscaleTailnet: tailnet,
      tailscaleTag: kTailscaleTag,
    }),
  });
  if (!res.ok) {
...
const createTailscaleFinalizer = ({
  ...
  const finalizeTailscaleOnboarding = async ({ tailscaleApiToken }) => {
    ...
    const servePort = Number(env.TAILSCALE_SERVE_PORT || kDefaultServePort) || kDefaultServePort;
    const funnelPort =
      Number(env.TAILSCALE_FUNNEL_PORT || kDefaultFunnelPort) || kDefaultFunnelPort;
    const appPort = Number(env.PORT || PORT || 3000) || 3000;
    const pagesDir = path.join(constants.OPENCLAW_DIR, "pages");
    ...
    await runTailscaleCli(
      shellCmd,
      `tailscale serve --bg --https=${servePort} 127.0.0.1:${appPort}`,
      { timeoutMs: 30000 },
    );
    await runTailscaleCli(
      shellCmd,
      `tailscale serve --bg --https=${servePort} --set-path=/pages/ ${shellQuote(pagesDir)}`,
      { timeoutMs: 30000 },
    );
    await runTailscaleCli(
      shellCmd,
      `tailscale funnel --bg --https=${funnelPort} 127.0.0.1:${appPort}`,
      { timeoutMs: 30000 },
    );
