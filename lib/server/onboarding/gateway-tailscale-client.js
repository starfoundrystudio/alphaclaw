...
const kGatewaySetupSchemaVersion = 2;
...
// Clawbridge removes the private identity after it durably records confirmation.
...
const normalizeGatewayStatus = (response, operation) => {
  ...
  const rawAgentVaultOperatorUrl = requireSingleLine(
    parsed.agent_vault_operator_url,
    "Agent Vault operator URL",
  );
  let parsedAgentVaultUrl;
  try {
    parsedAgentVaultUrl = new URL(rawAgentVaultOperatorUrl);
  } catch {
