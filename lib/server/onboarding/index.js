...
const applySubmittedEnvVars = (items, vars = []) => {
  for (const entry of vars || []) {
    ...
    if (
      ...
      kTransientOnboardingVarKeys.has(key)
    ) {
      continue;
    }
  ...
  const completeOnboarding = async ({
    ...
    if (hadImportedGit || !fs.existsSync(`${OPENCLAW_DIR}/.git`)) {
      const initCommand = hasGithubBackup
        ? `cd ${OPENCLAW_DIR} && git init -b main && git remote add origin "${remoteUrl}" && git config user.email "agent@alphaclaw.md" && git config user.name "Clawbridge Agent"`
        : `cd ${OPENCLAW_DIR} && git init -b main && git config user.email "agent@alphaclaw.md" && git config user.name "Clawbridge Agent"`;
      ...
      try {
        await shellCmd(
          `cd ${OPENCLAW_DIR} && git remote set-url origin "${remoteUrl}" && git config user.email "agent@alphaclaw.md" && git config user.name "Clawbridge Agent"`,
        );
      } catch {}
    }
    ...
    if (hasGithubBackup) {
      try {
        const commitMsg = importMode
          ? "imported existing setup via Clawbridge"
          : "initial setup";
