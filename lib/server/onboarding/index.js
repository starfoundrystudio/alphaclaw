...
const createOnboardingService = ({
  ...
  const completeOnboarding = async ({
    ...
    const finalSetupUrl = String(tailscaleResult?.setupUrl || getBaseUrl(req)).trim();
    if (finalSetupUrl) {
      syncBootstrapPromptFiles({
        fs,
        workspaceDir: WORKSPACE_DIR,
    ...
    startGateway();
    await runHostFinalizeSetup(shellCmd, "complete");
    const setupUrl = String(tailscaleResult?.setupUrl || finalSetupUrl || "").trim();
