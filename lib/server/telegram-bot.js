...
const inspectTelegramBotToken = async (
  ...
  // Agent Vault placeholders skip shape validation; the API call itself is
  // the validity check, with the proxy substituting the real token.
  if (
    !isTelegramBotTokenShape(normalizedToken) &&
    !isVaultPlaceholderValue(normalizedToken)
  ) {
