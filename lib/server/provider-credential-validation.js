...
const validateProviderCredentialValue = ({ provider = "", envKey = "", value = "" } = {}) => {
  ...
  // Agent Vault placeholders are non-secret stand-ins; the proxy injects the
  // real key, so provider format rules never apply to them.
  if (isVaultPlaceholderValue(normalizedValue)) return { ok: true };
