const os = require("os");
const path = require("path");
const { exec } = require("child_process");
const { OPENCLAW_DIR, GOG_KEYRING_PASSWORD } = require("./constants");
const { redactSecretText } = require("./secret-redaction");

// The Composio installer places the binary at ~/.composio/composio, which is
// not on PATH for a non-interactive server process.
const composioAugmentedPath = () => {
  const composioDir = path.join(os.homedir(), ".composio");
  const currentPath = String(process.env.PATH || "");
  if (currentPath.split(":").includes(composioDir)) return currentPath;
  return `${composioDir}:${currentPath}`;
};

const sanitizeOnboardCommandForLog = (cmd) =>
  redactSecretText(cmd);

const createCommands = ({ gatewayEnv, gatewayMaintenanceEnv = gatewayEnv }) => {
  const shellCmd = (cmd, opts = {}) =>
    new Promise((resolve, reject) => {
      const {
        logStdout,
        timeoutMs = 60000,
        ...execOpts
      } = opts;
      const shouldLogStdout =
        typeof logStdout === "boolean" ? logStdout : !cmd.includes("--json");
      console.log(
        `[onboard] Running: ${sanitizeOnboardCommandForLog(cmd).slice(0, 200)}`,
      );
      exec(cmd, { timeout: timeoutMs, ...execOpts }, (err, stdout, stderr) => {
        if (err) {
          err.stdout = String(stdout || "").trim();
          err.stderr = String(stderr || "").trim();
          err.cmd = cmd;
          console.error(
            `[onboard] Error: ${redactSecretText(stderr || err.message || "").slice(0, 300)}`,
          );
          return reject(err);
        }
        if (shouldLogStdout && stdout.trim()) {
          console.log(`[onboard] ${stdout.trim().slice(0, 300)}`);
        }
        resolve(stdout.trim());
      });
    });

  const clawCmd = (
    cmd,
    {
      quiet = false,
      timeoutMs = 15000,
      killSignal = "SIGTERM",
      allowConfigMutation = false,
    } = {},
  ) =>
    new Promise((resolve) => {
      if (!quiet) console.log(`[alphaclaw] Running: openclaw ${cmd}`);
      exec(
        `openclaw ${cmd}`,
        {
          env: allowConfigMutation ? gatewayMaintenanceEnv() : gatewayEnv(),
          timeout: timeoutMs,
          killSignal,
        },
        (err, stdout, stderr) => {
          const result = {
            ok: !err,
            stdout: stdout.trim(),
            stderr: stderr.trim(),
            code: err?.code,
            signal: err?.signal,
            killed: err?.killed,
            message: err?.message,
          };
          if (err) {
            result.killed = Boolean(err.killed);
            result.signal = err.signal || null;
            result.timedOut = Boolean(err.killed && err.signal === killSignal);
          }
          if (!quiet && !result.ok) {
            console.log(
              result.timedOut
                ? `[alphaclaw] Timed out after ${Math.round(timeoutMs / 1000)}s: openclaw ${cmd.split(" ").slice(0, 2).join(" ")}`
                : `[alphaclaw] Error: ${
                    result.stderr.slice(0, 200) ||
                    `exit ${result.code ?? result.signal ?? "unknown"}`
                  }`,
            );
          }
          resolve(result);
        },
      );
    });

  const gogCmd = (
    cmd,
    { quiet = false, authBypass = false, timeoutMs = 60000 } = {},
  ) =>
    new Promise((resolve) => {
      if (!quiet) console.log(`[alphaclaw] Running: gog ${cmd}`);
      exec(
        `gog ${cmd}`,
        {
          timeout: timeoutMs,
          env: {
            ...process.env,
            XDG_CONFIG_HOME: OPENCLAW_DIR,
            GOG_KEYRING_PASSWORD,
            ...(authBypass ? { ALPHACLAW_GOG_AUTH_BYPASS: "1" } : {}),
          },
        },
        (err, stdout, stderr) => {
          const result = {
            ok: !err,
            stdout: stdout.trim(),
            stderr: stderr.trim(),
          };
          if (!quiet && !result.ok) {
            console.log(`[alphaclaw] gog error: ${result.stderr.slice(0, 200)}`);
          }
          resolve(result);
        },
      );
    });

  const composioCmd = (cmd, { quiet = false, timeoutMs = 30000 } = {}) =>
    new Promise((resolve) => {
      if (!quiet) console.log(`[alphaclaw] Running: composio ${cmd}`);
      exec(
        `composio ${cmd}`,
        {
          timeout: timeoutMs,
          env: {
            ...process.env,
            PATH: composioAugmentedPath(),
          },
        },
        (err, stdout, stderr) => {
          const result = {
            ok: !err,
            stdout: stdout.trim(),
            stderr: stderr.trim(),
          };
          if (!quiet && !result.ok) {
            console.log(
              `[alphaclaw] composio error: ${redactSecretText(result.stderr).slice(0, 200)}`,
            );
          }
          resolve(result);
        },
      );
    });

  return { shellCmd, clawCmd, gogCmd, composioCmd };
};

module.exports = { createCommands, sanitizeOnboardCommandForLog };
