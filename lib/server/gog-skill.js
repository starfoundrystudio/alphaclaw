...
const buildGogSkillContent = ({ fs, accounts }) => {
  ...
  lines.push("");
  lines.push("```");
  lines.push("--account <email>   Account to use");
  ...
  lines.push("");
  lines.push(
    "- On a Clawbridge-managed instance, invoke `gog` from `PATH`; the managed launcher obtains a short-lived access token for the selected connected account.",
  );
  lines.push(
    "- Do not run `gog auth`, pass `--access-token`, or configure gog credential files directly. Connect, reconnect, and disconnect Google accounts from Clawbridge Google settings.",
  );
  lines.push(
  ...
  try {
    ...
    const removeSkillIfPresent = (reason) => {
      ...
      if (fs.existsSync(skillPath)) {
        ...
        console.log(`[gog-skill] Removed gog-cli skill (${reason})`);
      }
    };

    const { provider } = resolveGoogleProvider({ state });
    if (provider !== "gog") {
    ...
    if (!content) {
      removeSkillIfPresent("no connected accounts");
      return;
    }
