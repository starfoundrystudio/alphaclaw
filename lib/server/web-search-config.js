...
const kCompatibilityManifest = require("../openclaw-compatibility.manifest.json");

// Managed plugins that provide a web-search provider other than SearXNG,
// from the pinned compatibility manifest (`webSearchProviderIds` /
// `webSearchProviderEnvVars`).
const listOtherWebSearchProviderPlugins = (manifest = kCompatibilityManifest) =>
  Object.entries(manifest?.managedPlugins || {})
    .filter(([pluginId, definition]) => {
      const providerIds = Array.isArray(definition?.webSearchProviderIds)
        ? definition.webSearchProviderIds
        : [];
      return pluginId !== "searxng" && providerIds.length > 0;
    })
    .map(([pluginId, definition]) => ({
      pluginId,
      envVars: Array.isArray(definition?.webSearchProviderEnvVars)
        ? definition.webSearchProviderEnvVars.map(normalizeString).filter(Boolean)
        : [],
    }));

const hasOtherWebSearchProviderAvailable = ({
  cfg,
  env = process.env,
  manifest = kCompatibilityManifest,
} = {}) =>
  listOtherWebSearchProviderPlugins(manifest).some(({ pluginId, envVars }) => {
    const entry = cfg?.plugins?.entries?.[pluginId];
    if (entry && typeof entry === "object" && entry.enabled === true) return true;
    return envVars.some((key) => normalizeString(env?.[key]));
  });
...
const applyManagedSearxngWebSearchFallback = ({
  ...
  //
  // SearXNG is the fallback, not the preference: if any other web-search
  // provider plugin is enabled or has its credential in the environment,
  // leave the provider unset so OpenClaw's auto-detection (or the user's
  // later choice) picks that one, as discovery mode did on 2026.7.1.
  // Runtime-native search (Codex `openaiCodex`, Claude Code's own tool) is
  // a separate path and is not affected by the managed provider.
  if (!hasOtherWebSearchProviderAvailable({ cfg, env })) {
