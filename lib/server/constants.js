...
const kBootstrapKickoffMarkerPath = path.join(
  ALPHACLAW_DIR,
...
// The auth server completes this redirect itself during device auth; the
// token exchange must still name it as the redirect_uri.
const CODEX_DEVICE_REDIRECT_URI = "https://auth.openai.com/deviceauth/callback";
const kCodexDeviceAuthTtlMs = 15 * 60 * 1000;
...
/** Copying a full OpenClaw npm tree can exceed 60s on slow filesystems. */
const kOpenclawUpdateCopyTimeoutMs = 5 * 60 * 1000;
const kOpenclawRegistryUrl = "https://registry.npmjs.org/openclaw";
const kAlphaclawRegistryUrl = "https://npm.pkg.github.com/@starfoundrystudio%2falphaclaw";
const kAlphaclawGithubReleasesBaseUrl =
  "https://api.github.com/repos/starfoundrystudio/alphaclaw/releases";
...
const kWatchdogStartupGraceMs =
  parsePositiveInt(process.env.WATCHDOG_STARTUP_GRACE_MS, 60) * 1000;
const kWatchdogHealthTimeoutMs =
  parsePositiveInt(process.env.WATCHDOG_HEALTH_TIMEOUT_MS, 10) * 1000;
const kWatchdogEventLoopLagP95ThresholdMs = parsePositiveInt(
  process.env.WATCHDOG_EVENT_LOOP_LAG_P95_THRESHOLD_MS,
  1000,
);
const kWatchdogEventLoopLagMaxThresholdMs = parsePositiveInt(
  process.env.WATCHDOG_EVENT_LOOP_LAG_MAX_THRESHOLD_MS,
  3000,
);
const kWatchdogEventLoopLagResolutionMs = parsePositiveInt(
  process.env.WATCHDOG_EVENT_LOOP_LAG_RESOLUTION_MS,
  20,
);
...
const kWatchdogRepairTimeoutMs = parsePositiveInt(
  process.env.WATCHDOG_REPAIR_TIMEOUT_MS,
  10 * 60,
) * 1000;
...
/**
 * Security-gateway hop probe written by the host timer
 * (`alphaclaw-gateway-hop-probe.timer`, clawctl). Clawbridge only reads it.
 */
const kGatewayHopProbePath =
  String(process.env.ALPHACLAW_GATEWAY_HOP_PROBE_PATH || "").trim() ||
  "/run/alphaclaw/gateway-hop-probe.json";
/** The host probe runs every 2 minutes; three missed runs mean the data is stale. */
const kGatewayHopProbeStaleMs =
  parsePositiveInt(process.env.WATCHDOG_GATEWAY_HOP_STALE_SECONDS, 6 * 60) * 1000;
/** Consecutive probe failures before the watchdog raises a notification. */
const kGatewayHopAlarmFailureThreshold = parsePositiveInt(
  process.env.WATCHDOG_GATEWAY_HOP_ALARM_FAILURES,
  2,
);
...
const kSystemVars = new Set([
  ...
  "ALPHACLAW_TEAMYOU_MEMORY_PLUGIN_ARCHIVE",
  "ALPHACLAW_TEAMYOU_MEMORY_PLUGIN_VERSION",
  "ALPHACLAW_TEAMYOU_MEMORY_PLUGIN_URL",
]);
const kKnownVars = [
  {
  ...
  },
  {
    ...
    group: "ai",
    hint: "From Vercel AI Gateway. Keys start with vck_.",
    features: ["Models"],
  },
  {
  ...
  },
  {
    ...
    label: "Setup UI URL",
    group: "networking",
    hint: "Private Setup UI URL, e.g. https://alphaclaw.tail123.ts.net",
  },
  {
    key: "ALPHACLAW_PUBLIC_BASE_URL",
    label: "Public Callback URL",
    group: "networking",
    hint: "Public webhook/OAuth callback URL, e.g. https://callbacks.example.com",
  },
  {
    key: "ALPHACLAW_PUBLIC_EXTRA_PATH_PREFIXES",
    label: "Extra Public Paths",
    group: "networking",
    hint: "Comma-separated public path prefixes, e.g. /googlechat,/api/messages",
  },
  {
    key: "ALPHACLAW_BASE_URL",
    label: "Legacy Setup URL",
    group: "networking",
    hint: "Legacy fallback for the private Setup UI URL; prefer ALPHACLAW_SETUP_URL",
  },
  {
...
// Target Composio CLI version for composio-provider deployments. Existing
// instances are upgraded to this pin at boot and on dashboard refresh (see
// composio-install.js); bump it when a tested stable CLI release ships.
const kComposioCliTargetVersion =
  process.env.COMPOSIO_CLI_VERSION || "0.2.32";
...
const kAnchoredBrowsePaths = new Set(
  Array.isArray(kBrowseFilePolicies?.anchoredPaths)
    ? kBrowseFilePolicies.anchoredPaths
    : [],
);
const kAnchoredBrowseSubtrees = new Set(
  Array.isArray(kBrowseFilePolicies?.anchoredSubtrees)
    ? kBrowseFilePolicies.anchoredSubtrees
    : [],
);
const kAnchoredBrowseRootNames = new Set(
  Array.isArray(kBrowseFilePolicies?.anchoredRootNames)
