...
const syncConfigForTelegram = ({
  ...
  if (groupPolicy) {
    targetConfig.groupPolicy = groupPolicy;
  } else if (!String(targetConfig.groupPolicy || "").trim()) {
    targetConfig.groupPolicy = "open";
  }
  ...
  const maxConcurrent = capManagedCpxAgentConcurrency(cfg);
  const configuredSubagentMaxConcurrent = Number(
    cfg.agents.defaults.subagents?.maxConcurrent,
  );
  const subagentMaxConcurrent =
    Number.isSafeInteger(configuredSubagentMaxConcurrent) &&
    configuredSubagentMaxConcurrent > 0
      ? configuredSubagentMaxConcurrent
      : null;
