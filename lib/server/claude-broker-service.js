...
const discardClaudeOauthCredential = ({
  fsModule = fs,
  credentialsPath,
} = {}) => {
  if (!fsModule.existsSync(credentialsPath)) return { changed: false };
  let document;
  try {
    document = JSON.parse(fsModule.readFileSync(credentialsPath, "utf8"));
  } catch (error) {
    const wrapped = new Error("Claude CLI login credentials could not be read");
    wrapped.code = "claude_credentials_invalid";
    wrapped.cause = error;
    throw wrapped;
  }
  if (!document?.claudeAiOauth || typeof document.claudeAiOauth !== "object") {
...
const createClaudeBrokerService = ({
  ...
  const validateGrantUnlocked = async () => {
    const profile = authProfiles.getClaudeCliProfile?.() || null;
    if (!profile?.brokered) {
      return { checked: false, reason: "not_brokered" };
    }
    lastValidatedAt = now();
    ...
    try {
      ...
      setFailure(error);
      logger.error(
        `[alphaclaw] Claude OAuth grant validation failed (${lastErrorCode})`,
      );
      return { checked: true, healthy: false, error: lastErrorCode };
    }
  };

  const validateGrant = () => withLifecycleLock(validateGrantUnlocked);
  ...
  const discardPendingLogin = () =>
    withLifecycleLock(async () => {
      if (
        authProfiles.hasClaudeCliProfile?.() ||
        depositStore.isPending()
      ) {
        return { changed: false, preserved: true };
      }
      return discardClaudeOauthCredential({ fsModule, credentialsPath });
    });
  ...
  const status = async () => {
    ...
    if (configured) {
      try {
        ...
        if (
          !grantPresent &&
          authProfiles.getClaudeCliProfile?.()?.brokered
        ) {
          setFailure({ code: "grant_not_found" });
        }
      } catch (error) {
        setFailure(error);
      }
    ...
    return {
      ...
      healthy:
        authProfiles.getClaudeCliProfile?.()?.brokered
          ? lastErrorCode === null && grantPresent
          : null,
      reconnectRequired: kReconnectErrorCodes.has(lastErrorCode),
      lastValidatedAt,
