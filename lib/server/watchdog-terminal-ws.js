...
const createWatchdogTerminalWsBridge = ({
  ...
  getGatewayUrl,
  isAuthorizedRequest,
  isAdvancedControlAuthorized = () => false,
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    const isAdvancedControlSocket =
      requestUrl.pathname === "/openclaw" ||
      requestUrl.pathname.startsWith("/openclaw/");
    const isClawbridgeSocket =
    ...
    if (isAdvancedControlSocket || isClawbridgeSocket) {
    ...
    if (isAdvancedControlSocket && !isAdvancedControlAuthorized(upgradeReq)) {
      socket.write(
        "HTTP/1.1 403 Forbidden\r\nContent-Type: text/plain\r\nConnection: close\r\n\r\nAdvanced OpenClaw access acknowledgement required",
      );
      socket.destroy();
      return;
    }
