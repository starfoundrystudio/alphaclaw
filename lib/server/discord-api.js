...
const createDiscordApi = (getToken, { fetchImpl = fetch } = {}) => {
  const call = async (path, { method = "GET", body } = {}) => {
    const token = typeof getToken === "function" ? getToken() : getToken;
    if (!token) throw new Error("DISCORD_BOT_TOKEN is not set");
    const res = await fetchImpl(`${kDiscordApiBase}${path}`, {
      method,
      headers: {
  ...
  const getCurrentUser = () => call("/users/@me");
  const getCurrentApplication = () => call("/oauth2/applications/@me");
  const listCurrentUserGuilds = () => call("/users/@me/guilds");
