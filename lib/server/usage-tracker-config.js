...
const kAgentVaultPluginPath = path.resolve(
  __dirname,
  "..",
  "plugin",
  "agent-vault",
...
const isAgentVaultManaged = (env = process.env) =>
  String(env.ALPHACLAW_CONNECTIVITY_MODE || "").trim() ===
    "security_gateway" || String(env.AGENT_VAULT_ADDR || "").trim().length > 0;
...
const ensureAgentVaultPluginEntry = (cfg = {}) => {
  const before = JSON.stringify(cfg);
  ensurePluginAllowed({ cfg, pluginKey: "agent-vault" });
  if (!cfg.plugins.load.paths.includes(kAgentVaultPluginPath)) {
    cfg.plugins.load.paths.push(kAgentVaultPluginPath);
  }
  cfg.plugins.entries["agent-vault"] = {
    ...(cfg.plugins.entries["agent-vault"] &&
    typeof cfg.plugins.entries["agent-vault"] === "object"
      ? cfg.plugins.entries["agent-vault"]
      : {}),
    enabled: true,
  };
...
const reconcileManagedPluginConfig = (cfg = {}, { env = process.env } = {}) => {
  let changed = ensureUsageTrackerPluginEntry(cfg);
  if (isAgentVaultManaged(env) && ensureAgentVaultPluginEntry(cfg)) {
    changed = true;
  }
  if (
    applyManagedSearxngWebSearchFallback({
...
const ensureUsageTrackerPluginConfig = ({
  ...
  const changed = reconcileManagedPluginConfig(cfg, { env });
