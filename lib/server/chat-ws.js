const { isStalePluginInstanceError } = require("./gateway-stale-plugin-guard");
const { readOpenclawConfig } = require("./openclaw-config");

const kWsOpen = 1;
const kHistoryLimit = 200;
// Marks Clawbridge-authored user-role system notes (e.g. the bootstrap
// kickoff). Messages carrying it are delivered to the agent but hidden from
// the rendered chat transcript.
const kClawbridgeSystemNotePrefix = "[Clawbridge]";
// Sessions created before the rebrand carry notes under the old prefix; they
// must stay hidden from the transcript too.
const kLegacySystemNotePrefix = "[AlphaClaw]";
// OpenClaw (2026.7.1 through the pinned 2026.9.x) persists this agent-facing
// recovery instruction as a user
// turn. Hide only this known note, not arbitrary [System] user messages.
const kOpenclawRestartRecoveryNote =
...
// Bounds for the per-runId agent event buffer: runs that are never claimed by
// a chat.send response (e.g. active-memory recall passes sharing the session)
// would otherwise accumulate forever.
const kMaxBufferedAgentEventRuns = 16;
const kMaxBufferedAgentEventsPerRun = 500;
...
const normalizeHistoryMessages = (rawMessages = [], history = null) => {
  ...
  const messages = rawMessages
    .flatMap((messageRow) => {
      ...
      if (role === "user") {
        // Clawbridge-authored system notes (e.g. the bootstrap kickoff) are
        // agent-facing only; hide them from the rendered transcript.
        const trimmedContent = content.trimStart();
        if (
          trimmedContent.startsWith(kClawbridgeSystemNotePrefix) ||
          trimmedContent.startsWith(kLegacySystemNotePrefix) ||
          trimmedContent.startsWith(kOpenclawRestartRecoveryNote)
        ) {
          return [];
        }
...
// Errors that mean "the bridge could not reach the gateway right now" —
// transient during boots and restarts. The client shows these in the
// connection banner and retries, instead of rendering them as agent
// messages in the thread.
const kConnectionErrorFragments = [
  "not connected",
  "econnrefused",
  "connect failed",
...
const isConnectionLevelError = (error) => {
  const message = error instanceof Error ? error.message : String(error || "");
  const lower = message.toLowerCase();
  return kConnectionErrorFragments.some((fragment) => lower.includes(fragment));
};
...
const sanitizeError = (error) => {
  ...
  if (isStalePluginInstanceError(message)) {
    return "Your agent is recovering from a plugin reload. Chat will reconnect automatically.";
  }
  if (lower.includes("gateway starting")) {
    return "The gateway is starting. Chat will reconnect automatically.";
  }
  ...
  if (lower.includes("protocol mismatch")) {
    return "Chat cannot connect to the gateway (protocol version mismatch). Update Clawbridge to match your OpenClaw version.";
  }
...
const createChatWsService = ({
  ...
  try {
    ...
    return {
      ...
      requestGateway: async () => {
        throw new Error("OpenClaw gateway websocket unavailable");
      },
      getGatewayConnectionAgeMs: () => 0,
      hasActiveRuns: () => false,
    };
  }
  ...
  /** Agent events keyed by runId when the event references a run not yet in runTargets (race with chat.send response). Runs never claimed by a chat.send response are evicted by the buffer caps. */
  const pendingAgentEventsByRunId = new Map();
  const browserRuns = new WeakMap();

  const bufferAgentEventForRun = (runId, eventPayload) => {
    const list = pendingAgentEventsByRunId.get(runId) || [];
    if (list.length >= kMaxBufferedAgentEventsPerRun) return;
    list.push(eventPayload);
    // Delete-then-set keeps Map insertion order as recency for eviction.
    pendingAgentEventsByRunId.delete(runId);
    pendingAgentEventsByRunId.set(runId, list);
    while (pendingAgentEventsByRunId.size > kMaxBufferedAgentEventRuns) {
      const oldestRunId = pendingAgentEventsByRunId.keys().next().value;
      if (oldestRunId === undefined || oldestRunId === runId) break;
  ...
  // Runs streaming to a browser end with the Gateway connection: their
  // events cannot reach us any more, and OpenClaw 2026.9 resumes an
  // interrupted main-session turn itself after a restart, under a new run.
  // Tell each browser so it leaves streaming mode and reads history until the
  // session is idle; otherwise the chat stays silent until a reload (G3
  // finding #24).
  const notifyBrowsersOfInterruptedRuns = () => {
    const notified = new Set();
    for (const [runId, target] of runTargets.entries()) {
      runTargets.delete(runId);
      const key = `${target?.sessionKey || ""}\u0000${runId}`;
      if (!target?.ws || notified.has(key)) continue;
      notified.add(key);
      browserRuns.get(target.ws)?.delete(runId);
      sendJson(target.ws, {
        type: "interrupted",
  ...
  const handleGatewayEvent = (eventPayload = {}) => {
    ...
    const resolveTargetForPayload = () => {
      ...
      if (runId) {
        // An explicit runId that is not registered is either our own run
        // racing the chat.send response (buffered below and flushed once the
        // response claims it) or a concurrent side run in the same session,
        // e.g. an active-memory recall pass. Never fall back to session or
        // single-run matching for it: that attributed the recall run's
        // "NONE" output to the user's message and tore down the real run's
        // target when the recall run ended.
        return { runId, target: runTargets.get(runId) || null };
      }
    ...
    if (eventName === "agent") {
      ...
      if (!target) {
        if (runId) {
          bufferAgentEventForRun(runId, eventPayload);
        }
        return;
      }
      ...
      if (stream === "assistant") {
        // OpenClaw 2026.9 sends the FULL visible text as `delta` with
        // `replace: true` whenever the streamed text no longer extends what
        // it sent before (item boundaries, truncation, a cleared stream); a
        // text-only event with an empty delta is the same thing. Forwarding
        // those as appends doubled the bubble in Clawbridge (G3 finding
        // #16, 2026-09-21). Tell the browser to replace instead.
        const deltaText = String(data?.delta ?? "");
        const fullText = String(data?.text ?? "");
        const replace = data?.replace === true || (!deltaText && !!fullText);
        const content = replace ? fullText || deltaText : deltaText;
        if (!content) return;
        sendJson(target.ws, {
          type: "chunk",
          messageId: target.messageId,
  ...
  const requestGateway = async (
    ...
    return withTimeout(responsePromise, timeoutMs, `OpenClaw ${method} request`)
      .catch((error) => {
        if (typeof onGatewayRequestError === "function") {
          try {
  ...
  const handleHistory = async ({ ws, payload }) => {
    ...
    sendJson(ws, {
      type: "history",
      sessionKey,
      historyRequestId: Number.isSafeInteger(payload?.historyRequestId)
        ? payload.historyRequestId : null,
      messages,
      rawHistory,
    });
  };
    ...
    ws.on("message", (rawData) => {
      ...
      run().catch((err) => {
        ...
        sendJson(ws, {
          ...
          // A failed history read is never a run failure: show it in the
          // connection banner (which retries) instead of appending an
          // assistant message per poll (G3 finding #42).
          scope:
            type === "history" ||
            isConnectionLevelError(err) ||
            isStalePluginInstanceError(err)
              ? "connection"
              : "run",
          requestType: type,
    ...
    const rawMessages = Array.isArray(history?.messages)
      ...
      : Array.isArray(history?.history)
        ...
        : Array.isArray(history?.items)
          ? history.items
          : [];
    return normalizeHistoryMessages(rawMessages, history);
  };
  ...
  return {
    ...
    getGatewayConnectionAgeMs: () =>
      gatewaySocket && gatewaySocket.readyState === kWsOpen && gatewayConnectedAtMs
        ? Date.now() - gatewayConnectedAtMs
        : 0,
    // True while a browser chat run is streaming through this bridge.
    hasActiveRuns: () =>
      runTargets.size > 0 || pendingSessionBySessionKey.size > 0,
  };
};
