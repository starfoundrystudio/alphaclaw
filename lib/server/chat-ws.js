...
// Marks AlphaClaw-authored user-role system notes (e.g. the bootstrap
// kickoff). Messages carrying it are delivered to the agent but hidden from
// the rendered chat transcript.
const kAlphaclawSystemNotePrefix = "[AlphaClaw]";
...
const normalizeHistoryMessages = (rawMessages = [], history = null) => {
  ...
  const messages = rawMessages
    .flatMap((messageRow) => {
      ...
      if (role === "user") {
        // AlphaClaw-authored system notes (e.g. the bootstrap kickoff) are
        // agent-facing only; hide them from the rendered transcript.
        if (content.trimStart().startsWith(kAlphaclawSystemNotePrefix)) {
          return [];
        }
