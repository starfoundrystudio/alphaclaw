...
const createSandboxRoutes = ({
  ...
  let advancedControlAcknowledged = false;
  ...
  app.get("/api/onboard/runtime-ready.svg", (_req, res) => {
    if (!state.handoffReadyAt || Date.now() < state.handoffReadyAt) {
  ...
  app.get("/api/onboard/status", (_req, res) => {
    res.json({
      ...
      // The "empty" scenario models a freshly onboarded install whose agent
      // has not run its bootstrap ritual yet (chat is the landing screen).
      ...(state.onboarded
        ? {
            workspaceBootstrap:
              state.scenario === "empty"
                ? { complete: false, reason: "bootstrap_pending" }
                : { complete: true, reason: "setup_completed_marker" },
          }
        : {}),
  ...
  app.post("/api/onboard", (_req, res) => {
    ...
    state.handoffReadyAt = Date.now() + 1000;
    if (state.scenario === "setup") state.scenario = "healthy";
    res.json({
      ok: true,
      setupUrl: `http://clawbridge-sandbox.localhost:${port}/#/general`,
      publicBaseUrl: `http://localhost:${port}`,
      tailscaleDns: "alphaclaw-sandbox.tailnet.local",
  ...
  app.post("/api/onboard/github/verify", (_req, res) => {
    res.status(410).json({
      ok: false,
      error: "GitHub import is not available in the Clawbridge UI sandbox.",
    });
  });
  ...
  // Sandbox model-key vault flow: a requested key auto-approves after a short
  // delay so the pending → active lifecycle can be exercised end to end.
  const kSandboxVaultKeyApprovalMs = 15000;
  const getSandboxVaultKeys = () => {
    if (!state.modelVaultKeys) state.modelVaultKeys = {};
    return state.modelVaultKeys;
  };
  const isSandboxVaultKeyApproved = (entry) =>
    !!entry &&
    (entry.active || Date.now() - entry.createdAt >= kSandboxVaultKeyApprovalMs);
  ...
  app.get("/api/models/config", (_req, res) => {
    ...
    const primary =
      ...
      "";
    const anthropicVaulted = isSandboxVaultKeyApproved(
      getSandboxVaultKeys().anthropic,
    );
    res.json({
      ok: true,
      primary,
      ...
      authProfiles: [
        {
          ...
          key: anthropicVaulted
            ? "__agent_vault_anthropic_api_key__"
            : "sk-ant-sandbox",
        },
        {
      ...
      vaultBrokering: {
        enforced: true,
        providers: listVaultBrokeredModelProviders(),
      },
    });
  });
  app.post("/api/models/vault-key", (req, res) => {
    const provider = String(req.body?.provider || "").trim();
    if (!provider) {
      return res.status(400).json({ ok: false, error: "Missing provider" });
    }
    const vaultKeys = getSandboxVaultKeys();
    const credentialKey = `${provider.toUpperCase().replace(/-/g, "_")}_API_KEY`;
    const existing = vaultKeys[provider];
    if (isSandboxVaultKeyApproved(existing)) {
      vaultKeys[provider] = { ...existing, active: true };
      return res.json({ ok: true, status: "active", provider, credentialKey });
    }
    const proposalId =
      existing?.proposalId || 41 + Object.keys(vaultKeys).length;
    if (!existing) {
      vaultKeys[provider] = {
        createdAt: Date.now(),
        proposalId,
    ...
    return res.status(201).json({
      ...
      proposal: {
        ...
        reason: `Broker the ${provider} model API key so model calls authenticate at the Agent Vault proxy.`,
        requestInstructions: [],
        createdAt: new Date().toISOString(),
        approvalUrl: `https://www.teamyou.com/openclaw/agent-vault/inst_sandbox?return_to=%2Fapprove%2F${proposalId}%3Ftoken%3Dsandbox`,
      },
    });
  });
  app.get("/api/agent-vault/proposals/:id", (req, res) => {
    const id = Number.parseInt(String(req.params.id || ""), 10);
    const entry = Object.values(getSandboxVaultKeys()).find(
      (candidate) => candidate.proposalId === id,
    );
    res.json({
      ok: true,
      proposal: {
        id,
        status: !entry || isSandboxVaultKeyApproved(entry)
          ? "approved"
          : "pending",
      },
    });
  ...
  // Stateful so the disconnect → device sign-in journey can be exercised.
  let sandboxCodexConnected = true;
  let sandboxDevicePolls = 0;
  app.get("/api/codex/status", (_req, res) => {
    res.json(
      sandboxCodexConnected
        ? {
            connected: true,
            accountId: "sandbox-codex",
            ...(state.scenario === "attention"
              ? {
                  needsReconnect: true,
                  reconnectReason: "oauth_broker_grant_invalid",
                  reconnectMessage:
  ...
  app.post("/api/codex/exchange", (_req, res) => {
    sandboxCodexConnected = true;
    res.json({ ok: true, connected: true });
  });
  app.post("/api/codex/device/start", (_req, res) => {
    sandboxDevicePolls = 0;
    res.json({
      ok: true,
      sessionId: "sandbox-device-session",
  ...
  app.post("/api/codex/device/poll", (_req, res) => {
    sandboxDevicePolls += 1;
    if (sandboxDevicePolls >= 6) {
      sandboxCodexConnected = true;
      return res.json({ ok: true, status: "complete" });
    }
    res.json({ ok: true, status: "pending" });
  });
  app.get("/auth/codex/start", (_req, res) => {
    res.send(
      "<!DOCTYPE html><html><body><p>Sandbox Codex auth window. Close this window and paste a URL to test the manual flow.</p></body></html>",
    );
  });
  app.post("/api/codex/disconnect", (_req, res) => {
    sandboxCodexConnected = false;
  ...
  if (realClaudeCli) {
    ...
    app.post("/api/account-logins/claude-cli/login/:id/cancel", (_req, res) => {
      const changed = !!state.claudeCliLoginStarted;
      state.claudeCliLoginStarted = false;
      state.claudeCliLoggedIn = false;
      state.claudeCliLoginOutput = "";
      res.json({ ok: true, changed, status: "cancelled" });
    });
  ...
  app.get("/api/env", (_req, res) =>
    res.json({
      ok: true,
      vars: clone(state.envVars),
      reservedKeys: [],
      agentVaultManaged: true,
  ...
  app.get("/api/agent-vault/status", (_req, res) => {
    res.json({
      ok: true,
      status: {
        managed: true,
        ...
        initialized: true,
        connected: true,
        operatorUrl: "https://agent-vault-sandbox.example.ts.net",
        entryUrl:
          "https://www.teamyou.com/openclaw/agent-vault/inst_sandbox",
        ownerEnrollmentPending: false,
        restartRequired: false,
      },
    });
  });
  app.get("/api/agent-vault/credentials", (_req, res) => {
    const approvedModelKeys = Object.values(getSandboxVaultKeys())
      .filter(isSandboxVaultKeyApproved)
      .flatMap((entry) => entry.credentialKeys || [entry.credentialKey]);
    res.json({
      ok: true,
      vault: "default",
      credentials: ["GITHUB_TOKEN", "STRIPE_API_KEY", ...approvedModelKeys],
      credentialDetails: [
        {
          key: "GITHUB_TOKEN",
          vault: "default",
        ...
        },
        {
          ...
          // Sandbox exercise of the stale-credential rendering.
          usedBy: [],
          inUse: false,
        },
      ],
      ...
      consoleUrl:
        "https://example.invalid/openclaw/agent-vault/inst_sandbox?return_to=%2Fvaults%2Fdefault%2Fcredentials",
    });
  });
  app.post("/api/agent-vault/proposals", (req, res) => {
    res.status(201).json({
      ok: true,
      ...
      proposal: {
        ...
        createdAt: new Date().toISOString(),
        approvalUrl:
          "https://www.teamyou.com/openclaw/agent-vault/inst_sandbox?return_to=%2Fapprove%2F7%3Ftoken%3Dsandbox",
      },
    });
  });
  ...
  // Mirrors the real server: with zero sessions the default agent's main
  // session is synthesized so the chat composer is immediately usable.
  const kSandboxFallbackMainSession = {
    key: "agent:main:main",
    sessionId: "",
    updatedAt: 0,
    agentId: "main",
  ...
  app.get("/api/agent/sessions", (_req, res) =>
    res.json({
      ...
      sessions: state.sessions.length
        ...
        : [clone(kSandboxFallbackMainSession)],
    }),
  );
  ...
  app.get("/api/agents/:id/skills", (req, res) => res.json({
    ok: true,
    workspaceDir: `workspace/${req.params.id}`,
    skills: [
      { name: "github", emoji: "🐙", description: "Work with repositories and pull requests.", eligible: true, blockedByAgentFilter: false },
      { name: "google-drive", emoji: "📁", description: "Read and organize shared Drive files.", eligible: true, blockedByAgentFilter: false },
      { name: "mac-notes", emoji: "📝", description: "Requires macOS and is unavailable on this host.", eligible: false, blockedByAgentFilter: false },
    ],
  }));
  ...
  app.get("/api/channels/accounts", (_req, res) => {
    res.json({
      ...
      vaultBrokering: {
        ...
        consoleUrl:
          "https://www.teamyou.com/openclaw/agent-vault/inst_sandbox000?return_to=%2Fvaults%2Fdefault%2Fcredentials",
      },
    });
  });
  // Sandbox channel vault-token flow mirrors the model-key mock: a request
  // auto-approves after a short delay so the pending -> active lifecycle can
  // be exercised end to end in the wizard.
  app.post("/api/channels/vault-token", (req, res) => {
    const provider = String(req.body?.provider || "").trim();
    const accountId = String(req.body?.accountId || "").trim() || "default";
    if (!provider) {
      return res.status(400).json({ ok: false, error: "Missing provider" });
    }
    const vaultKeys = getSandboxVaultKeys();
    const entryKey = `channel:${provider}:${accountId}`;
    const suffix = accountId === "default" ? "" : `_${accountId.replace(/-/g, "_").toUpperCase()}`;
    const slotBase = provider === "slack"
      ? [`SLACK_BOT_TOKEN${suffix}`, `SLACK_APP_TOKEN${suffix}`]
      : [`${provider.toUpperCase()}_BOT_TOKEN${suffix}`];
    const slots = slotBase.map((envKey) => ({
      envKey,
      placeholder: `__agent_vault_${envKey.toLowerCase()}__`,
    }));
    const existing = vaultKeys[entryKey];
    if (isSandboxVaultKeyApproved(existing)) {
      vaultKeys[entryKey] = { ...existing, active: true };
      return res.json({ ok: true, status: "active", provider, accountId, slots });
    }
    const proposalId = existing?.proposalId || 61 + Object.keys(vaultKeys).length;
    if (!existing) {
      vaultKeys[entryKey] = {
        createdAt: Date.now(),
        proposalId,
        credentialKey: slots[0].envKey,
        credentialKeys: slots.map((slot) => slot.envKey),
      };
    }
    return res.status(201).json({
      ok: true,
      ...
      proposal: {
        ...
        credentialKeys: slots.map((slot) => slot.envKey),
        key: slots[0].envKey,
        description: `${provider} channel token`,
        reason: `Broker the ${provider} channel token so channel traffic authenticates at the Agent Vault proxy.`,
        requestInstructions: [],
        createdAt: new Date().toISOString(),
        approvalUrl: `https://www.teamyou.com/openclaw/agent-vault/inst_sandbox?return_to=%2Fapprove%2F${proposalId}%3Ftoken%3Dsandbox`,
      },
  ...
  app.post("/api/channels/discord/inspect-token", (_req, res) => res.json({
    ...
    bot: {
      ...
      guilds: [{ id: "234567890123456789", name: "Clawbridge Test Server" }],
    },
  }));
  app.post("/api/channels/slack/inspect-credentials", (_req, res) => res.json({
    ...
    slack: {
      ...
      workspace: {
        id: "T0123456789",
        name: "Clawbridge Test Workspace",
  ...
  app.put("/api/channels/accounts/policy", (req, res) => res.json({ ok: true, restartRequired: false, policy: req.body?.policy || {} }));
  ...
  app.get("/api/google/accounts", (_req, res) => res.json({ ok: true, accounts: [{ id: "default", email: "sandbox@example.com", client: "default", authenticated: true, services: ["gmail"], ...(state.scenario === "attention" ? { oauthHealth: { checked: true, healthy: false, reconnectRequired: true, error: "provider_http_401", checkedAt: Date.now() } } : {}) }] }));
  ...
  // Start the sandbox signed out with SANDBOX_COMPOSIO_LOGGED_OUT=1 to demo
  // the dashboard sign-in journey; the login/start mock signs in after a few
  // seconds to simulate the browser flow completing.
  let sandboxComposioLoggedIn =
    String(process.env.SANDBOX_COMPOSIO_LOGGED_OUT || "") !== "1";
  const sandboxComposioStatus = () => ({
    ...
    cliInstalled: true,
    cliVersion: "0.2.32",
    targetCliVersion: "0.2.32",
    cliInstalling: false,
    cliUpdating: false,
    loggedIn: sandboxComposioLoggedIn,
    account: sandboxComposioLoggedIn
      ? { email: "sandbox@example.com", orgName: "sandbox_workspace" }
      : { email: "", orgName: "" },
    loginPending: false,
    loginError: "",
    apiKeyConfigured: true,
    accounts: sandboxComposioLoggedIn
      ? [
      ...
      : [],
    googleAccounts: sandboxComposioLoggedIn
      ? [
        { id: "ca_sandbox_gmail", toolkit: "gmail", status: "ACTIVE", active: true, label: "sandbox@example.com" },
        { id: "ca_sandbox_cal", toolkit: "googlecalendar", status: "ACTIVE", active: true, label: "sandbox@example.com" },
      ]
  ...
  app.post("/api/composio/link", (req, res) => res.json({ ok: true, toolkit: String(req.body?.toolkit || "gmail"), redirectUrl: "https://backend.composio.dev/s/sandbox-link" }));
  app.post("/api/composio/login/start", (_req, res) => {
    setTimeout(() => {
      sandboxComposioLoggedIn = true;
    }, 6000);
    res.json({ ok: true, loginUrl: "https://dashboard.composio.dev/?cliKey=sandbox-login" });
  });
  let sandboxGmailWatch = { enabled: false, running: false, pid: null, startedAt: null, lastEventAt: null, lastError: "" };
  app.post("/api/composio/gmail-watch/enable", (_req, res) => {
    sandboxGmailWatch = { ...sandboxGmailWatch, enabled: true, running: true, pid: 4242, startedAt: 1752000000000 };
    res.json({ ok: true, gmailWatch: sandboxGmailWatch });
  });
  app.post("/api/composio/gmail-watch/disable", (_req, res) => {
    sandboxGmailWatch = { ...sandboxGmailWatch, enabled: false, running: false, pid: null };
    res.json({ ok: true, gmailWatch: sandboxGmailWatch });
  });
  ...
  app.get("/api/advanced-control/status", (_req, res) => {
    res.json({
      ok: true,
      acknowledged: advancedControlAcknowledged,
      label: "Advanced — unmanaged changes",
      warningVersion: "teamyou.advanced-control-warning/v1",
      managedConfigRevision: "sandbox",
    });
  });
  app.post("/api/advanced-control/acknowledge", (req, res) => {
    advancedControlAcknowledged = true;
    ...
    res.json({
      ...
      auditId: 1,
      url:
        returnTo === "/openclaw" || returnTo.startsWith("/openclaw/")
          ? returnTo
          : "/openclaw",
      warningVersion: "teamyou.advanced-control-warning/v1",
      managedConfigRevision: "sandbox",
