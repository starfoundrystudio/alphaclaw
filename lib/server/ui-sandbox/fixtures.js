...
const kModelAccessModes =
  kBootstrapModelCatalog.accessModes && typeof kBootstrapModelCatalog.accessModes === "object"
    ? clone(kBootstrapModelCatalog.accessModes)
    : null;
...
const createState = ({ mode = "dashboard", scenario = "healthy", port = 3001 } = {}) => {
  ...
  return {
    ...
    cronJobs,
    doctorCards,
    modelsConfig: createModelsConfig(),
    watchdogSettings: { autoRepair: true, notifications: true },
    openAiCompatApiEnabled: true,
    claudeCliLoggedIn: mode !== "setup",
    claudeCliConfigured: mode !== "setup",
  ...
  return {
    ...
    gatewayHop: attention
      ? {
          ...
          checkedAt: iso(-40 * 1000),
          checkedAgeMs: 40 * 1000,
          lastOkAt: iso(-7 * 60 * 1000),
          consecutiveFailures: 3,
          error: "ssh_failed",
          detail: "ssh: connect to host 10.162.2.3 port 22: Connection refused",
          stale: false,
          reason: null,
          path: "/run/alphaclaw/gateway-hop-probe.json",
          staleAfterMs: 6 * 60 * 1000,
          incidentActive: true,
          incidentOpenedAt: iso(-5 * 60 * 1000),
