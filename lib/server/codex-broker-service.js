...
// OpenClaw (2026.7.1 through the pinned 2026.9.x) starts its own OAuth refresh
// at five minutes. Refresh
// earlier so it never races the non-secret placeholder in the auth store.
const kRefreshMarginMs = 10 * 60 * 1000;
const kOpenclawRefreshMarginMs = 5 * 60 * 1000;
const kMinimumRefreshDelayMs = 60 * 1000;
const kRefreshSafetyMarginMs = 60 * 1000;
const kRefreshOperationBudgetMs = 60 * 1000;
...
const createCodexBrokerService = ({
  ...
  minimumRefreshDelayMs = kMinimumRefreshDelayMs,
  ...
  const shortRefreshTargetRemainingMs =
    kOpenclawRefreshMarginMs +
    refreshRetryMs +
    kRefreshOperationBudgetMs +
    kRefreshSafetyMarginMs;
  const minimumSafeRemainingMs =
    shortRefreshTargetRemainingMs + minimumRefreshDelayMs;
  ...
  const clearRevocationRetryTimer = () => {
    if (revocationRetryTimer) clearTimeoutFn(revocationRetryTimer);
    revocationRetryTimer = null;
    revocationRetryAttempt = 0;
  };

  const schedulePendingRevocationRetry = () => {
    if (!started || !revocationStore.isPending()) {
      clearRevocationRetryTimer();
      return;
    }
    if (revocationRetryTimer) return;
    const delays =
      Array.isArray(revocationRetryDelaysMs) &&
      revocationRetryDelaysMs.length > 0
  ...
  const scheduleForProfile = (profile = getProfile()) => {
    ...
    if (remainingMs <= minimumSafeRemainingMs) {
      // Initial OAuth credentials are published before they have gone through
      // the broker-response lifetime check. Refresh those immediately once;
      // an unusably short broker response then enters normal error backoff.
      scheduleRefresh(0);
      return;
    }
    // Short-lived provider tokens can be inside the normal lead window as soon
    // as they arrive. Refresh one minimum interval before OpenClaw's own
    // five-minute boundary instead of creating an immediate broker/publish loop.
    const desiredDelayMs =
      remainingMs > refreshMarginMs
        ? remainingMs - refreshMarginMs
        : remainingMs - shortRefreshTargetRemainingMs;
    scheduleRefresh(Math.max(minimumRefreshDelayMs, desiredDelayMs));
  };
  ...
  const retryPendingRevocation = () =>
    withLifecycleLock(retryPendingRevocationUnlocked).then((result) => {
      if (result.pending) schedulePendingRevocationRetry();
      return result;
    });
  ...
  const retryPendingAccessPublicationUnlocked = async () => {
    ...
    try {
      // Re-enter the atomic auth-store mutation before publication. Besides
      // making the live snapshot replayable, this consumes any physical
      // SQLite sanitation marker left by a committed token rotation whose WAL
      // checkpoint or VACUUM was temporarily busy.
      authProfiles.upsertCodexProfile({
        access: profile.access,
        refresh: CODEX_BROKER_REFRESH_PLACEHOLDER,
        expires: profile.expires,
