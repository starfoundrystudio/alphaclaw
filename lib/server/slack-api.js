 ...
 */
const createSlackApi = (getToken, { fetchImpl = fetch } = {}) => {
  const call = async (method, body = {}) => {
    const token = typeof getToken === "function" ? getToken() : getToken;
    if (!token) throw new Error("SLACK_BOT_TOKEN is not set");
    const res = await fetchImpl(`${kSlackApiBase}/${method}`, {
      method: "POST",
      headers: {
  ...
  const authScopes = () => call("auth.scopes");
  const appsPermissionsInfo = () => call("apps.permissions.info");
  const openSocketConnection = () => call("apps.connections.open");
