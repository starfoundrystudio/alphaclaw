...
const collectPricingFromPluginManifest = ({
  value,
  provider,
  pricingByModelKey,
}) => {
  if (Array.isArray(value)) {
    for (const entry of value) {
      collectPricingFromPluginManifest({ value: entry, provider, pricingByModelKey });
    }
    return;
  }
  ...
  if (
    ...
    cost &&
    typeof cost === "object" &&
    Number.isFinite(Number(cost.input)) &&
    Number.isFinite(Number(cost.output))
  ) {
    setPricingCandidates(
      pricingByModelKey,
      provider ? `${provider}/${modelId}` : modelId,
      {
        input: toFiniteRate(cost.input),
        output: toFiniteRate(cost.output),
        cacheRead: toFiniteRate(cost.cacheRead, 0),
        cacheWrite: toFiniteRate(cost.cacheWrite, 0),
      },
    );
  }
  for (const nested of Object.values(value)) {
    collectPricingFromPluginManifest({
      value: nested,
      provider,
      pricingByModelKey,
    });
...
const extractPricingFromPluginManifests = ({ distDirPath, pricingByModelKey }) => {
  const extensionsDir = path.join(distDirPath, "extensions");
  let providerDirs = [];
  try {
    providerDirs = fs.readdirSync(extensionsDir, { withFileTypes: true });
  } catch {
    return;
  }
...
const loadOpenclawNodeModulesPricingMap = () => {
  ...
  extractPricingFromPluginManifests({ distDirPath, pricingByModelKey });
