...
const createCronService = ({
  ...
  readConfig = () => readOpenclawConfig({ openclawDir: OPENCLAW_DIR }),
}) => {
  ...
  const getStatus = async () => {
    const status = await requestGateway("cron.status", {});
    let skipMissedJobs = false;
    try {
      skipMissedJobs = readConfig()?.cron?.skipMissedJobs === true;
    } catch {}
    return { ...status, skipMissedJobs };
  };
  ...
  const getBulkJobUsage = async ({ sinceMs = 0 } = {}) => {
    ...
    const safeSinceMs = toFiniteNumber(sinceMs, 0);
    const runs = await getBulkJobRuns({
      sinceMs: safeSinceMs,
      limitPerJob: kMaxPageSize,
    });
    const rows = await Promise.all(jobs.map(async (job) => {
      const usage = await Promise.resolve(getSessionUsageByKeyPattern({
        keyPattern: `%:cron:${job.id}%`,
        sinceMs: safeSinceMs,
      }));
      const totals = usage?.totals || {};
      const runCount = toFiniteNumber(totals.runCount, 0);
      const totalTokens = toFiniteNumber(totals.totalTokens, 0);
      const durations = (runs.byJobId[job.id]?.entries || [])
        .map((entry) => toFiniteNumber(entry?.durationMs, -1))
        .filter((durationMs) => durationMs >= 0);
      const totalDurationMs = durations.reduce((sum, value) => sum + value, 0);
      return [job.id, {
