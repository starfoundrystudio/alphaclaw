...
const createOpenclawVersionService = ({
  ...
  const findInstallDir = () => {
    // Resolve the consumer app root, not this package directory.
  ...
  // Install to a temp directory, then copy into the real node_modules so the
  // running process never asks npm to replace its own loaded package tree.
  ...
  const updateOpenclaw = async () => {
    ...
    try {
      ...
      let pluginReconcile = null;
      if (isOnboarded() && typeof reconcileOpenclawPlugins === "function") {
        pluginReconcile = await reconcileOpenclawPlugins({
          rootDir,
          openclawDir,
          fsModule,
          logger,
