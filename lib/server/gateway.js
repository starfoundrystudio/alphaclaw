...
let managedGatewayRestartPending = false;
let managedGatewayForceKillTimer = null;
const kManagedGatewayRestartKillGraceMs = 10 * 1000;
...
const launchGatewayProcess = () => {
  ...
  child.on("exit", (code, signal) => {
    ...
    if (managedGatewayForceKillTimer) {
      clearTimeout(managedGatewayForceKillTimer);
      managedGatewayForceKillTimer = null;
    }
    console.log(
    ...
    if (managedGatewayRestartPending && expectedExit) {
      managedGatewayRestartPending = false;
      console.log("[alphaclaw] Relaunching managed gateway after restart");
...
const requestManagedGatewayRestart = () => {
  if (
    !gatewayChild ||
    gatewayChild.exitCode !== null ||
    gatewayChild.killed ||
  ...
  managedGatewayRestartPending = true;
  console.log("[alphaclaw] Restarting managed gateway process");
  try {
    gatewayChild.kill("SIGTERM");
  } catch (err) {
    console.warn(`[alphaclaw] Failed to stop managed gateway: ${err.message}`);
    managedGatewayRestartPending = false;
    return false;
  }
  if (managedGatewayForceKillTimer) {
  ...
  managedGatewayForceKillTimer = setTimeout(() => {
    managedGatewayForceKillTimer = null;
    if (
      !gatewayChild ||
      gatewayChild.exitCode !== null ||
    ...
    console.warn("[alphaclaw] Managed gateway did not stop; forcing exit");
    try {
      gatewayChild.kill("SIGKILL");
    } catch {}
  }, kManagedGatewayRestartKillGraceMs);
  if (typeof managedGatewayForceKillTimer.unref === "function") {
    managedGatewayForceKillTimer.unref();
  }
  return true;
};
...
const restartGateway = (reloadEnv) => {
  reloadEnv();
  if (requestManagedGatewayRestart()) return;
  runGatewayCmd("stop");
  launchGatewayProcess();
};
...
const ensureGatewayProxyConfig = (origin) => {
  ...
  try {
    ...
    if (!String(cfg.gateway.mode || "").trim()) {
      console.warn(
        "[alphaclaw] Skipping gateway proxy config update; gateway.mode is missing. Run openclaw doctor --fix --yes.",
      );
      return false;
    }
    let changed = false;
