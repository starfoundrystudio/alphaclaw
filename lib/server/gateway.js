...
const waitForGatewayReady = async ({
  ...
  while (Date.now() - startedAt < timeoutMs) {
    if (await isGatewayRunning() && isAlive() && hasReadyEvidence()) return true;
...
// OpenClaw 2026.7.1 reports readiness from the replacement process itself.
...
const runGatewayRestartCmd = async (cmd, { retriedForMigrationLock = false } = {}) => {
  ...
  child.on("exit", (code, signal) => {
    ...
    gatewayExitHandler?.({
      code, signal,
      expectedExit: expectedExit || retryDelay !== null,
      ...(expectedExit || retryDelay !== null ? {
        expectedExitReason: retryDelay !== null ? "migration_retry" : "managed_restart",
    ...
    if (gatewayExitHandler) {
      try {
        gatewayExitHandler({
          code,
          signal,
          expectedExit: expectedExit || migrationRetryScheduled,
          ...
          ...(expectedExit || migrationRetryScheduled ? {
            expectedExitReason: migrationRetryScheduled ? "migration_retry" : "managed_restart",
...
const restartGatewayLight = async (reloadEnv) => {
  reloadEnv();
  if (gatewayRestartPromise || lifecycleOwnership.isBusy()) return runGatewayColdStart();
  if (await isGatewayRunning()) {
    if (gatewayRestartPromise || lifecycleOwnership.isBusy()) return runGatewayColdStart();
...
const createGatewaySignalHandler = ({
  ...
  return () => {
    if (handled) return;
    handled = true;
    // Clawbridge owns `openclaw gateway run` as a child process. Invoking the
    // separate `openclaw gateway stop` CLI during a systemd service swap is
    // both unnecessary and unsafe: the CLI itself runs startup migrations and
    // can be killed mid-lease. Terminate only the child we actually own.
    stopManagedChild();
    exitProcess(0);
  };
};
