...
} = require("./openclaw-pairing-store");
const {
...
const getChannelStatus = () => {
  try {
    ...
    const channels = {};
    const approvedPairings = readApprovedPairingEntries({ openclawDir: OPENCLAW_DIR });
    ...
    for (const ch of Object.keys(kChannelDefs)) {
      ...
      for (const [accountId, accountConfig] of accountEntries) {
        ...
        const normalizedAccountId = normalizeChannelAccountId(accountId);
        // Config allowFrom plus approvals in OpenClaw's state database
        // (G3 #36: only the first owner is written to the config).
        pairedByAccount.set(
          normalizedAccountId,
          countApprovedSenders({
            ...
            inlineAllowFrom: accountConfig?.allowFrom,
          }),
        );
      }
