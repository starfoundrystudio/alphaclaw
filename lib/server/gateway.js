...
const runGatewayRestartCmd = async (cmd, { retriedForMigrationLock = false } = {}) => {
  ...
  if (!retriedForMigrationLock) {
    ...
    if (delayMs !== null) {
      ...
      await sleep(delayMs);
      await runGatewayRestartCmd(cmd, { retriedForMigrationLock: true });
    }
  }
};
...
const launchGatewayProcess = ({ retriedForMigrationLock = false } = {}) => {
  ...
  child.on("exit", (code, signal) => {
    ...
    if (!expectedExit && isCurrentChild && !retriedForMigrationLock) {
      ...
      if (delayMs !== null) {
        ...
        gatewayMigrationRetryTimer = setTimeout(() => {
          gatewayMigrationRetryTimer = null;
