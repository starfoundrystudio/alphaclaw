...
// OpenClaw 2026.9 refuses to finish startup when the config (or its migration
// inputs) changed while the Gateway was starting, and asks for a retry. That
// happens when another writer touches openclaw.json mid-startup (G3 finding
// #23: the host's post-onboard timer rewrote it during a managed restart).
// The config itself is fine, so a prompt retry recovers.
const kStartupConfigChangedPattern =
  /selected config changed during startup|migration inputs changed during startup|without a prepared config snapshot/;
const kStartupConfigChangedRetryDelayMs = 3000;

const getStartupConfigChangedRetryDelayMs = (text) =>
  kStartupConfigChangedPattern.test(String(text || "")) ? kStartupConfigChangedRetryDelayMs : null;

// Startup exits that recover by retrying: a stale migrations lock (wait for
// its expiry) or a config that changed during startup (retry promptly).
const getGatewayStartupRetryDelayMs = (text, options) =>
  getMigrationLockRetryDelayMs(text, options) ?? getStartupConfigChangedRetryDelayMs(text);
...
const runGatewayRestartCmd = async (cmd, { retriedForMigrationLock = false } = {}) => {
  ...
  child.on("exit", (code, signal) => {
    ...
    const retryDelay = !retriedForMigrationLock
      ? getGatewayStartupRetryDelayMs(combinedOutputTail) : null;
    console.log(
  ...
  console.warn(
    exited
      ? `[alphaclaw] Gateway ${cmd} exited before becoming ready (code ${exitCode ?? "null"}, ${Date.now() - startedAt}ms)`
  ...
  if (!retriedForMigrationLock) {
    const delayMs = getGatewayStartupRetryDelayMs(combinedOutputTail);
    if (delayMs !== null) {
      console.warn(
        `[alphaclaw] Gateway start ${getStartupConfigChangedRetryDelayMs(combinedOutputTail) !== null ? "refused because its config changed during startup" : "blocked by a stale startup-migrations lock"}; retrying in ${Math.round(delayMs / 1000)}s`,
      );
      await sleep(delayMs);
...
const relaunchAfterFailedColdStart = async () => {
  if (
    gatewayRestartPromise ||
    gatewayMigrationRetryTimer ||
    lifecycleOwnership.isBusy() ||
    hasActiveManagedGatewayChild() ||
    (await isGatewayRunning())
  ) {
    return;
  }
  console.warn("[alphaclaw] Gateway restart failed; relaunching the managed Gateway");
  launchGatewayProcess();
};
...
const runGatewayColdStart = () => {
  ...
  if (!gatewayRestartPromise) {
    ...
    gatewayRestartPromise = run.finally(() => { gatewayRestartPromise = null; });
    // A failed restart must not leave the Gateway down. While the restart
    // owned startup, the exit handler's relaunch was refused; once ownership
    // is released, fall back to one normal launch so the watchdog sees a
    // real start/exit again (G3 finding #23: stranded until the next health
    // check ran a repair).
    run.catch(() => {
      const timer = setTimeout(() => { void relaunchAfterFailedColdStart(); }, 0);
      timer.unref?.();
    });
  }
  ...
  child.on("exit", (code, signal) => {
    ...
    if (!expectedExit && isCurrentChild && !retriedForMigrationLock) {
      ...
      if (delayMs !== null) {
        ...
        console.warn(
          `[alphaclaw] Initial gateway start ${getStartupConfigChangedRetryDelayMs(combinedOutputTail) !== null ? "refused because its config changed during startup" : "blocked by a stale startup-migrations lock"}; retrying in ${Math.round(delayMs / 1000)}s`,
        );
        gatewayMigrationRetryTimer = setTimeout(async () => {
          ...
          console.log("[alphaclaw] Retrying initial gateway start");
...
// Start time of the running managed Gateway child, or 0 when none is running
// or it has not reported ready. Used to tell whether an on-disk plugin patch
// predates the Gateway process that loaded the plugin.
const getManagedGatewayStartedAtMs = () => {
  if (!gatewayChild || gatewayChild.exitCode !== null || gatewayChild.killed) return 0;
  const startup = gatewayStartupState.get(gatewayChild);
  return startup?.ready ? Number(startup.startedAtMs) || 0 : 0;
};
...
// OpenClaw 2026.9 hands CLI plugin installs to a running Gateway. Our
// Gateway refused them while it ran with OPENCLAW_CONFIG_READONLY=1 (removed
// in beta.5), and a reconcile that writes config beside a live Gateway still
// races it. Plugin maintenance that has to run while the server is up
// therefore stops the managed Gateway,
