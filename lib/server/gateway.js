...
const runGatewayRestartCmd = async (cmd, { retriedForMigrationLock = false } = {}) => {
  ...
  if (!retriedForMigrationLock) {
    const retryAfterMs = parseMigrationLockRetryAfterMs(combinedOutputTail);
    const waitMs = retryAfterMs === null ? null : retryAfterMs - Date.now();
    if (waitMs !== null && waitMs < kMigrationLockMaxWaitMs) {
      const delayMs = Math.max(waitMs, 0) + 3000;
      console.warn(
