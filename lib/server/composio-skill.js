...
const buildComposioSkillContent = ({ fs, composioState }) => {
  ...
  if (googleAccounts.length) {
    ...
    lines.push(
      "No Google Workspace accounts are linked yet. The user can link one from the Clawbridge dashboard (General tab), or run `composio link <toolkit>` (e.g. `gmail`, `googlecalendar`). Re-check with `composio connections list`.",
    );
  }
  lines.push("");
