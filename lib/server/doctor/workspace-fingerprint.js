...
const shouldStopScan = (scanState, options) => {
...
const recordScanError = (scanState, filePath, error, fallbackMessage) => {
...
const scanFileIntoManifest = ({
  ...
  try {
    ...
    recordScanError(scanState, relativePath, error, "Could not stat file");
    return;
  }
  ...
  if (shouldHash) {
    try {
      ...
      recordScanError(scanState, relativePath, error, "Could not hash file");
    }
  }
...
const scanDirectoryIntoManifest = ({
  rootDir,
  options,
  scanState,
  manifest,
  ...
  if (shouldStopScan(scanState, options)) return;
  let entries = [];
  try {
    ...
    recordScanError(
      scanState,
      normalizeRelativePath(rootDir, currentDir),
      error,
