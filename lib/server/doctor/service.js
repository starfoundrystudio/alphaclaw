...
const { createWorkspaceSnapshotManager } = require("./workspace-snapshot-manager");
const {
  kDoctorEngine,
...
const createDoctorService = ({
  ...
  protectedPaths = [],
  lockedPaths = [],
  workspaceSnapshotManager = null,
}) => {
  const state = {
    activeRunId: 0,
    activeRunPromise: null,
    startingRunPromise: null,
  };
  ...
  const refreshWorkspaceSnapshot = ({ force = false } = {}) =>
    snapshotManager.refresh({ force });

  const getCachedWorkspaceSnapshot = () => snapshotManager.getSnapshot();

  const triggerWorkspaceSnapshotRefresh = ({ force = false } = {}) =>
    snapshotManager.triggerRefresh({ force });
  ...
  const buildWorkspaceScanStatus = (snapshot = null) => {
    const managerStatus =
      typeof snapshotManager.getStatus === "function" ? snapshotManager.getStatus() : {};
    return {
      ...(snapshot?.scan || {}),
      refreshing: !!managerStatus.refreshing,
      computedAt: managerStatus.computedAt || null,
      refreshStartedAt: managerStatus.refreshStartedAt || null,
  ...
  const buildStatus = () => {
    ...
    const needsWorkspaceSnapshot = !!baselineRun || !!initialBaseline || !latestCompletedRun;
    if (needsWorkspaceSnapshot) triggerWorkspaceSnapshotRefresh();
    const currentSnapshot = needsWorkspaceSnapshot ? getCachedWorkspaceSnapshot() : null;
    ...
    return {
      ...
      runInProgress: !!state.activeRunPromise || !!state.startingRunPromise,
      lastRunAt,
      lastRunAgeMs,
      ...
      workspaceScan: buildWorkspaceScanStatus(currentSnapshot),
      latestRun,
    };
  };
  ...
  const buildAlreadyRunningResult = () => ({
    ok: false,
    alreadyRunning: true,
  ...
  const runDoctor = async () => {
    if (state.activeRunPromise || state.startingRunPromise) {
      return buildAlreadyRunningResult();
    }
    let workspaceSnapshot = null;
    state.startingRunPromise = refreshWorkspaceSnapshot({ force: true });
    try {
      workspaceSnapshot = await state.startingRunPromise;
    } finally {
      state.startingRunPromise = null;
    }
    ...
    if (
      !workspaceSnapshot.scan?.degraded &&
      latestCompletedRun &&
  ...
  const importDoctorResult = async ({
    ...
    const cards = [...bootstrapTruncationCards, ...normalizedResult.cards];
    captureEvidenceSnippets(cards, workspaceRoot);
    const workspaceSnapshot = await refreshWorkspaceSnapshot({ force: true });
    const runId = createDoctorRun({
      status: kDoctorRunStatus.completed,
      engine,
      workspaceRoot,
