...
const { withManagedOpenclawEnv } = require("./openclaw-runtime-env");
...
const kSharedAuthRuntimeStateKey = "authProfiles.state";
...
const readEffectiveAgentEntries = () => {
  ...
  const cliEnv = withManagedOpenclawEnv(Object.fromEntries(
    Object.entries({
      ...process.env,
  ...
  try {
    ...
    return listAgentEntries({ agents });
  } catch (error) {
...
const resolveSharedStateDatabasePath = () =>
  path.join(OPENCLAW_DIR, "state", "openclaw.sqlite");
...
const hasSqliteTable = (db, tableName) =>
  !!db
    .prepare(
...
const inspectSharedAuthOwnership = () => {
  const databasePath = resolveSharedStateDatabasePath();
  if (!fs.existsSync(databasePath)) return "legacy-main";
  return withNodeSqlite((sqlite) => {
    let db;
    try {
      db = new sqlite.DatabaseSync(databasePath, { readOnly: true });
      db.exec("PRAGMA busy_timeout = 5000;");
      if (!hasSqliteTable(db, "config_machine_state")) return "legacy-main";
      const row = db
        .prepare(
          "SELECT value_json FROM config_machine_state WHERE state_key = ?",
        )
        .get(kSharedAuthOwnershipStateKey);
      if (!row) return "legacy-main";
      const ownership = JSON.parse(row.value_json);
      if (
        ownership &&
      ...
      const error = new Error(
        `OpenClaw ${kSharedAuthOwnershipStateKey} ownership is invalid; run openclaw doctor --fix with the Gateway stopped.`,
      );
      error.code = "invalid_shared_auth_store_ownership";
      throw error;
    } finally {
      try {
        db?.close();
...
const resolveAuthStoreDatabaseTarget = (agentId = kDefaultAgentId) => {
  const normalizedAgentId = normalizeAgentId(resolveAgentTargetId(agentId));
  if (
    normalizedAgentId === kDefaultAgentId &&
    inspectSharedAuthOwnership() === "state-db"
  ) {
    return {
      kind: "shared-state",
      databasePath: resolveSharedStateDatabasePath(),
    };
  }
  return {
    kind: "agent",
    databasePath: resolveAuthProfileDatabasePath(agentId),
  };
};

const canWriteAuthStoreTarget = (db, target) =>
  target.kind === "shared-state"
    ? hasSqliteTable(db, "config_machine_state")
    : hasSqliteTable(db, "auth_profile_store") &&
      hasSqliteTable(db, "auth_profile_state");

const readAuthStoreRows = (db, target) => {
  if (!canWriteAuthStoreTarget(db, target)) return null;
  if (target.kind === "shared-state") {
    ...
    if (!storeRow?.value_json) return null;
    const stateRow = db
      .prepare(
        "SELECT value_json FROM config_machine_state WHERE state_key = ?",
      )
      .get(kSharedAuthRuntimeStateKey);
    return { storeJson: storeRow.value_json, stateJson: stateRow?.value_json };
  }
  const storeRow = db
  ...
  const stateRow = db
    ...
    .get(kAuthProfilePrimaryKey);
  return { storeJson: storeRow.store_json, stateJson: stateRow?.state_json };
};

const parseAuthStoreRows = (rows) => {
  if (!rows?.storeJson) return null;
  const parsedStore = JSON.parse(rows.storeJson);
  const parsedState = rows.stateJson ? JSON.parse(rows.stateJson) : {};
...
const readAuthStoreDatabase = (agentId = kDefaultAgentId) => {
  const target = resolveAuthStoreDatabaseTarget(agentId);
  if (!fs.existsSync(target.databasePath)) return null;
  return withNodeSqlite((sqlite) => {
    let db;
    try {
      db = new sqlite.DatabaseSync(target.databasePath, { readOnly: true });
      db.exec("PRAGMA busy_timeout = 5000;");
      return parseAuthStoreRows(readAuthStoreRows(db, target));
    } catch {
      return null;
    } finally {
      try {
...
const writeAuthStoreRows = (db, store, target) => {
  const now = Date.now();
  ...
  if (target.kind === "shared-state") {
    db.prepare(
      ...
      `,
    ).run(kSharedAuthStoreStateKey, storeJson, now);
  } else {
    db.prepare(
      `
      ...
      `,
    ).run(kAuthProfilePrimaryKey, storeJson, now);
  }
  const statePayload =
      ...
      db.prepare(
        `
          ...
          ON CONFLICT(state_key) DO UPDATE SET
            ...
            updated_at_ms = excluded.updated_at_ms
        `,
      ).run(kSharedAuthRuntimeStateKey, JSON.stringify(statePayload), now);
    } else {
      db.prepare(
        `
...
const writeAuthStoreDatabase = (agentId = kDefaultAgentId, store) => {
  const target = resolveAuthStoreDatabaseTarget(agentId);
  if (!fs.existsSync(target.databasePath)) return false;
  return withNodeSqlite((sqlite) => {
    let db;
    try {
      db = new sqlite.DatabaseSync(target.databasePath);
      db.exec("PRAGMA busy_timeout = 5000;");
      if (!canWriteAuthStoreTarget(db, target)) return false;
      db.exec("BEGIN IMMEDIATE;");
      writeAuthStoreRows(db, store, target);
      db.exec("COMMIT;");
      return true;
    } catch (err) {
      try {
...
const mutateAuthStoreDatabase = (agentId, mutate) => {
  ...
  return withNodeSqlite((sqlite) => {
    ...
    try {
      ...
      db.exec("BEGIN IMMEDIATE;");
      const databaseStore = parseAuthStoreRows(readAuthStoreRows(db, target));
      ...
      const sanitizationPending = fs.existsSync(
        resolveAuthDatabaseSanitizationMarkerPath(target.databasePath),
      );
      ...
      if (requiresPhysicalSanitization && !sanitizationPending) {
        ...
        markAuthDatabaseSanitizationPending(target.databasePath);
      }
      writeAuthStoreRows(db, store, target);
      db.exec("COMMIT;");
      if (requiresPhysicalSanitization) {
        physicallySanitizeAuthProfileDatabase(db);
        clearAuthDatabaseSanitizationPending(target.databasePath);
      }
...
const saveAuthStore = (agentId, store) => {
  ...
  if (persistInOpenclaw) {
    ...
    if (wroteDatabase) {
      // SQLite is OpenClaw's provider-auth source of truth. Once the merged
...
const loadOpenclawConfig = () => {
  return readOpenclawConfig({ openclawDir: OPENCLAW_DIR, fallback: {} });
};
...
const saveOpenclawConfig = (cfg) => {
  writeOpenclawConfig({ openclawDir: OPENCLAW_DIR, config: cfg, spacing: 2 });
};
...
const createAuthProfiles = () => {
  ...
  const removeClaudeCliProfile = () => {
    const changed = removeProfile(kClaudeCliProfileId);
    const store = loadAuthStore(kDefaultAgentId);
    const nextOrder = Array.isArray(store.order?.anthropic)
      ? store.order.anthropic.filter((entry) => entry !== kClaudeCliProfileId)
      : [];
    setAuthOrder("anthropic", nextOrder);
    const nextStore = loadAuthStore(kDefaultAgentId);
    if (nextStore.lastGood?.anthropic === kClaudeCliProfileId) {
      delete nextStore.lastGood.anthropic;
    }
    if (nextStore.lastGood?.[kClaudeCliProviderId] === kClaudeCliProfileId) {
      delete nextStore.lastGood[kClaudeCliProviderId];
    }
    saveAuthStore(kDefaultAgentId, nextStore);
    if (canSyncOpenclawAuthReferences()) {
      let cfg = loadOpenclawConfig();
      cfg = removeConfigAuthReference(cfg, kClaudeCliProfileId);
      cfg = syncConfigAuthOrder(cfg, "anthropic", nextOrder);
      saveOpenclawConfig(cfg);
    }
    return changed;
  };
