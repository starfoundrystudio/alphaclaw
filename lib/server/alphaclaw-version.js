...
const fetchLatestVersionFromRegistry = async ({
  ...
  if (typeof fetchImpl !== "function") {
    throw new Error("Fetch is not available for Clawbridge version checks");
  }
  ...
  const data = await parseJsonResponse(
    response,
    "Failed to fetch latest Clawbridge version",
  );
...
const detectUpdateStrategy = ({ env = process.env } = {}) => {
  ...
  return createUpdateStrategy({
    ...
    label: "This install",
    description:
      "This will install the latest @starfoundrystudio/alphaclaw package in place and restart Clawbridge.",
    steps: [
      "Clawbridge will install the latest published package in place",
...
const createAlphaclawVersionService = ({
  ...
  const buildVersionStatus = ({
    ...
    return {
      ...
      hasUpdate: Boolean(alphaclawHasUpdate),
          ...
          childProcess.exec(
            ...
            (copyErr) => {
              ...
              if (copyErr) {
                ...
                return reject(
                  new Error(
                    `Failed to copy updated Clawbridge files: ${copyErr.message}`,
                  ),
                );
              }
  ...
  const getVersionStatus = async (refresh) => {
    const strategy = detectUpdateStrategy({ env });
    ...
    try {
      ...
      return buildVersionStatus({
        ...
        ok: false,
        error: err.message || "Failed to fetch latest Clawbridge version",
      });
    }
  };
  ...
  const updateAlphaclaw = async () => {
    ...
    if (kUpdateInProgress) {
      return {
        status: 409,
        body: { ok: false, error: "Clawbridge update already in progress" },
      };
    }
    if (strategy.action !== "self-update") {
      return {
        ...
        body: {
          ok: false,
          error:
            strategy.description || "This deployment is updated outside Clawbridge.",
          updateStrategy: strategy,
        },
      };
    }
    ...
    try {
      ...
      return {
        status: 500,
        body: { ok: false, error: err.message || "Failed to update Clawbridge" },
      };
    }
  };
