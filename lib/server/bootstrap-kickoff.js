...
const { kAlphaclawSystemNotePrefix } = require("./chat-ws");
const {
...
// runs before the user ever types anything. The kAlphaclawSystemNotePrefix
...
const kBootstrapKickoffMessage =
  `${kAlphaclawSystemNotePrefix} Workspace setup just finished and the user ` +
...
// The post-onboard reconcile restarts the gateway shortly after onboarding;
// a kickoff run caught in that restart makes OpenClaw inject a visible
// crash-recovery prompt. Only fire once the gateway connection has been
// continuously up for this long.
const kDefaultMinGatewayStableMs = 90 * 1000;
...
const createBootstrapKickoffService = ({
  ...
  constants,
  requestGateway,
  // Defaults to "always stable" so callers without connection-age tracking
  // (and unit tests) keep the simple send-immediately behavior.
  getGatewayConnectionAgeMs = () => Infinity,
  logger = console,
  maxAttempts = kDefaultMaxAttempts,
  attemptDelayMs = kDefaultAttemptDelayMs,
  minGatewayStableMs = kDefaultMinGatewayStableMs,
  ...
  const attemptKickoff = async () => {
    ...
    // The sessions.list call above (re)establishes the gateway connection,
    // so the age reading below is meaningful. A recent (re)connect means the
    // gateway may still be mid-restart churn; hold the kickoff until the
    // connection has stayed up for the stability window.
    const gatewayConnectionAgeMs = getGatewayConnectionAgeMs();
    if (gatewayConnectionAgeMs < minGatewayStableMs) {
      return { ok: false, reason: "gateway_not_stable", retry: true };
    }
  ...
  const runKickoffFlow = async () => {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      if (bootstrapState.reason === "bootstrap_pending") {
        try {
          const result = await attemptKickoff();
