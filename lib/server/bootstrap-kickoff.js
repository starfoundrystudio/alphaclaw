  ...
  const runKickoffFlow = async () => {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      if (bootstrapState.reason === "bootstrap_pending") {
        try {
          // The first agent turn must obey the same gate as dashboard entry:
          // a listening gateway can still have Vault or watchdog repairs pending.
          if (await isRuntimeReady()) return await attemptKickoff();
        } catch (error) {
          logger.error(
