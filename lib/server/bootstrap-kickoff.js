...
const kDefaultUnansweredReplyGraceMs = 5 * 60 * 1000;
...
const createBootstrapKickoffService = ({
  ...
  // A greeting counts as "unanswered" only after the agent has had time to
  // answer it. Without this the boot that sends the greeting re-checked
  // history one second later and sent it again (G3, 2026-09-21: the agent
  // greeted twice, "the setup ping came through twice").
  unansweredReplyGraceMs = kDefaultUnansweredReplyGraceMs,
  now = () => Date.now(),
  logger = console,
  ...
  const retryUnansweredKickoff = async () => {
    ...
    if (
      ...
      now() - markedAtMs < unansweredReplyGraceMs
    ) {
      return { ok: false, reason: "kickoff_too_recent" };
    }
    if (!(await isRuntimeReady()) || !(await isModelAuthReady())) {
