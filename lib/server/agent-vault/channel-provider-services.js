...
const buildChannelProviderAccessRequests = (provider, accountId = "default") => {
  ...
  return config.services.map((service) => ({
    ...
    reason: `Broker the ${config.provider} channel credential(s) for account "${config.accountId}" so traffic to ${service.host} authenticates at the Agent Vault proxy. AlphaClaw stores only non-secret placeholders on the instance.`,
    userMessage: `AlphaClaw wants Agent Vault to hold the ${config.provider} channel credential(s) for account "${config.accountId}" and inject them into ${config.provider} traffic. The values are entered on the Agent Vault approval page and never stored on the instance.`,
  }));
};
...
const listDeniedChannelPluginIds = () =>
  [...new Set(resolveCatalogChannelIds())]
    .filter((id) => !kChannelProviderVaultServices[id])
    .sort();
