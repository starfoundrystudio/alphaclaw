...
const buildModelProviderAccessRequests = (provider) => {
  ...
  return config.services.map((service) => ({
    ...
    reason: `Broker the ${config.provider} model API key so calls to ${service.host} authenticate at the Agent Vault proxy. AlphaClaw stores only the non-secret placeholder ${config.placeholder} on the instance.`,
    userMessage: `AlphaClaw wants Agent Vault to hold the ${config.provider} model API key and inject it into model API calls to ${service.host}. The key value is entered on the Agent Vault approval page and never stored on the instance.`,
  }));
};
