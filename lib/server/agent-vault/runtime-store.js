...
// Loopback-aware shim in front of the vault tunnel; only the gateway child
// uses it (CLI invocations keep the tunnel port so they work while the
// alphaclaw service, which hosts the shim, is stopped).
const kAgentVaultShimPort = 14323;
...
const buildAgentVaultRuntimeEnv = (
  ...
  const proxyUrl = new URL(
    `http://${kAgentVaultProxyHost}:${viaShim ? kAgentVaultShimPort : kAgentVaultProxyPort}`,
  );
