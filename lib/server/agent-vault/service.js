...
const createAgentVaultService = ({
  ...
  return {
    ...
    getProposal,
    getStatus,
    // The TeamYou entry-hop landing for this instance's vault console,
    // deep-linked to the credentials page (the entry hop allowlists this
    // return path) — the only link that works for owners on managed
    // instances (raw operator URLs dead-end at a login form). Empty when
    // the entry URL is unset.
    getVaultConsoleUrl: () => {
      const entryUrl = getTeamYouEntryUrl();
      if (!entryUrl) return "";
