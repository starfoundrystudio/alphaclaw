...
const createAgentVaultService = ({
  ...
  const reconcileLegacyCredentials = async () => {
    ...
    return {
      removedKeys,
      flippedKeys: placeholderResult.flippedKeys,
      restartRequired:
        removedKeys.length > 0 || placeholderResult.restartRequired,
    };
  };
  ...
  const ensureModelProviderAccess = async (provider) => {
    ...
    // One proposal covering every missing host so the owner approves once.
