...
} = require("../openclaw-known-plugins");
const { normalizeChannelConfig } = require("../agents/shared");

const kPlaceholderScanPattern = /__(?:agent_vault|av)_[a-z0-9_]+__/g;
...
const kManagedChannelCredentialFields = Object.freeze({
  ...
  slack: Object.freeze({
    ...
    // Managed Slack uses Socket Mode. A literal signing secret is still a
    // credential leak, but there is no runtime substitution slot for it.
    signingSecret: null,
    userToken: null,
  }),
  telegram: Object.freeze({
    botToken: "TELEGRAM_BOT_TOKEN",
    // Managed Telegram uses polling. Webhook verification credentials are
    // quarantined instead of being copied to another local secret store.
    webhookSecret: null,
  }),
});

const isEnvironmentReference = (value) =>
  typeof value === "string" && /^\$\{[A-Z][A-Z0-9_]*\}$/.test(value.trim());

const isSupportedSecretRef = (value) =>
  value &&
  typeof value === "object" &&
  !Array.isArray(value) &&
  ["env", "file", "exec"].includes(String(value.source || "").trim());

const isUnsafeManagedChannelCredential = (value) => {
  if (value === undefined || value === null || value === "") return false;
  if (isEnvironmentReference(value) || isVaultPlaceholderValue(value)) return false;
  return !isSupportedSecretRef(value);
};

const scopedChannelEnvKey = (baseKey, accountId) => {
  const normalizedAccountId = String(accountId || "").trim();
  if (!normalizedAccountId || normalizedAccountId === "default") return baseKey;
  return `${baseKey}_${normalizedAccountId
    .toUpperCase()
    .replace(/[^A-Z0-9]+/g, "_")
    .replace(/^_+|_+$/g, "")}`;
};
...
const createAgentVaultService = ({
  ...
  listKnownPluginIds = listKnownOpenclawPluginIds,
}) => {
  let runtimeClaimPromise = null;
  let runtimeRestartPending = false;
  let runtimeEnrollmentReady = false;
  const proposedChannelMigrations = new Map();
  const pendingChannelMigrationPaths = new Map();
  const getEnvVars = () =>
  ...
  // Every vault placeholder this instance references (.env values,
  // openclaw.json, model auth profiles). New credentials get the short
  // `__av_` form; a key whose legacy `__agent_vault_` placeholder is still
  // referenced keeps it, since the vault service substitutes that exact
  // string.
  const collectPlaceholdersInUse = () => {
    const inUse = new Set();
    const add = (value) => {
      const normalized = String(value || "").trim();
      if (isVaultPlaceholderValue(normalized)) inUse.add(normalized);
    };
    for (const entry of getEnvVars()) add(entry?.value);
    try {
      const serialized = JSON.stringify(
        readOpenclawConfig({ openclawDir, fallback: {} }),
      );
      for (const match of serialized.matchAll(kPlaceholderScanPattern)) {
        inUse.add(match[0]);
      }
    } catch {}
  ...
  const reconcileModelProviderPlaceholders = ({ available, serviceHosts }) => {
    ...
    const placeholdersInUse = collectPlaceholdersInUse();
    for (const provider of listVaultBrokeredModelProviders()) {
  ...
  const reconcileChannelPlaceholders = ({ available, serviceHosts }) => {
    ...
    for (const flip of flips) {
      ...
      try {
        ...
        if (serialized.includes(flip.rawValue)) {
          const sanitizedConfig = JSON.parse(
            serialized.split(flip.rawValue).join(`\${${flip.key}}`),
          );
          assertOpenclawConfigSafeForMutation({
            config: sanitizedConfig,
            openclawDir,
            operation: "Agent Vault channel placeholder migration",
          });
          writeOpenclawConfig({ openclawDir, config: sanitizedConfig, spacing: 2 });
        }
      } catch {}
      try {
  ...
  // 2026.9.4 accepts SecretRefs at channel credential positions, so stale
  // literal values can otherwise remain valid indefinitely on upgraded
  // instances. Managed policy is stricter: quarantine literals and `store`
  // refs, remove backup copies containing literal values, and request the
  // matching Agent Vault slots. This is intentionally independent of the
  // normal env migration so a config literal is never copied into `.env`.
  const quarantineManagedChannelConfigCredentials = async ({
    available,
    serviceHosts,
  }) => {
    if (!hasManagedOpenclawConfig()) {
      return {
    ...
    const quarantinedConfigPaths = [];
    const rawValues = new Set();
    const affectedProviders = new Set();
    let changed = false;

    const quarantineContainer = ({ provider, accountId, container, pathPrefix }) => {
      if (!container || typeof container !== "object" || Array.isArray(container)) return;
      const fields = kManagedChannelCredentialFields[provider] || {};
      for (const [field, baseEnvKey] of Object.entries(fields)) {
        const current = container[field];
        if (!isUnsafeManagedChannelCredential(current)) continue;
        if (typeof current === "string" && current.trim()) rawValues.add(current);
        const configPath = `${pathPrefix}.${field}`;
        quarantinedConfigPaths.push(configPath);
        const migrationKey = `${provider}:${accountId}`;
        affectedProviders.add(migrationKey);
        const paths = pendingChannelMigrationPaths.get(migrationKey) || new Set();
        paths.add(configPath);
        pendingChannelMigrationPaths.set(migrationKey, paths);
        if (baseEnvKey) {
          const envKey = scopedChannelEnvKey(baseEnvKey, accountId);
          container[field] = `\${${envKey}}`;
        } else {
    ...
    for (const provider of Object.keys(kManagedChannelCredentialFields)) {
      const providerConfig = channels[provider];
      if (!providerConfig || typeof providerConfig !== "object") continue;
      quarantineContainer({
        provider,
        accountId: "default",
        container: providerConfig,
    ...
    if (changed) {
      assertOpenclawConfigSafeForMutation({
        config,
        openclawDir,
        operation: "managed channel credential quarantine",
      });
      writeOpenclawConfig({ openclawDir, config, spacing: 2 });
      try {
        for (const name of fs.readdirSync(openclawDir)) {
          ...
          try {
            ...
            if ([...rawValues].some((value) => contents.includes(value))) {
              fs.rmSync(backupPath, { force: true });
            }
          } catch {}
        }
    ...
    const migrationKeys = new Set([
      ...affectedProviders,
      ...pendingChannelMigrationPaths.keys(),
    ]);
    for (const providerAccount of migrationKeys) {
      const separator = providerAccount.indexOf(":");
      const provider = providerAccount.slice(0, separator);
      const accountId = providerAccount.slice(separator + 1);
      const vaultConfig = getChannelVaultConfig(provider, accountId);
      if (!vaultConfig) continue;
      const ready =
        vaultConfig.services.every((service) =>
          serviceHosts.has(service.host.toLowerCase()),
        ) &&
        vaultConfig.slots.every((slot) => available.has(slot.envKey));
      if (ready) {
        proposedChannelMigrations.delete(providerAccount);
        pendingChannelMigrationPaths.delete(providerAccount);
        continue;
      }
      const migrationKey = `${provider}:${accountId}`;
      let result = proposedChannelMigrations.get(migrationKey);
      if (!result) {
        try {
          result = await ensureChannelProviderAccess(provider, accountId);
          proposedChannelMigrations.set(migrationKey, result);
        } catch (error) {
          result = {
      ...
      migrationProposals.push({ provider, accountId, ...result });
    }

    const unresolvedPaths = [
      ...new Set(
        [...pendingChannelMigrationPaths.values()].flatMap((paths) => [...paths]),
      ),
    ];

    return {
      quarantinedConfigPaths: unresolvedPaths,
      migrationProposals,
  ...
  const ensureChannelPluginDenyList = () => {
    ...
    try {
      ...
      // Deny only channel plugins OpenClaw actually has (bundled or
      // installed). An uninstalled id cannot be enabled anyway, and on
      // OpenClaw 2026.9 each one printed a "plugin not found" warning on
      // every config-writing command (G3 finding #20, option 2). A channel
      // installed later is denied on the next reconcile.
      const knownPluginIds = listKnownPluginIds({ openclawDir });
      const deniedIds = listDeniedChannelPluginIds().filter((id) =>
        knownPluginIds.has(id),
      );
  ...
  const reconcileLegacyCredentials = async () => {
    ...
    const channelConfigSweep = await quarantineManagedChannelConfigCredentials({
      available,
      serviceHosts,
    });
    ...
    return {
      ...
      ...(channelConfigSweep.quarantinedConfigPaths.length
        ? { quarantinedConfigPaths: channelConfigSweep.quarantinedConfigPaths }
        : {}),
      ...(channelConfigSweep.migrationProposals.length
        ? { migrationProposals: channelConfigSweep.migrationProposals }
        : {}),
      restartRequired:
        removedKeys.length > 0 ||
        placeholderResult.restartRequired ||
        channelResult.restartRequired ||
        channelConfigSweep.restartRequired ||
        denyChanged ||
        discordProxyChanged,
    };
  };
  ...
  // approves a provider exactly once. `buildRequests` may be a function of
  // the /discover payload for requests that depend on what is available.
  const ensureBrokeredAccess = async (buildRequests) => {
    const runtimeClient = getRuntimeClient();
    if (!runtimeClient) {
      throw Object.assign(
    ...
    const discovered = await runtimeClient.client.discover();
    const requests =
      typeof buildRequests === "function"
        ? buildRequests(discovered)
        : buildRequests;
    ...
    const proposal = await runtimeClient.client.createProposal({
      ...
      // With nothing to paste (a service-only update) the short reason is
      // the whole owner-facing message.
      user_message: mergedCredentials.length
        ? accesses[0].userMessage
        : accesses[0].reason,
    });
    return {
  ...
  const ensureModelProviderAccess = async (provider) => {
    ...
    const config = getModelProviderVaultConfig(provider, { placeholdersInUse });
  ...
  const listConfiguredChannelAccountIds = (provider) => {
    let config = {};
    try {
      config = readOpenclawConfig({ openclawDir, fallback: {} });
    } catch {}
    ...
    return Object.keys(normalized.accounts || {});
  };

  const isAppliedProposal = async (proposalId) => {
    const id = Number.parseInt(String(proposalId || ""), 10);
    if (!Number.isSafeInteger(id) || id <= 0) return false;
    try {
      const proposal = await getRuntimeClient()?.client.getProposal(id);
      return String(proposal?.status || "").toLowerCase() === "applied";
    } catch {
      return false;
    }
  };

  // One service per host serves all of a provider's accounts, and a
  // proposed substitution list replaces the stored one (G3 finding #38).
  // Every request therefore carries the substitutions of the provider's
  // other configured accounts whose credentials the vault holds. An account
  // add (`addAccount`) always re-proposes the service because /discover
  // cannot show whether its substitutions are current; the confirming call
  // after approval names the applied proposal and skips that.
  const ensureChannelProviderAccess = async (
    provider,
    accountId = "default",
    { addAccount = false, approvedProposalId = null } = {},
  ) => {
    const placeholdersInUse = collectPlaceholdersInUse();
    ...
    const includeService =
      addAccount && !(approvedProposalId && (await isAppliedProposal(approvedProposalId)))
        ? "always"
        : "missing";
    const configuredAccountIds = listConfiguredChannelAccountIds(provider);
    if (addAccount && configuredAccountIds.includes(config.accountId)) {
      throw Object.assign(
        new Error(
          `Channel account "${provider}/${config.accountId}" already exists. Give the new account its own name first.`,
        ),
        { status: 409 },
      );
    }
    const otherAccountIds = configuredAccountIds.filter(
      (id) => id !== config.accountId,
    );
    const result = await ensureBrokeredAccess((discovered) => {
      const available = new Set(
        (Array.isArray(discovered?.available_credentials)
      ...
      const retainedSlots = otherAccountIds
        .flatMap(
          (id) =>
            getChannelVaultConfig(provider, id, { placeholdersInUse })?.slots ||
            [],
        )
        .filter((slot) => available.has(slot.envKey));
      return buildChannelProviderAccessRequests(provider, config.accountId, {
        placeholdersInUse,
        retainedSlots,
        includeService,
      });
  ...
  const getStatus = async ({ attemptClaim = true } = {}) => {
    ...
    return {
      ...
      quarantinedConfigPaths: migration?.quarantinedConfigPaths || [],
      migrationProposals: migration?.migrationProposals || [],
      ...(migration?.quarantinedConfigPaths?.length
        ? {
            warning:
              "Clawbridge quarantined channel credentials stored directly in OpenClaw configuration. Complete the pending TeamYou Agent Vault request, then reconnect the affected channel if needed.",
          }
        : {}),
  ...
  // Read-only readiness: only the enrollment worker may claim credentials,
  // reconcile configuration, or restart the gateway. A public setup probe
  // must not trigger those mutations itself.
  const isRuntimeReady = async () => {
    if (getConnectivityMode() !== "security_gateway") return true;
    if (!runtimeEnrollmentReady || runtimeClaimPromise || runtimeRestartPending) {
      return false;
    }
    try {
      const runtimeClient = getRuntimeClient();
      if (
        !runtimeClient?.runtime.tokenAcknowledged ||
        !runtimeClient.runtime.handoffComplete
      ) return false;
      await runtimeClient.client.discover();
      return runtimeEnrollmentReady && !runtimeClaimPromise && !runtimeRestartPending;
    } catch {
      return false;
    }
  };

  // Boot and background enrollment share one mutation/restart operation.
  // The first gateway launch must consume the reconciled configuration.
  let enrollmentPromise = null;
  const reconcileRuntime = ({ onRestartRequired = onRuntimeRestartRequired } = {}) => {
    if (getConnectivityMode() !== "security_gateway") return Promise.resolve({ ready: true });
    if (!enrollmentPromise) {
      enrollmentPromise = (async () => {
        runtimeEnrollmentReady = false;
        ...
        if (runtimeRestartPending && typeof onRestartRequired === "function") {
          await onRestartRequired();
          runtimeRestartPending = false;
        }
        runtimeEnrollmentReady = result.ready === true && !runtimeRestartPending;
        return { ready: runtimeEnrollmentReady };
  ...
  const startRuntimeClaimPolling = ({
    ...
    const tick = async () => {
      ...
      running = true;
      try {
        await reconcileRuntime({ onRestartRequired });
      } catch (error) {
