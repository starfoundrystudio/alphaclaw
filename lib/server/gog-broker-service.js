...
const createGogBrokerService = ({
  ...
  const disconnect = ({ account }) =>
    withLifecycleLock(async () => {
      ...
      if (!result.pending) accountHealth.delete(account.id);
      return {
        brokered: true,
  ...
  const validateBrokeredAccountsUnlocked = async () => {
    const accounts = (readState().accounts || []).filter((candidate) =>
      isGogBrokerConsumer(candidate.brokerConsumer),
    );
    const checkedAt = now();
    const results = [];
    for (const account of accounts) {
      if (!brokerClient.isConfigured()) {
        ...
        accountHealth.set(account.id, health);
        results.push({ accountId: account.id, ...health });
        continue;
      }
      try {
        validateLease(
          await brokerClient.getGogAccessToken({
        ...
        const health = {
          checked: true,
          healthy: false,
          reconnectRequired: kReconnectErrorCodes.has(code),
          error: code,
          checkedAt,
        };
        accountHealth.set(account.id, health);
        logger.error(
          `[alphaclaw] Google OAuth grant validation failed for account ${account.id} (${code})`,
        );
        results.push({ accountId: account.id, ...health });
      }
    }
  ...
  const validateBrokeredAccounts = () =>
    withLifecycleLock(validateBrokeredAccountsUnlocked);

  const getAccountHealth = (accountId) =>
    accountHealth.get(String(accountId || "")) || null;
  ...
  const start = () =>
    withLifecycleLock(async () => {
      ...
      started = true;
      if (!brokerClient.isConfigured()) {
        const validation = await validateBrokeredAccountsUnlocked();
        return { started: true, configured: false, validation };
      }
      ...
      const validation = await validateBrokeredAccountsUnlocked();
      return { started: true, configured: true, migrations, validation };
    });
