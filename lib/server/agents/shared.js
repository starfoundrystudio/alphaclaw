...
const setAgentEntries = (cfg = {}, entries = []) => {
  ...
  for (const entry of Array.isArray(entries) ? entries : []) {
    ...
    if (!agentId) continue;
    const { id: _ignoredId, ...storedEntry } = entry;
    canonicalEntries[agentId] = storedEntry;
  }
  cfg.agents = { ...existingAgents, entries: canonicalEntries };
...
// Agent Vault placeholders (e.g. `__av_slack_bot_token__`, or the legacy
// `__agent_vault_slack_bot_token__`) are not
// secrets: the vault proxy swaps in the real value on the wire. Channel config
// on managed instances holds them directly so a newly added channel works
// without a Gateway restart (G3 finding #35); normalization must keep them.
// Same pattern as agent-vault/model-provider-services (not imported: that
// module's dependency chain requires this file).
const kVaultPlaceholderPattern = /^__(?:agent_vault|av)_[a-z0-9_]+__$/;
const isChannelTokenReference = (value) =>
  isEnvRef(value) || kVaultPlaceholderPattern.test(String(value || "").trim());
  ...
  if (Object.keys(existingAccounts).length > 0) {
    if (tokenField || extraTokenFields.length > 0) {
      for (const [accountId, accountConfig] of Object.entries(
        ...
        if (rawTokenFieldValue && !isChannelTokenReference(rawTokenFieldValue)) {
        ...
        extraTokenFields.forEach((fieldName, index) => {
          ...
          if (!rawValue) return;
          if (isChannelTokenReference(rawValue)) return;
...
const readPairedCountsByAccount = ({
  ...
  const approved = OPENCLAW_DIR
    ? readApprovedPairingEntries({ openclawDir: OPENCLAW_DIR })
    : null;
...
// OpenClaw 2026.9 retired `agents.entries.*.default`: it converts a marker
// into per-surface roles (agents.defaults.systemAgent/heartbeat/…, talk,
// channel bindings) and strips it on its next write. Clawbridge therefore
// never writes the marker (re-adding it made every save a second config
// change and a second Gateway reload, see
// docs/openclaw-2026.9.5-plugin-cli-compat-audit.md B3). It still reads a
// legacy marker until OpenClaw converts it.
const resolveDefaultAgentIdFromConfig = (cfg = {}) => {
  const agents = listAgentEntries(cfg);
  const ids = agents.map((entry) => String(entry?.id || "").trim()).filter(Boolean);
  const legacy = agents.find((entry) => entry?.default === true);
  const legacyId = String(legacy?.id || "").trim();
  if (legacyId) return legacyId;
  const systemAgentId = String(cfg?.agents?.defaults?.systemAgent?.agentId || "").trim();
  if (systemAgentId && ids.includes(systemAgentId)) return systemAgentId;
  if (ids.length === 1) return ids[0];
  if (ids.includes(kDefaultAgentId)) return kDefaultAgentId;
  return ids[0] || kDefaultAgentId;
};
...
const withNormalizedAgentsConfig = ({ OPENCLAW_DIR, cfg }) => {
  ...
  const existingList = listAgentEntries(nextCfg);
  ...
  // No default markers are added or forced here (retired in OpenClaw 2026.9;
  // see resolveDefaultAgentIdFromConfig). An existing one is left for
  // OpenClaw to convert.
  nextCfg.agents = { ...existingAgents };
  return setAgentEntries(nextCfg, nextList);
};
