...
const createChannelsDomain = ({
  ...
  const createChannelAccount = async (
    ...
    try {
      ...
      try {
        ...
        if (!addResult?.ok) {
          throw new Error(
            summarizeOpenclawFailure(addResult, { fallback: "Could not add channel account" }),
          );
        }
        ...
        const bindResult = await clawCmdWithConfigConflictRetry(
          ...
          { quiet: true, timeoutMs: kChannelCliTimeoutMs },
          { label: "agents bind" },
        );
        if (!bindResult?.ok) {
          throw new Error(
            summarizeOpenclawFailure(bindResult, { fallback: "Could not bind channel account" }),
          );
        }
  ...
  const createWhatsAppChannelAccount = async ({
    ...
    try {
      ...
      if (!addResult?.ok) {
        throw new Error(
            summarizeOpenclawFailure(addResult, { fallback: "Could not add WhatsApp channel account" }),
          );
      }
