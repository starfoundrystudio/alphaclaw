const { summarizeOpenclawFailure } = require("../openclaw-cli-output");

// Channel add/remove/bind write config and wait for the running Gateway to
// apply it. Measured on OpenClaw 2026.9.5: ~17 s idle and ~29 s right after
// another config write on a fast machine, roughly twice that on a managed
// host (docs/openclaw-2026.9.5-plugin-cli-compat-audit.md). 30 s was hit.
const kChannelCliTimeoutMs = 90 * 1000;

const path = require("path");

const {
  kChannelTokenFields,
  kChannelLabels,
  kMaskedChannelToken,
  shellEscapeArg,
  loadConfig,
  saveConfig,
  ensurePluginAllowed,
  cloneJson,
  normalizeBindingMatch,
  matchesBinding,
  isValidChannelAccountId,
  normalizeChannelProvider,
  deriveChannelEnvKey,
  deriveChannelExtraEnvKeys,
  getConfiguredChannelEnvKeys,
  assertActiveChannelTokenEnvVars,
  normalizeChannelConfig,
  appendBindingToConfig,
  hasLegacyDefaultChannelAccount,
  listConfiguredChannelAccounts,
  withNormalizedAgentsConfig,
  listAgentEntries,
} = require("./shared");
const { parseJsonObjectFromNoisyOutput } = require("../utils/json");
const {
  getAccountConfig,
  toPublicChannelPolicy,
  attachRoomBindings,
  applyChannelPolicy,
} = require("./channel-policy");

const kOpenRoomDefaultsByProvider = {
  telegram: { groups: { "*": { requireMention: true } } },
  discord: { guilds: { "*": { requireMention: true } } },
  slack: { channels: { "*": { requireMention: true } } },
};

const kAllowBotsDefaultsByProvider = {
  discord: "mentions",
  slack: "mentions",
};

const {
  getChannelClassification,
  getChannelVaultConfig,
  getDiscordManagedProxyConfigValue,
  isChannelProviderShelved,
  isPlaceholderForCredentialKey,
  isVaultPlaceholderValue,
} = require("../agent-vault/channel-provider-services");
const {
  hasAgentVaultRuntime: defaultHasVaultRuntime,
} = require("../agent-vault/runtime-store");
const {
  createVaultAwareFetch: defaultCreateVaultProbeFetch,
} = require("../agent-vault/vault-fetch");

const createChannelsDomain = ({
  fsImpl,
  OPENCLAW_DIR,
  readEnvFile,
  writeEnvFile,
  reloadEnv,
  restartGateway,
  reconcileOpenclawPlugins,
  rootDir = path.dirname(OPENCLAW_DIR),
  clawCmd,
  hasVaultRuntime = defaultHasVaultRuntime,
  createVaultProbeFetch = defaultCreateVaultProbeFetch,
  probeChannelToken = null,
}) => {
  let createChannelAccountInProgress = false;

  const reconcileManagedPlugins = async ({ onProgress = () => {}, provider = "" } = {}) => {
    if (typeof reconcileOpenclawPlugins !== "function") return;
    onProgress({ phase: "plugin", label: "Installing channel plugin..." });
    return await reconcileOpenclawPlugins({
      rootDir,
      openclawDir: OPENCLAW_DIR,
      fsModule: fsImpl,
      logger: console,
      env: process.env,
      // Only this channel's plugin: an unrelated plugin's install failure
      // must not abort the channel add (G3 #21).
      ...(provider ? { onlyPluginKeys: [provider] } : {}),
    });
  };

  const getChannelAccountToken = ({
    provider: rawProvider,
    accountId: rawAccountId,
  } = {}) => {
    const provider = normalizeChannelProvider(rawProvider);
    const accountId = String(rawAccountId || "").trim() || "default";
    const cfg = withNormalizedAgentsConfig({
      OPENCLAW_DIR,
      cfg: loadConfig({ fsImpl, OPENCLAW_DIR }),
    });
    const providerConfig =
      cfg.channels?.[provider] && typeof cfg.channels[provider] === "object"
        ? cfg.channels[provider]
        : null;
    if (!providerConfig) {
      throw new Error(`Channel "${provider}" not found`);
    }
    const hasAccounts =
      providerConfig.accounts && typeof providerConfig.accounts === "object";
    const hasLegacyDefault =
      accountId === "default" &&
      !hasAccounts &&
      hasLegacyDefaultChannelAccount({ config: providerConfig });
    if (!hasLegacyDefault && !providerConfig.accounts?.[accountId]) {
      throw new Error(`Channel account "${provider}/${accountId}" not found`);
    }
    const envKey = deriveChannelEnvKey({ provider, accountId });
    const extraEnvKeys = deriveChannelExtraEnvKeys({ provider, accountId });
    const envVars = readEnvFile();
    const envEntry = (Array.isArray(envVars) ? envVars : []).find(
      (entry) => String(entry?.key || "").trim() === envKey,
    );
    const appEnvKey = extraEnvKeys[0] || "";
    const appEnvEntry = appEnvKey
      ? (Array.isArray(envVars) ? envVars : []).find(
          (entry) => String(entry?.key || "").trim() === appEnvKey,
        )
      : null;
    return {
      provider,
      accountId,
      envKey,
      token: String(envEntry?.value || ""),
      ...(provider === "slack"
        ? {
            appEnvKey,
            appToken: String(appEnvEntry?.value || ""),
          }
        : {}),
    };
  };

  const createChannelAccount = async (
    input = {},
    { onProgress = () => {} } = {},
  ) => {
    if (createChannelAccountInProgress) {
      throw new Error("A channel account creation is already in progress");
    }
    createChannelAccountInProgress = true;
    try {
      const provider = normalizeChannelProvider(input.provider);
      const name =
        String(input.name || "").trim() || kChannelLabels[provider] || provider;

      const cfg = withNormalizedAgentsConfig({
        OPENCLAW_DIR,
        cfg: loadConfig({ fsImpl, OPENCLAW_DIR }),
      });

      const agentId = String(input.agentId || "").trim();
      const agent = listAgentEntries(cfg).find((entry) => entry.id === agentId);
      if (!agent) throw new Error(`Agent "${agentId}" not found`);

      const existingChannelConfig =
        cfg.channels?.[provider] && typeof cfg.channels[provider] === "object"
          ? cfg.channels[provider]
          : {};
      const normalizedChannelConfig = normalizeChannelConfig({
        provider,
        channelConfig: existingChannelConfig,
      });
      const existingAccounts =
        normalizedChannelConfig.accounts &&
        typeof normalizedChannelConfig.accounts === "object"
          ? normalizedChannelConfig.accounts
          : {};
      const requestedAccountId = String(input.accountId || "").trim();
      const accountId =
        requestedAccountId ||
        (Object.keys(existingAccounts).length > 0 ? "" : "default");
      if (!accountId) {
        throw new Error("Channel account id is required");
      }
      if (!isValidChannelAccountId(accountId)) {
        throw new Error(
          "Channel account id must be lowercase letters, numbers, and hyphens only",
        );
      }
      if (existingAccounts[accountId]) {
        throw new Error(
          `Channel account "${provider}/${accountId}" already exists`,
        );
      }
      if (
        (provider === "discord" || provider === "whatsapp") &&
        Object.keys(existingAccounts).length > 0
      ) {
        throw new Error(
          `${kChannelLabels[provider] || "This provider"} supports a single channel account`,
        );
      }

      // Managed-instance classification gate (spec §5, decisions D2/D6):
      // a channel without a classification entry cannot be enabled while the
      // Agent Vault runtime is claimed. Tier C channels (whatsapp) keep
      // their existing flow under custody hygiene.
      const managedVault = hasVaultRuntime();
      const classification = getChannelClassification(provider);
      if (managedVault && !classification) {
        throw new Error(
          `Channel "${provider}" is not classified for Agent Vault-managed instances and cannot be enabled. See docs/vault-brokered-channels-spec.md.`,
        );
      }
      // Shelved channels are classified but cannot function behind the Agent
      // Vault proxy (WhatsApp's Noise handshake breaks under MITM). Reject
      // cleanly on managed instances rather than half-configuring a channel
      // the deny-list will refuse to activate.
      if (managedVault && isChannelProviderShelved(provider)) {
        throw new Error(
          `Channel "${provider}" is not available on Agent Vault-managed instances: it cannot complete linking behind the security proxy. See docs/vault-brokered-channels-spec.md.`,
        );
      }

      if (provider === "whatsapp") {
        return await createWhatsAppChannelAccount({
          input,
          cfg,
          agentId,
          accountId,
          name,
          normalizedChannelConfig,
          existingAccounts,
          onProgress,
        });
      }

      const token = String(input.token || "").trim();
      if (!token) throw new Error("Channel token is required");
      let discordApplicationId = String(
        input.discordApplicationId || "",
      ).trim();
      if (
        provider === "discord" &&
        discordApplicationId &&
        !/^\d+$/.test(discordApplicationId)
      ) {
        throw new Error("Discord application id must contain only numbers");
      }

      const envKey = deriveChannelEnvKey({ provider, accountId });
      const extraEnvKeys = deriveChannelExtraEnvKeys({ provider, accountId });
      const appToken = String(input.appToken || "").trim();
      if (provider === "slack" && !appToken) {
        throw new Error("Slack App Token is required");
      }

      // Vault-brokered channels (tier S) on managed instances accept only
      // this account's placeholders — raw tokens are rejected so the vault
      // flow is the single door (D2). The post-approval probe runs the same
      // inspection the wizard used to run on raw tokens, but through the
      // vault proxy so substitution applies; failure blocks the flow (D5).
      const vaultConfig =
        managedVault && classification?.tier === "S"
          ? getChannelVaultConfig(provider, accountId)
          : null;
      if (vaultConfig) {
        // Slots are keyed by env name; their array order is approval-page
        // presentation only (Slack lists the app token first).
        const appSlot =
          vaultConfig.slots.find((slot) =>
            /_APP_TOKEN/.test(String(slot?.envKey || "")),
          ) || null;
        const tokenSlot = vaultConfig.slots.find((slot) => slot !== appSlot);
        const tokenIsPlaceholder = isVaultPlaceholderValue(token);
        const appTokenIsPlaceholder = appSlot
          ? isVaultPlaceholderValue(appToken)
          : true;
        if (!tokenIsPlaceholder || !appTokenIsPlaceholder) {
          throw new Error(
            `${kChannelLabels[provider] || provider} tokens are managed by Agent Vault on this instance. Use "Store token in Agent Vault" in channel settings instead of entering the token here.`,
          );
        }
        // Either placeholder form: a re-added account keeps the legacy
        // `__agent_vault_` string its vault substitution still names.
        if (
          !isPlaceholderForCredentialKey(token, tokenSlot.envKey) ||
          (appSlot && !isPlaceholderForCredentialKey(appToken, appSlot.envKey))
        ) {
          throw new Error(
            `Agent Vault placeholder does not match channel account "${provider}/${accountId}". Request vault access for this account first.`,
          );
        }
        const vaultFetch = createVaultProbeFetch();
        if (!vaultFetch) {
          throw new Error(
            "Agent Vault runtime is unavailable for the post-approval credential check",
          );
        }
        if (typeof probeChannelToken === "function") {
          onProgress({
            phase: "verifying",
            label: "Verifying vault credential...",
          });
          const probe = await probeChannelToken({
            provider,
            token,
            appToken,
            fetchImpl: vaultFetch,
          });
          if (provider === "discord" && !discordApplicationId) {
            discordApplicationId = String(probe?.applicationId || "").trim();
          }
        }
      }
      const tokenField = kChannelTokenFields[provider];
      const currentEnvVars = readEnvFile();
      const previousEnvVars = Array.isArray(currentEnvVars)
        ? currentEnvVars
        : [];
      const duplicateEnvEntry = previousEnvVars.find((entry) => {
        const existingKey = String(entry?.key || "").trim();
        const existingValue = String(entry?.value || "").trim();
        if (!existingKey || !existingValue) return false;
        if (existingKey === envKey) return false;
        return existingValue === token;
      });
      let orphanedEnvKey = null;
      if (duplicateEnvEntry) {
        const dupKey = String(duplicateEnvEntry.key || "").trim();
        const configuredKeys = getConfiguredChannelEnvKeys(cfg);
        if (configuredKeys.has(dupKey)) {
          throw new Error(`Channel token already exists in ${dupKey}`);
        }
        orphanedEnvKey = dupKey;
        console.log(
          `[alphaclaw] Overwriting orphaned channel env var ${dupKey} (no matching config entry)`,
        );
      }
      let orphanedExtraEnvKey = null;
      if (provider === "slack") {
        const appEnvKey = extraEnvKeys[0];
        const duplicateAppTokenEntry = previousEnvVars.find((entry) => {
          const existingKey = String(entry?.key || "").trim();
          const existingValue = String(entry?.value || "").trim();
          if (!existingKey || !existingValue) return false;
          if (existingKey === envKey || existingKey === appEnvKey) return false;
          return existingValue === appToken;
        });
        if (duplicateAppTokenEntry) {
          const dupKey = String(duplicateAppTokenEntry.key || "").trim();
          const configuredKeys = getConfiguredChannelEnvKeys(cfg);
          if (configuredKeys.has(dupKey)) {
            throw new Error(`Channel token already exists in ${dupKey}`);
          }
          orphanedExtraEnvKey = dupKey;
          console.log(
            `[alphaclaw] Overwriting orphaned channel env var ${dupKey} (no matching config entry)`,
          );
        }
      }
      const nextEnvVars = previousEnvVars.filter((entry) => {
        const key = String(entry?.key || "").trim();
        return (
          key !== envKey &&
          key !== orphanedEnvKey &&
          !extraEnvKeys.includes(key) &&
          key !== orphanedExtraEnvKey
        );
      });
      nextEnvVars.push({ key: envKey, value: token });
      if (provider === "slack" && extraEnvKeys[0]) {
        nextEnvVars.push({ key: extraEnvKeys[0], value: appToken });
      }

      // Same shape as OpenClaw's own channel setup wizard (G3 finding #35):
      // install the plugin first without enabling its entry (enabling first
      // made OpenClaw 2026.9 install it itself and collide with this install,
      // #34), then write the plugin entry, channel account and agent binding
      // in ONE config write, so the running Gateway hot-reloads once instead
      // of four times. On Agent Vault instances the channel config holds the
      // vault placeholders themselves, so no Gateway restart is needed to
      // load new token env vars; other instances keep env references and one
      // restart.
      const tokensInConfig = !!vaultConfig;
      const previousConfig = cloneJson(cfg);
      let rollbackConfig = previousConfig;
      try {
        onProgress({ phase: "configuring", label: "Configuring..." });
        writeEnvFile(nextEnvVars);
        reloadEnv();
        assertActiveChannelTokenEnvVars({
          cfg: withNormalizedAgentsConfig({
            OPENCLAW_DIR,
            cfg: loadConfig({ fsImpl, OPENCLAW_DIR }),
          }),
          envVars: nextEnvVars,
        });
        const reconcileResult = await reconcileManagedPlugins({ onProgress, provider });
        // A version-pinned hotfix written during this install (Slack on
        // 2026.9.5) is loaded lazily when the channel starts, but restart once
        // anyway in case the Gateway already imported the unpatched module.
        const hotfixLandedNow =
          reconcileResult?.hotfixesApplied === true ||
          (Array.isArray(reconcileResult?.hotfixes) &&
            reconcileResult.hotfixes.some((hotfix) => hotfix?.status === "applied"));
        // The install wrote its own record and enabled the entry; a failure
        // after this point must not undo that (a missing install record next
        // to an installed package makes the next attempt collide again).
        const nextCfg = withNormalizedAgentsConfig({
          OPENCLAW_DIR,
          cfg: loadConfig({ fsImpl, OPENCLAW_DIR }),
        });
        rollbackConfig = cloneJson(nextCfg);
        ensurePluginAllowed({ cfg: nextCfg, pluginKey: provider });
        const nextProviderConfig = normalizeChannelConfig({
          provider,
          channelConfig:
            nextCfg.channels?.[provider] &&
            typeof nextCfg.channels[provider] === "object"
              ? nextCfg.channels[provider]
              : {},
        });
        const nextAccounts =
          nextProviderConfig.accounts &&
          typeof nextProviderConfig.accounts === "object"
            ? { ...nextProviderConfig.accounts }
            : {};
        const existingNextAccount =
          nextAccounts[accountId] && typeof nextAccounts[accountId] === "object"
            ? nextAccounts[accountId]
            : {};
        const openRoomDefaults = kOpenRoomDefaultsByProvider[provider] || null;
        const hasInheritedGroupPolicy = !!String(
          existingNextAccount.groupPolicy || nextProviderConfig.groupPolicy || "",
        ).trim();
        const roomConfigKey = openRoomDefaults
          ? Object.keys(openRoomDefaults)[0]
          : "";
        const hasInheritedRoomConfig = !!(
          roomConfigKey &&
          (existingNextAccount[roomConfigKey] || nextProviderConfig[roomConfigKey])
        );
        const allowBotsDefault = kAllowBotsDefaultsByProvider[provider];
        const hasInheritedAllowBots =
          existingNextAccount.allowBots !== undefined ||
          nextProviderConfig.allowBots !== undefined;
        nextAccounts[accountId] = {
          ...existingNextAccount,
          ...(name ? { name } : {}),
          [tokenField]: tokensInConfig ? token : `\${${envKey}}`,
          ...(provider === "slack" && extraEnvKeys[0]
            ? { appToken: tokensInConfig ? appToken : `\${${extraEnvKeys[0]}}` }
            : {}),
          ...(provider === "discord" && discordApplicationId
            ? { applicationId: discordApplicationId }
            : {}),
          ...(provider === "discord" && input.discordGuildMembersIntent === true
            ? { intents: { guildMembers: true } }
            : {}),
          ...(openRoomDefaults && !hasInheritedGroupPolicy
            ? { groupPolicy: "open" }
            : {}),
          ...(openRoomDefaults && !hasInheritedRoomConfig
            ? openRoomDefaults
            : {}),
          ...(allowBotsDefault && !hasInheritedAllowBots
            ? { allowBots: allowBotsDefault }
            : {}),
          dmPolicy: "pairing",
        };
        nextProviderConfig.accounts = nextAccounts;
        nextProviderConfig.enabled = true;
        if (provider === "discord" && managedVault) {
          // Route the plugin's REST + gateway websocket through the vault
          // proxy. Env reference, never a literal URL — and only written
          // while the vault runtime exists (empty expansion leaves the
          // literal `${...}` string; Phase A item 4).
          nextProviderConfig.proxy = getDiscordManagedProxyConfigValue();
        }
        if (!String(nextProviderConfig.defaultAccount || "").trim()) {
          nextProviderConfig.defaultAccount = "default";
        }
        nextCfg.channels =
          nextCfg.channels && typeof nextCfg.channels === "object"
            ? { ...nextCfg.channels }
            : {};
        nextCfg.channels[provider] = nextProviderConfig;
        appendBindingToConfig({
          cfg: nextCfg,
          agentId,
          match: normalizeBindingMatch({ channel: provider, accountId }),
        });
        onProgress({ phase: "binding", label: "Binding agent..." });
        saveConfig({ fsImpl, OPENCLAW_DIR, config: nextCfg });
        if (!tokensInConfig || hotfixLandedNow) {
          onProgress({ phase: "restarting", label: "Rebooting..." });
          await restartGateway();
        }
      } catch (error) {
        try {
          writeEnvFile(previousEnvVars);
          reloadEnv();
        } catch {}
        try {
          saveConfig({ fsImpl, OPENCLAW_DIR, config: rollbackConfig });
        } catch {}
        throw error;
      }

      const binding = {
        agentId,
        match: normalizeBindingMatch({
          channel: provider,
          accountId,
        }),
      };
      return {
        channel: provider,
        account: {
          id: accountId,
          name,
          envKey,
        },
        binding,
      };
    } finally {
      createChannelAccountInProgress = false;
    }
  };

  const updateChannelAccount = (input = {}) => {
    const provider = normalizeChannelProvider(input.provider);
    const accountId = String(input.accountId || "").trim() || "default";
    const nextName = String(input.name || "").trim();
    const nextAgentId = String(input.agentId || "").trim();
    const nextToken = String(input.token || "").trim();
    const nextAppToken = String(input.appToken || "").trim();
    if (!nextName) throw new Error("Channel name is required");
    if (!nextAgentId) throw new Error("Agent is required");
    if (
      hasVaultRuntime() &&
      getChannelClassification(provider)?.tier === "S" &&
      ((nextToken && !isVaultPlaceholderValue(nextToken)) ||
        (nextAppToken && !isVaultPlaceholderValue(nextAppToken)))
    ) {
      throw new Error(
        `${kChannelLabels[provider] || provider} tokens are managed by Agent Vault on this instance. Rotate the credential from the Agent Vault console instead of entering a token here.`,
      );
    }

    const cfg = withNormalizedAgentsConfig({
      OPENCLAW_DIR,
      cfg: loadConfig({ fsImpl, OPENCLAW_DIR }),
    });
    const agent = listAgentEntries(cfg).find(
      (entry) => entry.id === nextAgentId,
    );
    if (!agent) throw new Error(`Agent "${nextAgentId}" not found`);

    const providerConfig =
      cfg.channels?.[provider] && typeof cfg.channels[provider] === "object"
        ? { ...cfg.channels[provider] }
        : null;
    if (!providerConfig) {
      throw new Error(`Channel "${provider}" not found`);
    }

    const hasAccounts =
      providerConfig.accounts && typeof providerConfig.accounts === "object";
    const hasLegacyDefault =
      accountId === "default" &&
      !hasAccounts &&
      hasLegacyDefaultChannelAccount({ config: providerConfig });
    if (!hasLegacyDefault && !providerConfig.accounts?.[accountId]) {
      throw new Error(`Channel account "${provider}/${accountId}" not found`);
    }

    let tokenUpdated = false;
    if (provider === "slack" && nextAppToken) {
      const appEnvKey = deriveChannelExtraEnvKeys({ provider, accountId })[0];
      const currentEnvVars = readEnvFile();
      const previousEnvVars = Array.isArray(currentEnvVars)
        ? currentEnvVars
        : [];
      const existingAppToken = String(
        previousEnvVars.find(
          (entry) => String(entry?.key || "").trim() === appEnvKey,
        )?.value || "",
      );
      const duplicateEnvEntry = previousEnvVars.find((entry) => {
        const existingKey = String(entry?.key || "").trim();
        const existingValue = String(entry?.value || "").trim();
        if (!existingKey || !existingValue) return false;
        if (existingKey === appEnvKey) return false;
        return existingValue === nextAppToken;
      });
      if (duplicateEnvEntry) {
        const dupKey = String(duplicateEnvEntry.key || "").trim();
        const configuredKeys = getConfiguredChannelEnvKeys(cfg);
        if (configuredKeys.has(dupKey)) {
          throw new Error(`Channel token already exists in ${dupKey}`);
        }
      }
      if (existingAppToken !== nextAppToken) {
        const nextEnvVars = previousEnvVars.filter(
          (entry) => String(entry?.key || "").trim() !== appEnvKey,
        );
        nextEnvVars.push({ key: appEnvKey, value: nextAppToken });
        writeEnvFile(nextEnvVars);
        reloadEnv();
        tokenUpdated = true;
      }
    }
    if (nextToken) {
      const envKey = deriveChannelEnvKey({ provider, accountId });
      const currentEnvVars = readEnvFile();
      const previousEnvVars = Array.isArray(currentEnvVars)
        ? currentEnvVars
        : [];
      const existingToken = String(
        previousEnvVars.find(
          (entry) => String(entry?.key || "").trim() === envKey,
        )?.value || "",
      );
      const duplicateEnvEntry = previousEnvVars.find((entry) => {
        const existingKey = String(entry?.key || "").trim();
        const existingValue = String(entry?.value || "").trim();
        if (!existingKey || !existingValue) return false;
        if (existingKey === envKey) return false;
        return existingValue === nextToken;
      });
      if (duplicateEnvEntry) {
        const dupKey = String(duplicateEnvEntry.key || "").trim();
        const configuredKeys = getConfiguredChannelEnvKeys(cfg);
        if (configuredKeys.has(dupKey)) {
          throw new Error(`Channel token already exists in ${dupKey}`);
        }
      }
      if (existingToken !== nextToken) {
        const nextEnvVars = previousEnvVars.filter(
          (entry) => String(entry?.key || "").trim() !== envKey,
        );
        nextEnvVars.push({ key: envKey, value: nextToken });
        writeEnvFile(nextEnvVars);
        reloadEnv();
        tokenUpdated = true;
      }
    }

    if (hasLegacyDefault) {
      providerConfig.name = nextName;
    } else {
      providerConfig.accounts = { ...providerConfig.accounts };
      providerConfig.accounts[accountId] = {
        ...(providerConfig.accounts[accountId] || {}),
        name: nextName,
      };
    }
    cfg.channels =
      cfg.channels && typeof cfg.channels === "object"
        ? { ...cfg.channels }
        : {};
    cfg.channels[provider] = providerConfig;

    const bindings = Array.isArray(cfg.bindings) ? cfg.bindings : [];
    const targetMatch = normalizeBindingMatch({ channel: provider, accountId });
    const nextBindings = bindings.filter((binding) => {
      const match = binding?.match || {};
      const hasScopedFields =
        !!match.peer ||
        !!match.parentPeer ||
        !!String(match.guildId || "").trim() ||
        !!String(match.teamId || "").trim() ||
        (Array.isArray(match.roles) && match.roles.length > 0);
      if (hasScopedFields) return true;
      return !matchesBinding(match, targetMatch);
    });
    cfg.bindings = nextBindings;
    appendBindingToConfig({
      cfg,
      agentId: nextAgentId,
      match: targetMatch,
    });
    saveConfig({ fsImpl, OPENCLAW_DIR, config: cfg });

    return {
      channel: provider,
      account: {
        id: accountId,
        name: nextName,
        boundAgentId: nextAgentId,
      },
      tokenUpdated,
    };
  };

  const updateChannelAccountPolicy = (input = {}) => {
    const provider = normalizeChannelProvider(input.provider);
    if (provider === "whatsapp") {
      throw new Error("WhatsApp access policy is fixed to the managed owner allowlist");
    }
    const accountId = String(input.accountId || "").trim() || "default";
    const cfg = withNormalizedAgentsConfig({
      OPENCLAW_DIR,
      cfg: loadConfig({ fsImpl, OPENCLAW_DIR }),
    });
    const providerConfig =
      cfg.channels?.[provider] && typeof cfg.channels[provider] === "object"
        ? cloneJson(cfg.channels[provider])
        : null;
    if (!providerConfig) throw new Error(`Channel "${provider}" not found`);
    const currentAccount = getAccountConfig({ providerConfig, accountId });
    if (!currentAccount) {
      throw new Error(`Channel account "${provider}/${accountId}" not found`);
    }
    const nextAccount = applyChannelPolicy({
      provider,
      accountConfig: currentAccount,
      policy: input.policy || {},
    });
    const existingBindings = Array.isArray(cfg.bindings) ? cfg.bindings : [];
    cfg.bindings = existingBindings.filter((binding) => {
      const match = binding?.match || {};
      const matchAccountId = String(match.accountId || "").trim() || "default";
      return !(
        String(match.channel || "").trim() === provider &&
        matchAccountId === accountId &&
        match.peer &&
        ["group", "channel"].includes(String(match.peer.kind || "").trim())
      );
    });
    const validAgentIds = new Set(listAgentEntries(cfg).map((entry) => entry.id));
    for (const room of Array.isArray(input.policy?.rooms) ? input.policy.rooms : []) {
      const routeAgentId = String(room?.routeAgentId || "").trim();
      const roomId = String(room?.id || "").trim();
      if (!routeAgentId || !roomId) continue;
      if (!validAgentIds.has(routeAgentId)) {
        throw new Error(`Agent "${routeAgentId}" not found`);
      }
      appendBindingToConfig({
        cfg,
        agentId: routeAgentId,
        match: normalizeBindingMatch({
          channel: provider,
          accountId,
          ...(room?.guildId ? { guildId: String(room.guildId).trim() } : {}),
          peer: {
            kind: provider === "telegram" ? "group" : "channel",
            id: roomId,
          },
        }),
      });
    }
    if (providerConfig.accounts && typeof providerConfig.accounts === "object") {
      providerConfig.accounts = { ...providerConfig.accounts, [accountId]: nextAccount };
    } else {
      for (const key of Object.keys(providerConfig)) {
        if (!["enabled", "accounts", "defaultAccount"].includes(key)) {
          delete providerConfig[key];
        }
      }
      Object.assign(providerConfig, nextAccount);
    }
    cfg.channels = { ...(cfg.channels || {}), [provider]: providerConfig };
    saveConfig({ fsImpl, OPENCLAW_DIR, config: cfg });
    return {
      channel: provider,
      accountId,
      policy: toPublicChannelPolicy({ provider, accountConfig: nextAccount }),
    };
  };

  const deleteChannelAccount = async (input = {}) => {
    const provider = normalizeChannelProvider(input.provider);
    const accountId = String(input.accountId || "").trim() || "default";

    const cfg = withNormalizedAgentsConfig({
      OPENCLAW_DIR,
      cfg: loadConfig({ fsImpl, OPENCLAW_DIR }),
    });
    const providerConfig =
      cfg.channels?.[provider] && typeof cfg.channels[provider] === "object"
        ? cfg.channels[provider]
        : null;
    if (!providerConfig) {
      throw new Error(`Channel "${provider}" not found`);
    }
    const hasAccounts =
      providerConfig.accounts && typeof providerConfig.accounts === "object";
    const hasLegacyDefault =
      accountId === "default" &&
      !hasAccounts &&
      hasLegacyDefaultChannelAccount({ config: providerConfig });
    if (!hasLegacyDefault && !providerConfig.accounts?.[accountId]) {
      throw new Error(`Channel account "${provider}/${accountId}" not found`);
    }

    // One config write (G3 finding #35): the running Gateway hot-reloads the
    // removed channel. Only WhatsApp still runs `openclaw channels remove
    // --delete`, which also clears its linked-device credentials.
    if (provider !== "whatsapp") {
      const nextCfg = withNormalizedAgentsConfig({
        OPENCLAW_DIR,
        cfg: loadConfig({ fsImpl, OPENCLAW_DIR }),
      });
      const nextChannels =
        nextCfg.channels && typeof nextCfg.channels === "object"
          ? { ...nextCfg.channels }
          : {};
      const nextProviderConfig = normalizeChannelConfig({
        provider,
        channelConfig:
          nextChannels[provider] && typeof nextChannels[provider] === "object"
            ? nextChannels[provider]
            : {},
      });
      const nextAccounts =
        nextProviderConfig.accounts &&
        typeof nextProviderConfig.accounts === "object"
          ? { ...nextProviderConfig.accounts }
          : {};
      delete nextAccounts[accountId];
      if (Object.keys(nextAccounts).length > 0) {
        nextProviderConfig.accounts = nextAccounts;
        nextChannels[provider] = nextProviderConfig;
      } else {
        delete nextChannels[provider];
      }
      nextCfg.channels = nextChannels;

      const targetMatch = normalizeBindingMatch({
        channel: provider,
        accountId,
      });
      const existingBindings = Array.isArray(nextCfg.bindings)
        ? nextCfg.bindings
        : [];
      nextCfg.bindings = existingBindings.filter((binding) => {
        const match = binding?.match || {};
        const hasScopedFields =
          !!match.peer ||
          !!match.parentPeer ||
          !!String(match.guildId || "").trim() ||
          !!String(match.teamId || "").trim() ||
          (Array.isArray(match.roles) && match.roles.length > 0);
        if (hasScopedFields) return true;
        return !matchesBinding(match, targetMatch);
      });
      if (!nextChannels[provider] && nextCfg.plugins?.entries?.[provider]) {
        nextCfg.plugins.entries[provider] = {
          ...(nextCfg.plugins.entries[provider] || {}),
          enabled: false,
        };
      }
      saveConfig({ fsImpl, OPENCLAW_DIR, config: nextCfg });

      const envKey = deriveChannelEnvKey({ provider, accountId });
      const extraEnvKeys = deriveChannelExtraEnvKeys({ provider, accountId });
      const currentEnvVars = readEnvFile();
      const previousEnvVars = Array.isArray(currentEnvVars)
        ? currentEnvVars
        : [];
      const nextEnvVars = previousEnvVars.filter(
        (entry) =>
          String(entry?.key || "").trim() !== envKey &&
          !extraEnvKeys.includes(String(entry?.key || "").trim()),
      );
      if (nextEnvVars.length !== previousEnvVars.length) {
        writeEnvFile(nextEnvVars);
        reloadEnv();
      }

      // Discord's gateway websocket runs through the vault proxy; it has
      // always been restarted on removal, so that is kept.
      if (provider === "discord") {
        await restartGateway();
      }
      return { ok: true };
    }

    const removeArgs = [
      "channels remove",
      `--channel ${shellEscapeArg(provider)}`,
      `--account ${shellEscapeArg(accountId)}`,
      "--delete",
    ].filter(Boolean);
    const removeResult = await clawCmd(removeArgs.join(" "), {
      quiet: true,
      timeoutMs: kChannelCliTimeoutMs,
      allowConfigMutation: true,
    });
    if (!removeResult?.ok) {
      throw new Error(
            summarizeOpenclawFailure(removeResult, { fallback: "Could not delete channel account" }),
          );
    }

    const envKey = deriveChannelEnvKey({ provider, accountId });
    const extraEnvKeys = deriveChannelExtraEnvKeys({ provider, accountId });
    const currentEnvVars = readEnvFile();
    const previousEnvVars = Array.isArray(currentEnvVars) ? currentEnvVars : [];
    const nextEnvVars = previousEnvVars.filter(
      (entry) =>
        String(entry?.key || "").trim() !== envKey &&
        !extraEnvKeys.includes(String(entry?.key || "").trim()),
    );
    if (nextEnvVars.length !== previousEnvVars.length) {
      writeEnvFile(nextEnvVars);
      reloadEnv();
    }

    const nextCfg = withNormalizedAgentsConfig({
      OPENCLAW_DIR,
      cfg: loadConfig({ fsImpl, OPENCLAW_DIR }),
    });
    const nextChannels =
      nextCfg.channels && typeof nextCfg.channels === "object"
        ? { ...nextCfg.channels }
        : {};
    const nextProviderConfig = normalizeChannelConfig({
      provider,
      channelConfig:
        nextChannels[provider] && typeof nextChannels[provider] === "object"
          ? nextChannels[provider]
          : {},
    });
    const nextAccounts =
      nextProviderConfig.accounts &&
      typeof nextProviderConfig.accounts === "object"
        ? { ...nextProviderConfig.accounts }
        : {};
    delete nextAccounts[accountId];
    if (Object.keys(nextAccounts).length > 0) {
      nextProviderConfig.accounts = nextAccounts;
      nextChannels[provider] = nextProviderConfig;
    } else {
      delete nextChannels[provider];
    }
    nextCfg.channels = nextChannels;
    const targetMatch = normalizeBindingMatch({ channel: provider, accountId });
    const existingBindings = Array.isArray(nextCfg.bindings)
      ? nextCfg.bindings
      : [];
    nextCfg.bindings = existingBindings.filter((binding) => {
      const match = binding?.match || {};
      const hasScopedFields =
        !!match.peer ||
        !!match.parentPeer ||
        !!String(match.guildId || "").trim() ||
        !!String(match.teamId || "").trim() ||
        (Array.isArray(match.roles) && match.roles.length > 0);
      if (hasScopedFields) return true;
      return !matchesBinding(match, targetMatch);
    });
    if (!nextChannels[provider] && nextCfg.plugins?.entries?.[provider]) {
      nextCfg.plugins.entries[provider] = {
        ...(nextCfg.plugins.entries[provider] || {}),
        enabled: false,
      };
    }
    saveConfig({ fsImpl, OPENCLAW_DIR, config: nextCfg });

    if (provider === "whatsapp") {
      await restartGateway();
    }
    return { ok: true };
  };

  const listConfiguredChannelAccountsWithMaskedTokens = ({ includePolicy = false } = {}) => {
    const cfg = withNormalizedAgentsConfig({
      OPENCLAW_DIR,
      cfg: loadConfig({ fsImpl, OPENCLAW_DIR }),
    });
    const channels = listConfiguredChannelAccounts({
      fsImpl,
      OPENCLAW_DIR,
      cfg,
    });
    const envVars = readEnvFile();
    const envKeySet = new Set(
      (Array.isArray(envVars) ? envVars : [])
        .filter((v) => v?.key && String(v?.value || "").trim())
        .map((v) => String(v.key).trim()),
    );
    return channels.map((entry) => ({
      ...entry,
      accounts: entry.accounts.map((account) => ({
        ...account,
        ...(includePolicy
          ? {
              policy: attachRoomBindings({
                cfg,
                provider: entry.channel,
                accountId: account.id,
                policy: toPublicChannelPolicy({
                  provider: entry.channel,
                  accountConfig:
                    getAccountConfig({
                      providerConfig: cfg.channels?.[entry.channel] || {},
                      accountId: account.id,
                    }) || {},
                }),
              }),
            }
          : {}),
        token: envKeySet.has(String(account.envKey || "").trim())
          ? kMaskedChannelToken
          : "",
      })),
    }));
  };

  const createWhatsAppChannelAccount = async ({
    input,
    cfg,
    agentId,
    accountId,
    name,
    normalizedChannelConfig,
    existingAccounts,
    onProgress,
  }) => {
    const ownerNumber = String(input.token || "").trim();
    if (!ownerNumber) throw new Error("WhatsApp owner number is required");

    const envKey = deriveChannelEnvKey({ provider: "whatsapp", accountId });
    const currentEnvVars = readEnvFile();
    const previousEnvVars = Array.isArray(currentEnvVars) ? currentEnvVars : [];
    const previousConfig = cloneJson(cfg);

    const nextEnvVars = previousEnvVars.filter(
      (entry) => String(entry?.key || "").trim() !== envKey,
    );
    nextEnvVars.push({ key: envKey, value: ownerNumber });

    // Install first, then one config write (G3 findings #34/#35), as for
    // the other channels. WhatsApp keeps its restart: its allowlists are env
    // references to the owner number.
    let rollbackConfig = previousConfig;
    try {
      onProgress({ phase: "configuring", label: "Configuring..." });
      writeEnvFile(nextEnvVars);
      reloadEnv();
      await reconcileManagedPlugins({ onProgress, provider: "whatsapp" });

      const refreshedCfg = withNormalizedAgentsConfig({
        OPENCLAW_DIR,
        cfg: loadConfig({ fsImpl, OPENCLAW_DIR }),
      });
      rollbackConfig = cloneJson(refreshedCfg);
      ensurePluginAllowed({ cfg: refreshedCfg, pluginKey: "whatsapp" });

      const nextAccounts = { ...existingAccounts };
      nextAccounts[accountId] = {
        ...(nextAccounts[accountId] &&
        typeof nextAccounts[accountId] === "object"
          ? nextAccounts[accountId]
          : {}),
        ...(name ? { name } : {}),
        allowFrom: [`\${${envKey}}`],
        groupAllowFrom: [`\${${envKey}}`],
        dmPolicy: "allowlist",
        groupPolicy: "allowlist",
        selfChatMode: true,
      };
      normalizedChannelConfig.accounts = nextAccounts;
      normalizedChannelConfig.enabled = true;
      if (!String(normalizedChannelConfig.defaultAccount || "").trim()) {
        normalizedChannelConfig.defaultAccount = "default";
      }
      refreshedCfg.channels =
        refreshedCfg.channels && typeof refreshedCfg.channels === "object"
          ? { ...refreshedCfg.channels }
          : {};
      refreshedCfg.channels.whatsapp = normalizedChannelConfig;
      appendBindingToConfig({
        cfg: refreshedCfg,
        agentId,
        match: normalizeBindingMatch({ channel: "whatsapp", accountId }),
      });
      saveConfig({ fsImpl, OPENCLAW_DIR, config: refreshedCfg });

      onProgress({ phase: "restarting", label: "Rebooting..." });
      await restartGateway();
    } catch (error) {
      try {
        writeEnvFile(previousEnvVars);
        reloadEnv();
      } catch {}
      try {
        saveConfig({ fsImpl, OPENCLAW_DIR, config: rollbackConfig });
      } catch {}
      throw error;
    }

    return {
      channel: "whatsapp",
      account: { id: accountId, name, envKey },
      binding: {
        agentId,
        match: normalizeBindingMatch({ channel: "whatsapp", accountId }),
      },
      restartRequired: true,
    };
  };

  const runChannelAccountLogin = async ({
    provider: rawProvider,
    accountId: rawAccountId,
  } = {}) => {
    const provider = normalizeChannelProvider(rawProvider);
    if (provider !== "whatsapp") {
      throw new Error("Channel login is currently only supported for WhatsApp");
    }
    const accountId = String(rawAccountId || "").trim() || "default";
    const loginArgs = [
      "channels login",
      `--channel ${shellEscapeArg(provider)}`,
      accountId !== "default" ? `--account ${shellEscapeArg(accountId)}` : "",
    ].filter(Boolean);
    const loginStartedAt = Date.now();
    const result = await clawCmd(loginArgs.join(" "), {
      quiet: true,
      timeoutMs: 12000,
      killSignal: "SIGKILL",
      allowConfigMutation: true,
    });
    const elapsedMs = Date.now() - loginStartedAt;
    console.log(
      `[channels] login ${provider}/${accountId} finished ok=${!!result?.ok} code=${String(
        result?.code ?? "",
      )} elapsedMs=${elapsedMs}`,
    );
    return {
      ok: !!result?.ok,
      stdout: String(result?.stdout || ""),
      stderr: String(result?.stderr || ""),
      completed: !!result?.ok,
    };
  };

  const getChannelAccountLoginStatus = async ({
    provider: rawProvider,
    accountId: rawAccountId,
  } = {}) => {
    const provider = normalizeChannelProvider(rawProvider);
    if (provider !== "whatsapp") {
      throw new Error("Channel login status is currently only supported for WhatsApp");
    }
    const accountId = String(rawAccountId || "").trim() || "default";
    const result = await clawCmd(
      `channels status --channel ${shellEscapeArg(provider)} --json`,
      { quiet: true, timeoutMs: 15000 },
    );
    if (!result?.ok) {
      throw new Error(
        summarizeOpenclawFailure(result, { fallback: "Could not read channel login status" }),
      );
    }
    const payload = parseJsonObjectFromNoisyOutput(
      [result.stdout, result.stderr].filter(Boolean).join("\n"),
    ) || {};
    const accounts =
      payload?.accounts?.[provider] ||
      payload?.channels?.[provider]?.accounts ||
      payload?.channelAccounts?.[provider] ||
      [];
    const account = (Array.isArray(accounts) ? accounts : []).find(
      (entry) => String(entry?.accountId || entry?.id || "default") === accountId,
    );
    return {
      provider,
      accountId,
      linked: Boolean(
        account?.linked === true ||
          account?.loggedIn === true ||
          account?.connected === true ||
          ["linked", "logged in", "connected", "running"].includes(
            String(account?.status || "").trim().toLowerCase(),
          ),
      ),
    };
  };

  return {
    getChannelAccountToken,
    createChannelAccount,
    updateChannelAccount,
    updateChannelAccountPolicy,
    deleteChannelAccount,
    runChannelAccountLogin,
    getChannelAccountLoginStatus,
    listConfiguredChannelAccountsWithMaskedTokens,
  };
};

module.exports = { createChannelsDomain };
