...
const createPluginReconcileRunner = ({
  ...
  async ({ onlyPluginKeys = null } = {}) => {
    ...
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      ...
      if (last.ok) {
        // The child logs each hotfix it writes; a channel flow restarts the
        // Gateway once when one landed during this install, in case the
        // Gateway already imported the unpatched module (G3 #35).
        const hotfixesApplied = /Applied OpenClaw plugin hotfix/.test(
          `${last.stdout || ""}\n${last.stderr || ""}`,
        );
        return { ok: true, attempt, ...(hotfixesApplied ? { hotfixesApplied } : {}) };
      }
      logger.warn?.(
