...
// Persistently crashing listeners (e.g. a CLI build with a broken realtime
// client) back off exponentially instead of hot-looping every 5 seconds.
const kMaxRestartDelayMs = 5 * 60 * 1000;
...
const createComposioListenService = ({
  ...
  const scheduleRestart = () => {
    ...
    const delay = Math.min(
      kRestartDelayMs * 2 ** Math.min(consecutiveFailures, 10),
      kMaxRestartDelayMs,
    );
  ...
  const startListener = async () => {
    ...
    spawned.on("exit", (code, signal) => {
      ...
      const cleanTail = stripAnsi(stderrTail).trim().slice(0, 200);
      saveWatch({
        pid: null,
        lastError: `${reason}${cleanTail ? `: ${cleanTail}` : ""}`,
      });
      scheduleRestart();
    });
