...
const insertAdvancedControlAcknowledgement = ({
  userIdentity,
  sessionId,
  instanceId,
  warningVersion,
  ...
  const result = ensureDb()
    .prepare(
      `
        INSERT INTO advanced_control_acknowledgements (
          user_identity,
          session_id,
          instance_id,
          warning_version,
          managed_config_revision,
          client_ip,
          acknowledged_at
        ) VALUES (
          $user_identity,
    ...
    .run({
      $user_identity: String(userIdentity || "unknown"),
      $session_id: String(sessionId || ""),
      $instance_id: String(instanceId || "unknown"),
      $warning_version: String(warningVersion || ""),
      $managed_config_revision: String(managedConfigRevision || ""),
      $client_ip: String(clientIp || ""),
      $acknowledged_at: String(acknowledgedAt || new Date().toISOString()),
    });
  return Number(result.lastInsertRowid || 0);
};

const listAdvancedControlAcknowledgements = ({ limit = 50 } = {}) => {
  ...
  return ensureDb()
    .prepare(
      `
        SELECT
          ...
          client_ip,
          acknowledged_at
        FROM advanced_control_acknowledgements
    ...
    .map((row) => ({
      id: Number(row.id),
      userIdentity: String(row.user_identity || ""),
      sessionId: String(row.session_id || ""),
      instanceId: String(row.instance_id || ""),
      warningVersion: String(row.warning_version || ""),
      managedConfigRevision: String(row.managed_config_revision || ""),
