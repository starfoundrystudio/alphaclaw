...
const containsCredentialValue = (value) => {
  ...
  return Object.entries(value).some(
    ([key, nested]) =>
      key.toLowerCase() === "value" || containsCredentialValue(nested),
  );
};
...
const registerAgentVaultRoutes = ({
  ...
  app.post("/api/agent-vault/proposals", requireAuth, async (req, res) => {
    if (containsCredentialValue(req.body)) {
    ...
    try {
      const result = await agentVaultService.ensureServiceAccess(req.body);
...
module.exports = { containsCredentialValue, registerAgentVaultRoutes };
