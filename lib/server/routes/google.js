...
const registerGoogleRoutes = ({
  ...
  app.get("/api/google/accounts", async (req, res) => {
    if (typeof gogBrokerService?.validateBrokeredAccounts === "function") {
      await gogBrokerService.validateBrokeredAccounts();
    }
    const state = readState();
    ...
    const accounts = listGoogleAccounts(state).map((account) => {
      ...
      const oauthHealth = account.brokerConsumer
        ? gogBrokerService?.getAccountHealth?.(account.id) || null
        : null;
      return {
        ...account,
        services,
