...
const { resolvePrivateUiBaseUrl } = require("../deployment-surface");
...
const parseExecApprovalsSnapshot = (result) => {
  const rawOutput = [result?.stdout, result?.stderr].filter(Boolean).join("\n");
  const parsed = parseJsonObjectFromNoisyOutput(rawOutput) || {};
  const file = parsed?.file && typeof parsed.file === "object" ? parsed.file : parsed;
  return ensureWildcardAgent(file && typeof file === "object" ? file : {});
};

const resolveSetupUiBaseUrl = (req) => resolvePrivateUiBaseUrl({ req });
...
const registerNodeRoutes = ({
  ...
  app.post("/api/nodes/exec-config", async (req, res) => {
    ...
    for (const command of commands) {
      const result = await clawCmd(command, { allowConfigMutation: true });
      if (!result.ok) {
        return res.status(500).json({
          ok: false,
  ...
  app.get("/api/nodes/exec-approvals", async (_req, res) => {
    ...
    if (!result?.ok) {
      return res.status(502).json({
        ok: false,
        error: result?.stderr || "Could not load execution approvals",
      });
    }
    const approvals = parseExecApprovalsSnapshot(result);
  ...
  app.post("/api/nodes/exec-approvals/allowlist", async (req, res) => {
    ...
    const snapshotResult = await clawCmd("approvals get --json", { quiet: true });
    if (!snapshotResult?.ok) {
      return res.status(502).json({
        ok: false,
        error: snapshotResult?.stderr || "Could not load execution approvals",
      });
    }
    const approvals = parseExecApprovalsSnapshot(snapshotResult);
    const allowlist = approvals.agents["*"].allowlist;
    ...
    const result = await clawCmd(
      `approvals allowlist add --agent '*' --json ${quoteCliArg(pattern)}`,
    );
    if (!result?.ok) {
      return res.status(502).json({
        ok: false,
        error: result?.stderr || "Could not add execution approval allowlist entry",
      });
    }
    const updated = parseExecApprovalsSnapshot(result);
    const entry = updated?.agents?.["*"]?.allowlist?.find(
  ...
  app.delete("/api/nodes/exec-approvals/allowlist/:id", async (req, res) => {
    ...
    const approvals = parseExecApprovalsSnapshot(snapshotResult);
    const allowlist = approvals.agents["*"].allowlist;
    const entry = allowlist.find((candidate) => String(candidate?.id || "") === id);
    if (!entry?.pattern) {
      return res.status(404).json({ ok: false, error: "Allowlist entry not found" });
    }
    const result = await clawCmd(
      `approvals allowlist remove --agent '*' --json ${quoteCliArg(entry.pattern)}`,
    );
    if (!result?.ok) {
      return res.status(502).json({
        ok: false,
        error: result?.stderr || "Could not remove execution approval allowlist entry",
      });
    }
    return res.json({ ok: true });
  });
