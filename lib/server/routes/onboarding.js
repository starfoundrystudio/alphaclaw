...
} = require("../onboarding/import/import-temp");
const {
  importPortableCronStore: defaultImportPortableCronStore,
} = require("../onboarding/import/portable-cron-import");
const {
  importPortableAuthStores: defaultImportPortableAuthStores,
} = require("../onboarding/import/portable-auth-import");
...
const registerOnboardingRoutes = ({
  ...
  app.post("/api/onboard/import/scan", async (req, res) => {
    ...
    try {
      ...
      if (!scan.sourceLayout?.supported) {
        cleanupImportTempDir(tempDir, { fsModule: fs });
  ...
  app.post("/api/onboard/import/apply", async (req, res) => {
    ...
    try {
      ...
      if (scan.sourceLayout.kind === "full-openclaw-root") {
        await importPortableCronStore({
          fs,
      ...
      if (envVars.length > 0 || merged.length !== existing.length) {
        writeEnvFile(merged);
        reloadEnv();
      }
      const systemVars =
      ...
      cleanupImportTempDir(req.body?.tempDir, { fsModule: fs });
      return res
        .status(500)
        .json({ ok: false, error: sanitizeOnboardingError(err) });
    }
