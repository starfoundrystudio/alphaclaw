...
const { ensureGithubRepoAccessible } = require("../onboarding/github");
const {
...
const registerSystemRoutes = ({
  ...
  const upsertEnvVar = (vars, key, value) => {
    const idx = vars.findIndex((entry) => entry.key === key);
    if (idx >= 0) {
      vars[idx] = { key, value };
      return;
    }
  ...
  const normalizeGithubRepoInput = (repoInput) =>
    String(repoInput || "")
      .trim()
      .replace(/^git@github\.com:/i, "")
      .replace(/^https:\/\/github\.com\//i, "")
      .replace(/\.git$/i, "");
  const resolveGithubRepoInput = (repoInput) => {
    const normalized = typeof resolveGithubRepoUrl === "function"
      ? resolveGithubRepoUrl(repoInput)
      : normalizeGithubRepoInput(repoInput);
  ...
  const hasGithubSyncConfig = () => {
    ...
    return Boolean(
      String(process.env.GITHUB_TOKEN || fileVarMap.get("GITHUB_TOKEN") || "").trim() &&
        String(
          process.env.GITHUB_WORKSPACE_REPO ||
            fileVarMap.get("GITHUB_WORKSPACE_REPO") ||
            "",
        ).trim(),
    );
  };
  ...
  app.post("/api/github-sync/config", async (req, res) => {
    const repoInput = String(req.body?.repo || "").trim();
    const githubToken = String(req.body?.token || "").trim();
    const schedule = String(req.body?.schedule || kDefaultSystemCronSchedule).trim();
    if (!repoInput || !githubToken) {
      return res.status(400).json({
        ok: false,
    ...
    let repoUrl = "";
    try {
      repoUrl = resolveGithubRepoInput(repoInput);
    } catch (err) {
      return res.status(400).json({
        ok: false,
        error: err.message || 'GitHub repo must be in "owner/repo" format.',
      });
    }

    const [repoOwner = "", repoName = ""] = repoUrl.split("/");
    const repoResult = await ensureGithubRepoAccessible({
      repoUrl,
      repoName,
      githubToken,
    });
    if (!repoResult.ok) {
      return res.status(repoResult.status || 400).json({
        ok: false,
        error: repoResult.error || "GitHub repository setup failed",
      });
    }

    const nextEnvVars = readEnvFile();
    upsertEnvVar(nextEnvVars, "GITHUB_TOKEN", githubToken);
    upsertEnvVar(nextEnvVars, "GITHUB_WORKSPACE_REPO", repoUrl);
    writeEnvFile(nextEnvVars);
    reloadEnv();
    const syncCron = applySystemCronConfig({
      enabled: true,
      schedule,
    });
