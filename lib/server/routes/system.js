...
const registerSystemRoutes = ({
  ...
  app.get("/api/gateway/dashboard", async (req, res) => {
    ...
    if (result.ok && result.stdout) {
      const tokenMatch = result.stdout.match(/#token=([^\s]+)/);
      if (tokenMatch) {
        return res.json({ ok: true, url: `/openclaw/#token=${tokenMatch[1]}` });
      }
    }
    const managedGatewayToken = String(process.env.OPENCLAW_GATEWAY_TOKEN || "").trim();
    if (managedGatewayToken) {
      return res.json({
        ok: true,
        url: `/openclaw/#token=${encodeURIComponent(managedGatewayToken)}`,
      });
    }
