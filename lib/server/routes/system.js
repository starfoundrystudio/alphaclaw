...
const { getRequestOrigin } = require("../deployment-surface");
const { listAgentEntries } = require("../agents/shared");
const {
...
} = require("../agent-vault/env-classification");
const {
  hasAgentVaultRuntime,
} = require("../agent-vault/runtime-store");
const https = require("https");

const isAgentVaultManagedInstallation = (fileVars = []) =>
  hasAgentVaultRuntime() ||
  String(process.env.ALPHACLAW_CONNECTIVITY_MODE || "").trim() ===
    "security_gateway" ||
  fileVars.some(
    (entry) =>
      entry?.key === "ALPHACLAW_CONNECTIVITY_MODE" &&
      String(entry?.value || "").trim() === "security_gateway",
  );
...
const registerSystemRoutes = ({
  ...
  // OpenClaw 2026.9 pairs every browser as a device. `openclaw dashboard
  // --json` mints a short-lived, one-time `browserUrl` whose fragment carries a
  // Control UI bootstrap profile; a browser that connects with it is paired
  // silently, so the customer never sees OpenClaw's "Approve this browser"
  // screen (which tells them to run CLI commands on the host). The URL points
  // at the loopback Gateway, so rewrite it onto the proxied `/openclaw`
  // namespace and the caller's own origin for the WebSocket target.
  const buildBootstrapDashboardUrl = ({ browserUrl, requestOrigin }) => {
    let parsed;
    try {
      parsed = new URL(String(browserUrl || ""));
    } catch {
      return "";
    }
    const params = new URLSearchParams(parsed.hash.replace(/^#/, ""));
    if (!params.get("bootstrapToken")) return "";
    ...
    if (origin) {
      ...
      if (originUrl) {
        const wsProtocol = originUrl.protocol === "http:" ? "ws:" : "wss:";
        params.set("gatewayUrl", `${wsProtocol}//${originUrl.host}/openclaw`);
      }
    } else {
      params.delete("gatewayUrl");
    }
    return `/openclaw/#${params.toString()}`;
  };
  // Minting the link is CPU-bound CLI work: 13–17 s on a 4 vCPU host with
  // 2026.9.5, over clawCmd's 15 s default, so it silently fell back to the
  // token URL and the browser landed on OpenClaw's approval screen (G3
  // finding #26). Give it its own timeout and share one mint between
  // overlapping requests (double clicks, a second tab).
  const kBootstrapDashboardTimeoutMs = 60 * 1000;
  let bootstrapDashboardMint = null;
  const mintBootstrapDashboardPayload = () => {
    if (!bootstrapDashboardMint) {
      bootstrapDashboardMint = clawCmd("dashboard --no-open --json", {
        quiet: true,
        timeoutMs: kBootstrapDashboardTimeoutMs,
      }).finally(() => {
        bootstrapDashboardMint = null;
      });
    }
    return bootstrapDashboardMint;
  };
  const openclawPairsEveryBrowser = () => {
  ...
  const getBootstrapDashboardUrl = async (req) => {
    const result = await mintBootstrapDashboardPayload();
    if (!result?.ok || !result.stdout) {
      console.warn(
        `[alphaclaw] Control UI bootstrap link failed${result?.timedOut ? " (timed out)" : ""}: ${String(result?.stderr || result?.message || "no output").trim().slice(0, 200)}`,
      );
      return null;
    }
    const payload = parseJsonObjectFromNoisyOutput(result.stdout);
    const url = buildBootstrapDashboardUrl({
      browserUrl: payload?.browserUrl,
      requestOrigin: getRequestOrigin(req),
    });
    if (!url) return null;
    const expiresAtMs = Number(payload?.browserBootstrapExpiresAtMs);
    return {
      url,
  ...
  // A brand-new install has zero sessions, which would leave the chat UI
  // with nothing selectable and a disabled composer. The gateway lazily
  // creates agent main sessions on first chat.send, so advertising the
  // default agent's main session before it exists is safe: selecting it and
  // sending a message materializes it.
  const buildFallbackMainSession = (config = {}) => {
    const agentId = resolveDefaultAgentId(config);
    const key = buildAgentMainSessionKey(agentId);
    return {
      key,
      sessionId: "",
      updatedAt: 0,
  ...
  const listSendableAgentSessions = async () => {
    ...
    if (mapped.length === 0) {
      return [buildFallbackMainSession(config)];
    }
    return mapped;
  };
  ...
  app.get("/api/env", (req, res) => {
    ...
    const merged = [];
    const agentVaultManaged = isAgentVaultManagedInstallation(fileVars);
    ...
    for (const def of kKnownVars) {
      ...
      if (
        agentVaultManaged &&
        isAgentVaultCredentialKey(def.key, { knownGroup: def.group })
      ) {
        if (value) legacyCredentialKeys.add(def.key);
        continue;
      }
      merged.push({
        key: def.key,
    ...
    for (const v of fileVars) {
      ...
      if (agentVaultManaged && isAgentVaultCredentialKey(v.key)) {
        if (v.value) legacyCredentialKeys.add(v.key);
        continue;
      }
      merged.push({
        key: v.key,
    ...
    res.json({
      ...
      agentVaultManaged,
      legacyCredentialKeys: Array.from(legacyCredentialKeys).sort(),
    });
  });
  ...
  app.put("/api/env", (req, res) => {
    ...
    const agentVaultManaged =
      isAgentVaultManagedInstallation(existingEnvVars);
    const credentialKeys = agentVaultManaged
      ? Array.from(
          new Set(
            vars
              .map((v) => String(v?.key || "").trim().toUpperCase())
              .filter((key) => isAgentVaultCredentialKey(key)),
          ),
        )
      : [];
    if (credentialKeys.length) {
      return res.status(400).json({
        ok: false,
        error: `Credentials must be requested through Agent Vault: ${credentialKeys.join(", ")}`,
      });
    }
    ...
    const existingValueByKey = new Map(
      existingEnvVars.map((v) => [
        String(v?.key || "").trim(),
        String(v?.value ?? ""),
      ]),
    );
    const changedFiltered = filtered.filter((v) => {
      const key = String(v?.key || "").trim();
      if (!existingValueByKey.has(key)) return true;
      return String(v?.value ?? "") !== existingValueByKey.get(key);
    });
    const providerCredentialValidation = validateEnvCredentialValues(changedFiltered);
    if (!providerCredentialValidation.ok) {
      return res
        .status(providerCredentialValidation.status || 400)
        .json({ ok: false, error: providerCredentialValidation.error });
    }
    ...
    const existingAgentVaultCredentialVars = agentVaultManaged
      ? existingEnvVars.filter((v) => isAgentVaultCredentialKey(v.key))
      : [];
    ...
    const existingHiddenKnownVars = existingEnvVars.filter((v) =>
      ...
      !(
        agentVaultManaged &&
        isAgentVaultCredentialKey(v.key, {
          knownGroup: kKnownVars.find((def) => def.key === v.key)?.group,
        })
      ),
    );
    const nextEnvVars = [
      ...
      ...existingAgentVaultCredentialVars,
  ...
  const buildStatusPayload = async () => {
    ...
    return {
      ...
      gatewayHop:
        typeof watchdog?.getStatus === "function"
          ? watchdog.getStatus()?.gatewayHop || null
          : null,
    };
  };
  ...
  app.put("/api/alphaclaw/config/features/openai-compat-api", async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({
        ok: false,
        error: err.message || "Could not update Clawbridge config",
      });
    }
  });
  ...
  app.get("/api/gateway/dashboard", async (req, res) => {
    ...
    try {
      const bootstrap = await getBootstrapDashboardUrl(req);
      if (bootstrap) {
        return res.json({
          ok: true,
          url: bootstrap.url,
          source: "bootstrap",
          ...(bootstrap.expiresAtMs ? { expiresAtMs: bootstrap.expiresAtMs } : {}),
        });
      }
    } catch {
      // Older OpenClaw lines have no --json; fall through to the token hand-off.
    }
    // OpenClaw 2026.9 pairs every browser: a token-only URL always lands on
    // its "Approve this browser" screen, which tells the customer to run
    // commands on the host. Report a retryable failure instead.
    if (openclawPairsEveryBrowser()) {
      return res.status(503).json({
        ok: false,
        retryable: true,
        error: "OpenClaw could not prepare secure access in time. Try again in a moment.",
      });
    }
