...
const getClaudeCliStatus = async ({
  ...
  if (profile?.brokered && typeof claudeBrokerService?.validateGrant === "function") {
    await claudeBrokerService.validateGrant();
  }
  const broker =
      ...
      child.on("exit", (code, signal) => {
        if (operation.status === "running") {
          ...
          if (operation.status === "complete") {
            try {
              void Promise.resolve(onClaudeLoginReady()).catch(() => {});
            } catch {}
          }
        }
