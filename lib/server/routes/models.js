...
const registerModelRoutes = ({
  ...
  const getProvidersFromModelConfigChange = ({
    ...
    if (configuredModels && typeof configuredModels === "object") {
      const currentModelKeys = new Set(Object.keys(currentConfig.configuredModels || {}));
      modelKeys.push(
  ...
  const validateModelProviderCredentials = ({
    ...
    for (const provider of providers) {
      ...
      if (!envValue) {
        const providerProfiles = (authProfiles.listProfiles(agentId) || []).filter(
          (profile) => profile?.provider === provider,
        );
        const profileResult = validateAuthProfileCredentialValues(providerProfiles);
  ...
  app.get("/api/models", async (req, res) => {
    ...
    return res.json({
      ...response,
      accessModes: response.accessModes || kBootstrapModelCatalog.accessModes || null,
    });
  });
  ...
  app.put("/api/models/config", async (req, res) => {
    ...
    try {
      ...
      const modelProviderCredentialValidation = validateModelProviderCredentials({
  ...
  app.post("/api/models/vault-key", async (req, res) => {
    ...
    try {
      const result = await agentVaultService.ensureModelProviderAccess(provider);
