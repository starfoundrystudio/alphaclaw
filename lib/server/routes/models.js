const fs = require("fs");
const {
  isDeniedModelKey,
  stripDeniedModelsFromCatalog,
} = require("../model-denylist");
const {
  kBootstrapModelCatalog,
  kFallbackOnboardingModels,
  OPENCLAW_DIR,
  kRootDir,
} = require("../constants");
const {
  reconcileOpenclawPlugins: defaultReconcileOpenclawPlugins,
} = require("../../cli/openclaw-plugin-compat");
const { createModelCatalogCache } = require("../model-catalog-cache");
const { resolveThinkingOptionsForModel } = require("../openclaw-thinking");
const { readOpenclawConfig } = require("../openclaw-config");
const { getCommandOutputCandidates } = require("../utils/command-output");
const {
  validateEnvCredentialValues,
  validateAuthProfileCredentialValues,
} = require("../provider-credential-validation");
const {
  getModelProviderVaultConfig,
  isVaultPlaceholderValue,
  listVaultBrokeredModelProviders,
} = require("../agent-vault/model-provider-services");
const { hasAgentVaultRuntime } = require("../agent-vault/runtime-store");

const parseJsonFromShellError = ({
  error,
  parseJsonFromNoisyOutput = () => null,
} = {}) => {
  for (const rawOutput of getCommandOutputCandidates(error)) {
    const parsed = parseJsonFromNoisyOutput(rawOutput);
    if (parsed) return parsed;
  }
  return null;
};

const getHostedCatalogRestartRequired = (payload = {}) => {
  const status = String(payload?.status || "").trim().toLowerCase();
  return (
    payload?.restartRequired === true ||
    payload?.needsRestart === true ||
    status === "updated"
  );
};

const shouldReconcileManagedRuntimePlugins = (result) =>
  Array.isArray(result?.managedPluginIds) && result.managedPluginIds.length > 0;

const normalizeAuthIdentifier = (value) =>
  String(value || "")
    .trim()
    .toLowerCase();

const isCodexOauthMutation = (profileId, credential) =>
  normalizeAuthIdentifier(profileId) === "openai:codex-cli" ||
  normalizeAuthIdentifier(profileId).startsWith("openai-codex:") ||
  (credential?.type === "oauth" &&
    (normalizeAuthIdentifier(credential?.provider) === "openai" ||
      normalizeAuthIdentifier(credential?.provider) === "openai-codex" ||
      credential?.refresh ===
        "alphaclaw-oauth-broker:v1:openclaw-codex:openai"));

const codexBrokerPolicyError = () => ({
  ok: false,
  status: 409,
  error:
    "Codex OAuth credentials must be connected or disconnected through the Codex account flow so the refresh grant remains brokered.",
});

const registerModelRoutes = ({
  app,
  shellCmd,
  gatewayEnv,
  gatewayMaintenanceEnv = gatewayEnv,
  parseJsonFromNoisyOutput,
  normalizeOnboardingModels,
  readOpenclawVersion,
  isOnboarded = () => true,
  authProfiles,
  agentVaultService = null,
  hasVaultRuntime = hasAgentVaultRuntime,
  readEnvFile,
  writeEnvFile,
  reloadEnv,
  reconcileOpenclawPlugins = defaultReconcileOpenclawPlugins,
  rootDir = kRootDir,
  openclawDir = OPENCLAW_DIR,
  fsModule = fs,
  modelCatalogCache = createModelCatalogCache({
    shellCmd,
    gatewayEnv,
    parseJsonFromNoisyOutput,
    normalizeOnboardingModels,
    readOpenclawVersion,
    shouldStartDynamicRefresh: isOnboarded,
    fallbackModels: kFallbackOnboardingModels,
    fallbackAccessModes: kBootstrapModelCatalog.accessModes || null,
  }),
}) => {
  const upsertEnvVar = (items, key, value) => {
    const next = Array.isArray(items) ? [...items] : [];
    const existing = next.find((entry) => entry.key === key);
    if (existing) {
      existing.value = value;
      return next;
    }
    next.push({ key, value });
    return next;
  };

  const removeEnvVar = (items, key) => {
    const next = Array.isArray(items) ? [...items] : [];
    return next.filter((entry) => entry.key !== key);
  };

  const readEnvVarMap = () => {
    if (typeof readEnvFile !== "function") return new Map();
    return new Map(
      (readEnvFile() || []).map((entry) => [
        String(entry?.key || "").trim(),
        String(entry?.value || "").trim(),
      ]),
    );
  };

  const buildEnvBackedProfiles = (agentId) => {
    const envMap = readEnvVarMap();
    const providers = authProfiles.listApiKeyProviders?.() || [];
    return providers.flatMap((provider) => {
      const envKey = authProfiles.getEnvVarForApiKeyProvider?.(provider);
      const envValue = String(envMap.get(envKey) || "").trim();
      if (!envKey || !envValue) return [];
      const profileId =
        authProfiles.getDefaultProfileIdForApiKeyProvider?.(provider) ||
        `${provider}:default`;
      return [
        {
          id: profileId,
          type: "api_key",
          provider,
          key: envValue,
        },
      ];
    });
  };

  const mergeProfilesWithEnvFallback = (profiles, agentId) => {
    const mergedProfiles = Array.isArray(profiles) ? [...profiles] : [];
    const profileIndexById = new Map(
      mergedProfiles.map((profile, index) => [profile?.id, index]),
    );
    for (const envProfile of buildEnvBackedProfiles(agentId)) {
      const existingIndex = profileIndexById.get(envProfile.id);
      if (existingIndex === undefined) {
        profileIndexById.set(envProfile.id, mergedProfiles.length);
        mergedProfiles.push(envProfile);
        continue;
      }
      const existingProfile = mergedProfiles[existingIndex] || {};
      const existingValue = String(
        existingProfile?.key ||
          existingProfile?.token ||
          existingProfile?.access ||
          "",
      ).trim();
      if (existingValue) continue;
      mergedProfiles[existingIndex] = {
        ...existingProfile,
        ...envProfile,
      };
    }
    return mergedProfiles;
  };

  const syncEnvVarsForProfiles = (profiles) => {
    if (
      !Array.isArray(profiles) ||
      typeof readEnvFile !== "function" ||
      typeof writeEnvFile !== "function" ||
      typeof reloadEnv !== "function"
    ) {
      return;
    }
    let nextEnvVars = readEnvFile();
    let changed = false;
    for (const profile of profiles) {
      if (profile?.type !== "api_key") continue;
      const envKey = authProfiles.getEnvVarForApiKeyProvider?.(
        profile.provider,
      );
      const envValue = String(profile?.key || "").trim();
      if (!envKey) continue;
      const prevValue = String(
        nextEnvVars.find((entry) => entry.key === envKey)?.value || "",
      );
      if (!envValue) {
        if (!prevValue) continue;
        nextEnvVars = removeEnvVar(nextEnvVars, envKey);
        changed = true;
        continue;
      }
      if (prevValue === envValue) continue;
      nextEnvVars = upsertEnvVar(nextEnvVars, envKey, envValue);
      changed = true;
    }
    if (!changed) return;
    writeEnvFile(nextEnvVars);
    reloadEnv();
  };

  const syncProfilesFromEnvVars = (agentId) => {
    if (
      typeof readEnvFile !== "function" ||
      typeof authProfiles.upsertApiKeyProfileForEnvVar !== "function" ||
      typeof authProfiles.removeApiKeyProfileForEnvVar !== "function"
    ) {
      return;
    }
    const envMap = readEnvVarMap();
    const providers = authProfiles.listApiKeyProviders?.() || [];
    for (const provider of providers) {
      const envKey = authProfiles.getEnvVarForApiKeyProvider?.(provider);
      if (!envKey) continue;
      const envValue = String(envMap.get(envKey) || "").trim();
      if (!envValue) {
        authProfiles.removeApiKeyProfileForEnvVar(provider, agentId);
        continue;
      }
      authProfiles.upsertApiKeyProfileForEnvVar(provider, envValue, agentId);
    }
  };

  // Once the Agent Vault runtime is claimed, raw API keys for brokered-capable
  // providers are rejected — the vault flow is the only door (spec §6). Before
  // the claim (onboarding, non-managed installs) the env path stands.
  const isVaultKeyEnforced = () => {
    try {
      return hasVaultRuntime();
    } catch {
      return false;
    }
  };

  const findBlockedRawKeyProviders = (profiles) =>
    Array.from(
      new Set(
        (Array.isArray(profiles) ? profiles : [])
          .filter(
            (profile) =>
              profile?.type === "api_key" &&
              getModelProviderVaultConfig(profile?.provider) &&
              String(profile?.key || "").trim() &&
              !isVaultPlaceholderValue(profile?.key),
          )
          .map((profile) => String(profile.provider)),
      ),
    );

  const validateVaultKeyPolicy = (profiles) => {
    if (!isVaultKeyEnforced()) return { ok: true };
    const blocked = findBlockedRawKeyProviders(profiles);
    if (!blocked.length) return { ok: true };
    return {
      ok: false,
      status: 400,
      error: `Model API keys for ${blocked.join(", ")} are managed by Agent Vault on this instance. Use "Store key in Agent Vault" in model settings instead of entering the key here.`,
    };
  };

  const activateVaultPlaceholder = ({ provider, placeholder, agentId }) => {
    const profileId =
      authProfiles.getDefaultProfileIdForApiKeyProvider?.(provider) ||
      `${provider}:default`;
    const credential = { type: "api_key", provider, key: placeholder };
    authProfiles.upsertProfile(profileId, credential, agentId);
    syncEnvVarsForProfiles([{ id: profileId, ...credential }]);
    authProfiles.syncConfigAuthReferencesForAgent?.(agentId);
    modelCatalogCache.markStale();
  };

  const getProvidersFromModelConfigChange = ({
    primary,
    configuredModels,
    currentConfig = {},
  }) => {
    const modelKeys = [];
    if (primary !== undefined && primary !== currentConfig.primary) {
      modelKeys.push(primary);
    }
    if (configuredModels && typeof configuredModels === "object") {
      const currentModelKeys = new Set(
        Object.keys(currentConfig.configuredModels || {}),
      );
      modelKeys.push(
        ...Object.keys(configuredModels).filter(
          (modelKey) => !currentModelKeys.has(modelKey),
        ),
      );
    }
    return Array.from(
      new Set(
        modelKeys
          .map((modelKey) => String(modelKey || "").split("/")[0])
          .filter(Boolean),
      ),
    );
  };

  const validateModelProviderCredentials = ({
    providers = [],
    profiles,
    agentId,
  } = {}) => {
    if (!providers.length) return { ok: true };
    const submittedProfilesByProvider = new Map(
      (Array.isArray(profiles) ? profiles : [])
        .filter((profile) => profile?.provider)
        .map((profile) => [String(profile.provider), profile]),
    );
    const envMap = readEnvVarMap();

    for (const provider of providers) {
      const submittedProfile = submittedProfilesByProvider.get(provider);
      if (submittedProfile) {
        const result = validateAuthProfileCredentialValues([submittedProfile]);
        if (!result.ok) return result;
        continue;
      }

      const envKey = authProfiles.getEnvVarForApiKeyProvider?.(provider);
      const envValue = envKey ? envMap.get(envKey) : "";
      const envResult = validateEnvCredentialValues([
        { key: envKey, value: envValue },
      ]);
      if (!envResult.ok) return envResult;

      if (!envValue) {
        const providerProfiles = (
          authProfiles.listProfiles(agentId) || []
        ).filter((profile) => profile?.provider === provider);
        const profileResult =
          validateAuthProfileCredentialValues(providerProfiles);
        if (!profileResult.ok) return profileResult;
      }
    }

    return { ok: true };
  };

  // ── Existing CLI-backed catalog/status routes ──

  app.get("/api/models", async (req, res) => {
    const response = await modelCatalogCache.getCatalogResponse();
    return res.json(
      stripDeniedModelsFromCatalog({
        ...response,
        accessModes:
          response.accessModes || kBootstrapModelCatalog.accessModes || null,
      }),
    );
  });

  app.post("/api/models/refresh", async (req, res) => {
    const scope = String(req.body?.scope || "all")
      .trim()
      .toLowerCase();
    if (!new Set(["all", "providers", "hosted"]).has(scope)) {
      return res.status(400).json({
        ok: false,
        error: "scope must be all, providers, or hosted",
      });
    }

    let hostedCatalog = null;
    if (scope === "all" || scope === "hosted") {
      try {
        const output = await shellCmd("openclaw models refresh --json", {
          env: gatewayEnv(),
          timeout: 30000,
        });
        const parsed = parseJsonFromNoisyOutput(output);
        if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
          throw new Error("OpenClaw returned an invalid hosted catalog result");
        }
        hostedCatalog = {
          ok: true,
          ...parsed,
          restartRequired: getHostedCatalogRestartRequired(parsed),
        };
      } catch (err) {
        const parsed = parseJsonFromShellError({
          error: err,
          parseJsonFromNoisyOutput,
        });
        hostedCatalog = parsed
          ? {
              ...parsed,
              ok: parsed.ok !== false,
              restartRequired: getHostedCatalogRestartRequired(parsed),
            }
          : {
              ok: false,
              restartRequired: false,
              error: err.message || "Hosted model catalog refresh failed",
            };
      }
    }

    if (scope === "hosted") {
      return res.status(hostedCatalog?.ok === false ? 502 : 200).json({
        ok: hostedCatalog?.ok !== false,
        scope,
        hostedCatalog,
        restartRequired: !!hostedCatalog?.restartRequired,
      });
    }

    try {
      const catalog = await modelCatalogCache.refreshProviderInventory();
      const hostedWarning = hostedCatalog?.ok === false
        ? hostedCatalog.error || "Hosted model catalog refresh failed"
        : "";
      return res.json(
        stripDeniedModelsFromCatalog({
          ...catalog,
          scope,
          hostedCatalog,
          warning: [catalog.warning, hostedWarning].filter(Boolean).join(" ") || undefined,
          restartRequired: !!hostedCatalog?.restartRequired,
        }),
      );
    } catch (err) {
      return res.status(502).json({
        ok: false,
        scope,
        hostedCatalog,
        restartRequired: !!hostedCatalog?.restartRequired,
        error: err.message || "Provider model discovery failed",
      });
    }
  });

  app.get("/api/models/thinking-options", async (req, res) => {
    const modelKey = String(req.query?.modelKey || "").trim();
    if (!modelKey.includes("/")) {
      return res
        .status(400)
        .json({ ok: false, error: "modelKey is required (provider/model)" });
    }
    try {
      let globalThinkingDefault = null;
      let agentRuntime = "openclaw";
      try {
        const cfg = readOpenclawConfig({ openclawDir });
        globalThinkingDefault =
          typeof cfg.agents?.defaults?.thinkingDefault === "string"
            ? cfg.agents.defaults.thinkingDefault.trim()
            : null;
        const providerId = modelKey.slice(0, modelKey.indexOf("/"));
        agentRuntime =
          cfg.agents?.defaults?.models?.[modelKey]?.agentRuntime?.id ||
          cfg.models?.providers?.[providerId]?.agentRuntime?.id ||
          "openclaw";
      } catch {}
      const { levels, modelDefault } = await resolveThinkingOptionsForModel({
        modelKey,
        agentRuntime,
      });
      const inheritedDefault = globalThinkingDefault || modelDefault || "off";
      return res.json({
        ok: true,
        modelKey,
        levels,
        modelDefault,
        inheritedDefault,
      });
    } catch (err) {
      return res.status(500).json({
        ok: false,
        error: err.message || "Failed to resolve thinking options",
      });
    }
  });

  app.get("/api/models/status", async (req, res) => {
    try {
      const output = await shellCmd("openclaw models status --json", {
        env: gatewayEnv(),
        timeout: 20000,
      });
      const parsed = parseJsonFromNoisyOutput(output) || {};
      res.json({
        ok: true,
        modelKey: parsed.resolvedDefault || parsed.defaultModel || null,
        fallbacks: parsed.fallbacks || [],
        imageModel: parsed.imageModel || null,
      });
    } catch (err) {
      const parsed = parseJsonFromShellError({
        error: err,
        parseJsonFromNoisyOutput,
      });
      if (parsed) {
        return res.json({
          ok: true,
          modelKey: parsed.resolvedDefault || parsed.defaultModel || null,
          fallbacks: parsed.fallbacks || [],
          imageModel: parsed.imageModel || null,
        });
      }
      res.json({
        ok: false,
        error: err.message || "Failed to read model status",
      });
    }
  });

  app.post("/api/models/set", async (req, res) => {
    const { modelKey } = req.body || {};
    if (!modelKey || typeof modelKey !== "string" || !modelKey.includes("/")) {
      return res.status(400).json({ ok: false, error: "Missing modelKey" });
    }
    if (isDeniedModelKey(modelKey)) {
      return res.status(400).json({
        ok: false,
        error: `${modelKey} is not available on this instance; choose another model`,
      });
    }
    try {
      const agentId = req.query.agentId || undefined;
      const credentialValidation = validateModelProviderCredentials({
        providers: [String(modelKey).split("/")[0]].filter(Boolean),
        agentId,
      });
      if (!credentialValidation.ok) {
        return res
          .status(credentialValidation.status || 400)
          .json({ ok: false, error: credentialValidation.error });
      }
      await shellCmd(`openclaw models set "${modelKey}"`, {
        env: gatewayMaintenanceEnv(),
        timeout: 30000,
      });
      modelCatalogCache.markStale();
      res.json({ ok: true });
    } catch (err) {
      res
        .status(400)
        .json({ ok: false, error: err.message || "Failed to set model" });
    }
  });

  // ── Model config (direct JSON) ──

  app.get("/api/models/config", (req, res) => {
    try {
      const { primary, configuredModels, providerRuntimeIds, modelRuntimeIds } =
        authProfiles.getModelConfig();
      const agentId = req.query.agentId || undefined;
      const profiles = mergeProfilesWithEnvFallback(
        authProfiles.listProfiles(agentId),
        agentId,
      );
      const store = authProfiles.loadAuthStore(agentId);
      res.json({
        ok: true,
        primary,
        configuredModels,
        providerRuntimeIds: providerRuntimeIds || {},
        modelRuntimeIds: modelRuntimeIds || {},
        authProfiles: profiles,
        authOrder: store.order || {},
        vaultBrokering: {
          enforced: isVaultKeyEnforced(),
          providers: listVaultBrokeredModelProviders(),
        },
      });
    } catch (err) {
      res
        .status(500)
        .json({ ok: false, error: err.message || "Failed to read config" });
    }
  });

  app.put("/api/models/config", async (req, res) => {
    const { primary, configuredModels, profiles, authOrder } = req.body || {};
    const agentId = req.query.agentId || undefined;
    if (
      primary !== undefined &&
      (typeof primary !== "string" || !primary.includes("/"))
    ) {
      return res
        .status(400)
        .json({ ok: false, error: "Invalid primary model key" });
    }
    if (
      configuredModels !== undefined &&
      (typeof configuredModels !== "object" || configuredModels === null)
    ) {
      return res
        .status(400)
        .json({ ok: false, error: "Invalid configuredModels" });
    }
    const deniedModelKey = [
      primary,
      ...Object.keys(configuredModels || {}),
    ].find((key) => isDeniedModelKey(key));
    if (deniedModelKey) {
      return res.status(400).json({
        ok: false,
        error: `${deniedModelKey} is not available on this instance; choose another model`,
      });
    }
    if (Array.isArray(profiles)) {
      const profileValidation = validateAuthProfileCredentialValues(profiles);
      if (!profileValidation.ok) {
        return res
          .status(profileValidation.status || 400)
          .json({ ok: false, error: profileValidation.error });
      }
      const vaultPolicy = validateVaultKeyPolicy(profiles);
      if (!vaultPolicy.ok) {
        return res
          .status(vaultPolicy.status || 400)
          .json({ ok: false, error: vaultPolicy.error });
      }
      if (
        profiles.some(
          ({ id: profileId, ...credential }) =>
            isCodexOauthMutation(profileId, credential) ||
            isCodexOauthMutation(
              profileId,
              authProfiles.getProfile?.(profileId, agentId),
            ),
        )
      ) {
        const policy = codexBrokerPolicyError();
        return res
          .status(policy.status)
          .json({ ok: false, error: policy.error });
      }
    }
    try {
      const currentModelConfig = authProfiles.getModelConfig?.() || {};
      const modelProviderCredentialValidation =
        validateModelProviderCredentials({
          providers: getProvidersFromModelConfigChange({
            primary,
            configuredModels,
            currentConfig: currentModelConfig,
          }),
          profiles,
          agentId,
        });
      if (!modelProviderCredentialValidation.ok) {
        return res
          .status(modelProviderCredentialValidation.status || 400)
          .json({ ok: false, error: modelProviderCredentialValidation.error });
      }
      const modelConfigResult = authProfiles.setModelConfig({
        primary,
        configuredModels,
      });

      if (Array.isArray(profiles)) {
        for (const { id: profileId, ...credential } of profiles) {
          if (profileId && credential.type && credential.provider) {
            authProfiles.upsertProfile(profileId, credential, agentId);
          }
        }
        syncEnvVarsForProfiles(profiles);
      }

      syncProfilesFromEnvVars(agentId);

      if (authOrder && typeof authOrder === "object") {
        for (const [provider, order] of Object.entries(authOrder)) {
          if (Array.isArray(order)) {
            authProfiles.setAuthOrder(provider, order, agentId);
          }
        }
      }

      // Keep config-visible auth references in sync with the durable auth store
      // so model re-adds restore refs.
      authProfiles.syncConfigAuthReferencesForAgent(agentId);

      if (
        shouldReconcileManagedRuntimePlugins(modelConfigResult) &&
        typeof reconcileOpenclawPlugins === "function"
      ) {
        try {
          await reconcileOpenclawPlugins({
            rootDir,
            openclawDir,
            fsModule,
            logger: console,
            env: process.env,
            // Only the runtime plugins this save enabled (G3 #21).
            onlyPluginKeys: modelConfigResult.managedPluginIds,
          });
        } catch (err) {
          throw new Error(
            "OpenClaw plugin installation failed. Please retry saving model settings; if it persists, check package registry/network access for OpenClaw plugins.",
          );
        }
      }

      modelCatalogCache.markStale();
      res.json({ ok: true });
    } catch (err) {
      res
        .status(500)
        .json({ ok: false, error: err.message || "Failed to save config" });
    }
  });

  // ── Vault-brokered model keys ──

  app.post("/api/models/vault-key", async (req, res) => {
    const provider = String(req.body?.provider || "").trim();
    const agentId = req.query.agentId || undefined;
    if (!provider) {
      return res.status(400).json({ ok: false, error: "Missing provider" });
    }
    const config = getModelProviderVaultConfig(provider);
    if (!config) {
      return res.status(400).json({
        ok: false,
        error: `Model provider ${provider} does not support Agent Vault brokering`,
      });
    }
    if (typeof agentVaultService?.ensureModelProviderAccess !== "function") {
      return res.status(409).json({
        ok: false,
        error: "Agent Vault is not available on this installation",
      });
    }
    try {
      const result =
        await agentVaultService.ensureModelProviderAccess(provider);
      if (result.status === "available") {
        activateVaultPlaceholder({
          provider,
          placeholder: result.placeholder,
          agentId,
        });
        return res.json({
          ok: true,
          status: "active",
          provider,
          credentialKey: result.credentialKey,
        });
      }
      return res.status(201).json({
        ok: true,
        status: "pending",
        provider,
        credentialKey: result.credentialKey,
        proposal: result.proposal,
      });
    } catch (err) {
      const status = Number(err?.status);
      return res.status(status >= 400 && status < 600 ? status : 500).json({
        ok: false,
        error: err.message || "Agent Vault request failed",
      });
    }
  });

  // ── Auth profiles (direct JSON) ──

  app.get("/api/models/auth", (req, res) => {
    try {
      const agentId = req.query.agentId || undefined;
      const profiles = authProfiles.listProfiles(agentId);
      const store = authProfiles.loadAuthStore(agentId);
      res.json({ ok: true, profiles, order: store.order || {} });
    } catch (err) {
      res.status(500).json({
        ok: false,
        error: err.message || "Failed to read auth profiles",
      });
    }
  });

  app.put("/api/models/auth/:profileId", (req, res) => {
    const { profileId } = req.params;
    const credential = req.body;
    if (!profileId || !credential?.type || !credential?.provider) {
      return res
        .status(400)
        .json({ ok: false, error: "Missing profileId, type, or provider" });
    }
    const validTypes = new Set(["api_key", "token", "oauth"]);
    if (!validTypes.has(credential.type)) {
      return res.status(400).json({
        ok: false,
        error: `Invalid credential type: ${credential.type}`,
      });
    }
    const profileValidation = validateAuthProfileCredentialValues([credential]);
    if (!profileValidation.ok) {
      return res
        .status(profileValidation.status || 400)
        .json({ ok: false, error: profileValidation.error });
    }
    const vaultPolicy = validateVaultKeyPolicy([credential]);
    if (!vaultPolicy.ok) {
      return res
        .status(vaultPolicy.status || 400)
        .json({ ok: false, error: vaultPolicy.error });
    }
    if (isCodexOauthMutation(profileId, credential)) {
      const policy = codexBrokerPolicyError();
      return res.status(policy.status).json({ ok: false, error: policy.error });
    }
    try {
      const agentId = req.query.agentId || undefined;
      if (
        isCodexOauthMutation(
          profileId,
          authProfiles.getProfile?.(profileId, agentId),
        )
      ) {
        const policy = codexBrokerPolicyError();
        return res
          .status(policy.status)
          .json({ ok: false, error: policy.error });
      }
      authProfiles.upsertProfile(profileId, credential, agentId);
      syncEnvVarsForProfiles([{ id: profileId, ...credential }]);
      modelCatalogCache.markStale();
      res.json({ ok: true });
    } catch (err) {
      res.status(500).json({
        ok: false,
        error: err.message || "Failed to save auth profile",
      });
    }
  });

  app.delete("/api/models/auth/:profileId", (req, res) => {
    const { profileId } = req.params;
    if (!profileId) {
      return res.status(400).json({ ok: false, error: "Missing profileId" });
    }
    try {
      const agentId = req.query.agentId || undefined;
      const existing = authProfiles.getProfile?.(profileId, agentId);
      if (isCodexOauthMutation(profileId, existing || { type: "oauth" })) {
        const policy = codexBrokerPolicyError();
        return res
          .status(policy.status)
          .json({ ok: false, error: policy.error });
      }
      const removed = authProfiles.removeProfile(profileId, agentId);
      modelCatalogCache.markStale();
      res.json({ ok: true, removed });
    } catch (err) {
      res.status(500).json({
        ok: false,
        error: err.message || "Failed to remove auth profile",
      });
    }
  });
};

module.exports = { registerModelRoutes };
