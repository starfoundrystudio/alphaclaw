...
const registerPairingRoutes = ({
  ...
  ensureManagedGatewayDevice = () =>
    ensureManagedGatewayDevicePreapproval({ fsModule, openclawDir }),
  gatewayToken = "",
  getGatewayPort = null,
}) => {
    ...
    try {
      ...
      for (const pendingDevice of pendingList) {
        ...
        if (approveResult.ok) {
          ...
          try {
            const repairResult = await ensureManagedGatewayDevice();
            if (repairResult?.ok === false) {
              console.log(
                `[alphaclaw] Managed backend approval cache repair failed: ${(repairResult.error || repairResult.reason || "unknown error").slice(0, 200)}`,
              );
            }
          } catch (error) {
            console.log(
              `[alphaclaw] Managed backend approval cache repair failed: ${String(error?.message || error).slice(0, 200)}`,
            );
          }
        } else {
          console.log(
