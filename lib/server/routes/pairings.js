...
const registerPairingRoutes = ({
  ...
  app.get("/api/devices", async (req, res) => {
    ...
    if (!result.ok) {
      if (result.stderr || result.stdout) {
        console.log(
          `[alphaclaw] devices list failed: ${(result.stderr || result.stdout).slice(0, 200)}`,
        );
      }
      return res.json({ pending: [], cliAutoApproveComplete: hasCliAutoApproveMarker() });
    }
    try {
      ...
      if (!hasCliAutoApproveMarker()) {
        ...
        if (firstCliPendingId) {
          ...
          const approveResult = await clawCmd(
            buildLocalDeviceCommand({
              baseCommand: `devices approve ${quoteCliArg(firstCliPendingId)}`,
              gatewayToken,
              getGatewayPort,
            }),
            {
  ...
  app.post("/api/devices/:id/approve", async (req, res) => {
    const result = await clawCmd(
      buildLocalDeviceCommand({
        baseCommand: `devices approve ${quoteCliArg(req.params.id)}`,
        gatewayToken,
        getGatewayPort,
      }),
    );
  ...
  app.post("/api/devices/:id/reject", async (req, res) => {
    const result = await clawCmd(
      buildLocalDeviceCommand({
        baseCommand: `devices reject ${quoteCliArg(req.params.id)}`,
        gatewayToken,
        getGatewayPort,
      }),
    );
