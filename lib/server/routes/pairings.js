...
const kPairingApproveTimeoutMs = 60000;
...
const registerPairingRoutes = ({
  ...
  app.post("/api/pairings/:id/approve", async (req, res) => {
    ...
    // The CLI cold-starts OpenClaw's plugin runtime; 15 s (the clawCmd
    // default) was too short on a busy host and killed approvals that had
    // already landed (G3 host 11).
    const result = await clawCmd(approveCmd, {
      timeoutMs: kPairingApproveTimeoutMs,
    });
