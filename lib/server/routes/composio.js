...
const { stripAnsi } = require("../utils/text");
...
const registerComposioRoutes = ({
  ...
  const buildStatusPayload = (state) => {
    ...
    const cliBusy =
      installer.isComposioInstalling() ||
      (installer.isComposioUpgrading?.() || false);
    return {
      ok: true,
      ...
      // While the CLI is being installed/updated, errors from the outgoing
      // binary are stale noise (e.g. an old client failing schema validation
      // against the current backend) — show the transition state instead.
      lastError: cliBusy ? "" : state.lastError,
      cliVersion: state.cliVersion || "",
      targetCliVersion: String(constants.kComposioCliTargetVersion || "").trim(),
      cliInstalling: installer.isComposioInstalling(),
      cliUpdating: installer.isComposioUpgrading?.() || false,
  ...
  // Reconcile the CLI when the provider is composio: install it if absent,
  // upgrade it to the pinned target version if outdated, and keep the
  // experimental listen flag written — all from the running server, since
  // customers cannot be asked to restart the Clawbridge process (the
  // dashboard Restart button only restarts the gateway).
  const maybeReconcileCli = (req, state) => {
    ...
    try {
      installer.ensureComposioListenFlag?.({ fs });
    } catch {}
    const target = String(constants.kComposioCliTargetVersion || "").trim();
    if (!target) return false;
    if (
      state.cliVersion &&
      composioInstallModule.compareComposioVersions(state.cliVersion, target) >= 0
    ) {
      return false;
    }
    if (busy) return true;
    installer.ensureComposioCliAtVersion?.({
      fs,
      targetVersion: target,
      stopListener: () => composioListenService.stop(),
      startListener: () => composioListenService.start(),
      onComplete: () => refreshStateAndArtifacts(req),
    });
    return true;
  };

  // Converge existing instances on boot, not only when someone opens the
  // dashboard.
  if (bootReconcile) {
    const bootTimer = setTimeout(() => {
      try {
        maybeReconcileCli(null, readComposioState({ fs, statePath }));
      } catch (err) {
        console.error("[alphaclaw] Composio boot reconcile failed:", err.message);
      }
    }, 0);
    bootTimer.unref?.();
  }
