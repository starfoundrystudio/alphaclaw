...
const { assertSupportedOpenclawPluginSdk } = require("./openclaw-plugin-sdk-contract");
...
const runOnboardedBootSequence = async ({
  ...
  await assertSupportedOpenclawPluginSdk();
  ...
  // Refresh brokered Codex credentials before OpenClaw starts reading its
  // provider-auth store. Failure is intentionally non-fatal to Clawbridge:
  // the placeholder stays in place, model access fails closed, and the
  // service retries while surfacing the degraded state in /api/codex/status.
  try {
    if (typeof codexBrokerService?.start === "function") {
      await codexBrokerService.start();
    }
  } catch (error) {
    console.error(
      `[alphaclaw] Failed to start Codex OAuth broker service: ${error.message}`,
    );
  }
  try {
    if (typeof claudeBrokerService?.start === "function") {
      await claudeBrokerService.start();
    }
  } catch (error) {
    console.error(
      `[alphaclaw] Failed to start Claude OAuth broker service: ${error.message}`,
    );
  }
  try {
    if (typeof gogBrokerService?.start === "function") {
      await gogBrokerService.start();
    }
  } catch (error) {
    console.error(
      `[alphaclaw] Failed to start gog OAuth broker service: ${error.message}`,
    );
  }
  ...
  try {
    const kickoff = bootstrapKickoff?.maybeRunBootstrapKickoff?.();
    // A greeting sent on an EARLIER boot that the agent never answered is
    // re-sent once this boot's runtime and model are usable. A greeting this
    // boot just sent is not "unanswered" yet; re-checking it here sent the
    // agent two greetings (G3, 2026-09-21).
    Promise.resolve(kickoff)
      .then((result) =>
        result?.reason === "kickoff_sent"
          ? null
          : bootstrapKickoff?.retryUnansweredKickoff?.(),
      )
      .catch(() => {});
  } catch (error) {
    console.error(
  ...
  try {
    pluginHotfixWatcher?.start?.();
  } catch (error) {
    console.error(
      `[alphaclaw] Failed to start plugin hotfix watcher: ${error.message}`,
    );
  }
  watchdog.start();
