...
const validateHost = (value) => {
  ...
  if (!host || /[\r\n\0]/.test(host)) {
    throw new OAuthBrokerError("invalid_config", "OAuth credential broker configuration is invalid");
  }
...
const validateCredentialFile = ({ fsModule, filePath, privateFile }) => {
  ...
  try {
    ...
    throw new OAuthBrokerError("not_configured", "OAuth credential broker is not configured");
  }
  if (!stats.isFile() || (stats.mode & (privateFile ? 0o077 : 0o022)) !== 0) {
    throw new OAuthBrokerError("invalid_config", "OAuth credential broker files are invalid");
  }
};
...
const runSshJsonRequest = ({
  ...
  new Promise((resolve, reject) => {
    ...
    child.stdin.on("error", () => finish(new OAuthBrokerError("request_send_failed")));
    child.on("close", (code) => {
      if (timedOut) {
        finish(new OAuthBrokerError("timeout", "OAuth credential broker timed out"));
        return;
      }
      let response;
      try {
        ...
        finish(new OAuthBrokerError(code === 0 ? "invalid_response" : "ssh_failed"));
        return;
      }
...
const createOAuthBrokerClient = ({
  ...
  const resolveConnection = () => {
    validateCredentialFile({ fsModule, filePath: identityFile, privateFile: true });
    validateCredentialFile({ fsModule, filePath: knownHostsFile, privateFile: false });
    let config;
    try {
  ...
  return {
    ...
    status: async () => {
      ...
      if (!Array.isArray(response.grants) || typeof response.denied !== "boolean") {
        throw new OAuthBrokerError("invalid_response");
      }
      return response;
    },
    ...
    revokeCodexGrant: async () => {
      ...
      if (response.revoked !== true || typeof response.provider_revocation !== "string") {
        throw new OAuthBrokerError("invalid_response");
      }
      return response;
    },
