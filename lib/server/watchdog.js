...
const createWatchdog = ({
  ...
  const reconcileManagedPlugins = async ({ phase, source, correlationId }) => {
    if (typeof reconcileOpenclawPlugins !== "function") {
      return { ok: true, skipped: true, reason: "reconcile_unavailable" };
    }
    try {
      ...
      logEvent("plugin_reconcile", source, "ok", details, correlationId);
      ...
      logEvent("plugin_reconcile", source, "failed", result, correlationId);
      return result;
    }
  };
  ...
  const runRepair = async ({ source, correlationId, force = false }) => {
    ...
    try {
      const preflightReconcile = await reconcileManagedPlugins({
        phase: "before_doctor",
        source,
        correlationId,
      });
      if (!preflightReconcile.ok) {
        return failRepair({
          result: preflightReconcile,
          source,
          correlationId,
        });
      }
      ...
      if (ok) {
        const postflightReconcile = await reconcileManagedPlugins({
          phase: "after_doctor",
          source,
          correlationId,
        });
        if (!postflightReconcile.ok) {
          return failRepair({
            result: postflightReconcile,
            source,
            correlationId,
          });
        }
        ...
        return {
          ...
          launchedGateway,
          result,
          managedPluginReconcile: {
            before: preflightReconcile,
            after: postflightReconcile,
          },
        };
      }
