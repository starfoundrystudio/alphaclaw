...
const {
  ...
  kGatewayHopAlarmFailureThreshold,
} = require("./constants");
const {
...
} = require("./gateway-hop-probe");
const kBootstrapHealthCheckMs = 5 * 1000;
/** Status readers share one probe-file read for this long. */
const kGatewayHopSnapshotCacheMs = 5 * 1000;
...
const createWatchdog = ({
  ...
  tryAcquireGatewayRepair = () => ({ release() {} }),
  clawCmd,
  shellCmd,
  ...
  readGatewayHopProbe: readGatewayHopProbeImpl = readGatewayHopProbe,
}) => {
  const state = {
    lifecycle: "stopped",
    health: "unknown",
  ...
  // The security-gateway hop is a separate incident stream from gateway
  // health: its remedy is on the other host, so it never feeds auto-repair.
  let gatewayHopIncident = { active: false, alarmSent: false, openedAt: null };
  let gatewayHopReadAtMs = 0;
  let gatewayHopSignature = "";
  let gatewayHopEvaluation = null;

  const buildDoctorRepairEnv = () => withOpenclawMaintenanceEnv({
    ...process.env,
  ...
  const readGatewayHopSnapshot = ({ force = false } = {}) => {
    const nowMs = Date.now();
    if (
      !force &&
      state.gatewayHop &&
      nowMs - gatewayHopReadAtMs < kGatewayHopSnapshotCacheMs
    ) {
      return refreshGatewayHopSnapshot(state.gatewayHop, { now: nowMs });
    }
    let snapshot;
    try {
      snapshot = readGatewayHopProbeImpl({ now: nowMs });
    } catch (error) {
      snapshot = buildUnavailableGatewayHopSnapshot({
        reason: `read_error:${String(error?.code || error?.message || "unknown").slice(0, 64)}`,
      });
    }
    gatewayHopReadAtMs = nowMs;
    state.gatewayHop = snapshot;
    return snapshot;
  };

  const describeGatewayHop = (snapshot) => ({
    ...snapshot,
    incidentActive: gatewayHopIncident.active,
    incidentOpenedAt: gatewayHopIncident.openedAt,
    alarmFailureThreshold: kGatewayHopAlarmFailureThreshold,
  });

  const buildGatewayHopLines = (snapshot) => [
    ...(snapshot.gatewayHost ? [`Host: ${asInlineCode(snapshot.gatewayHost)}`] : []),
    ...(snapshot.error ? [`Error: ${asInlineCode(snapshot.error)}`] : []),
    ...(snapshot.detail ? [`Detail: ${asInlineCode(snapshot.detail)}`] : []),
    `Consecutive failed probes: ${snapshot.consecutiveFailures}`,
    `Last reachable: ${snapshot.lastOkAt || "unknown"}`,
  ];

  const resetGatewayHopIncident = () => {
    gatewayHopIncident = { active: false, alarmSent: false, openedAt: null };
  };

  const evaluateGatewayHopUnlocked = async (correlationId) => {
    const snapshot = readGatewayHopSnapshot({ force: true });
    const alarm =
      snapshot.state === kGatewayHopState.failing &&
      snapshot.consecutiveFailures >= kGatewayHopAlarmFailureThreshold;
    const signature = [
      snapshot.state,
      snapshot.error || "",
      snapshot.reason || "",
    ...
    const quietState =
      snapshot.state === kGatewayHopState.unavailable ||
      snapshot.state === kGatewayHopState.notApplicable;
    if (signature !== gatewayHopSignature) {
      gatewayHopSignature = signature;
      if (!quietState || gatewayHopIncident.active) {
        logEvent(
          "gateway_hop",
          "host_probe",
          snapshot.state === kGatewayHopState.failing
            ? "failed"
            : snapshot.state === kGatewayHopState.healthy || quietState
              ? "ok"
              : "warn",
          describeGatewayHop(snapshot),
          correlationId,
        );
      }
    }
    ...
    if (alarm) {
      ...
      if (!gatewayHopIncident.alarmSent) {
        const result = await notify(
          [
            "🐺 *Clawbridge Watchdog*",
            withViewLogsSuffix("🔴 Security gateway unreachable"),
            ...buildGatewayHopLines(snapshot),
            "Auto-repair does not apply; the security gateway host needs attention (power cycle).",
          ].join("\n"),
          correlationId,
          "crash",
        );
        if (result?.ok || result?.skipped) gatewayHopIncident.alarmSent = true;
      }
      return snapshot;
    }

    if (!gatewayHopIncident.active) return snapshot;
    if (snapshot.state === kGatewayHopState.healthy) {
      resetGatewayHopIncident();
      await notify(
        [
          "🐺 *Clawbridge Watchdog*",
          withViewLogsSuffix("🟢 Security gateway reachable again"),
          ...(snapshot.gatewayHost
            ? [`Host: ${asInlineCode(snapshot.gatewayHost)}`]
            : []),
    ...
    } else if (snapshot.state === kGatewayHopState.notApplicable) {
      resetGatewayHopIncident();
    }
    // Stale, invalid or missing data keeps the incident open without noise;
    // a fresh healthy probe is the only recovery signal.
    return snapshot;
  };

  const evaluateGatewayHop = async (correlationId) => {
    if (gatewayHopEvaluation) return gatewayHopEvaluation;
    gatewayHopEvaluation = evaluateGatewayHopUnlocked(correlationId)
      .catch((error) => {
        console.error(
          `[watchdog] gateway hop probe evaluation failed: ${error?.message || error}`,
        );
        return state.gatewayHop;
      })
      .finally(() => {
        gatewayHopEvaluation = null;
      });
    return gatewayHopEvaluation;
  };
  ...
  const reconcileManagedPlugins = async ({ correlationId }) => {
    ...
    try {
      ...
      assertOpenclawConfigSafeForMutation({
        config,
        openclawDir,
        operation: "Clawbridge watchdog plugin reconciliation",
      });
  ...
  const runRepair = async ({ source, correlationId, force = false }) => {
    if (isGatewayLifecycleBusy()) {
      return { ok: false, skipped: true, reason: "gateway_lifecycle_in_progress" };
    }
    ...
    const lifecycleOwner = tryAcquireGatewayRepair();
    if (!lifecycleOwner) {
      return { ok: false, skipped: true, reason: "gateway_lifecycle_in_progress" };
    }
    ...
    try {
      ...
      if (ok) {
        ...
        try {
          const child = launchGatewayProcess({ lifecycleOwner });
  ...
  const runHealthCheck = async ({
    ...
    // The hop probe is host-owned data and independent of gateway lifecycle,
    // so it is evaluated even while a repair or restart is in progress.
    await evaluateGatewayHop(correlationId);
    ...
    const restartWindowActive = isGatewayLifecycleBusy() || (
  ...
  const onGatewayExit = ({
    ...
    expectedExitReason = "",
    recoveryWindowMs = 0,
    startupConfigRefusal = false,
    stderrTail = [],
  } = {}) => {
    const correlationId = createCorrelationId();
    clearDegradedHealthCheckTimer();
    const controllerOwnsRecovery = expectedExitReason === "managed_restart" ||
      expectedExitReason === "migration_retry" ||
      expectedExitReason === "restart_handoff";
    if (expectedExit && (code == null || code === 0 || controllerOwnsRecovery)) {
      state.lifecycle = "restarting";
      state.health = "unknown";
      state.uptimeStartedAt = null;
      state.crashRecoveryActive = false;
      // Preserve a bounded migration retry/startup window instead of racing
      // the gateway controller with a second launch or a doctor repair.
      markExpectedRestartWindow(controllerOwnsRecovery
        ? Math.min(8 * 60 * 1000, Math.max(kExpectedRestartWindowMs, Number(recoveryWindowMs) || 0))
        : kExpectedRestartWindowMs);
      startBootstrapHealthChecks();
      logEvent(
        "restart",
        "exit_event",
        "ok",
        { expectedExit: true, expectedExitReason, code: code ?? null, signal: signal ?? null },
        correlationId,
      );
      return;
    }
    ...
    if (startupConfigRefusal || code === 78) {
  ...
  const getStatus = () => {
    ...
    return {
      ...
      operationInProgress: state.operationInProgress || isGatewayLifecycleBusy(),
      gatewayPid: state.gatewayPid,
      eventLoopLag: state.lastEventLoopLag,
      gatewayHop: describeGatewayHop(readGatewayHopSnapshot()),
    };
  };
