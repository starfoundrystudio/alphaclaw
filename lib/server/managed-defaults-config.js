...
const ensureManagedOpenclawDefaults = (rawConfig = {}) => {
  ...
  // Retired by OpenClaw 2026.9: Doctor moves `agents.defaults.memorySearch`
  // to `memory.search` and `plugins.bundledDiscovery` to
  // `config_machine_state`; a config that still carries either key fails
  // validation on every CLI call. Strip them on every managed reconcile so a
  // stale writer or an older backup can never re-poison the file (G2 finding
  // #1 on test-g2-oc94-01, 2026-09-18).
  delete agentDefaults.memorySearch;

  const plugins = ensureObject(config, "plugins");
  delete plugins.bundledDiscovery;
