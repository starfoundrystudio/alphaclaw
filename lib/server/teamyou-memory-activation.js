...
const activateTeamyouMemoryIfBootstrapComplete = async ({
  ...
  if (!hasManagedTeamyouMemoryConfig(cfg)) {
    return {
      ok: true,
      activated: false,
      reason: "teamyou_memory_not_configured",
    };
  }
  ...
  const managedSlotLeaked = kManagedMemorySlotPluginIds.has(memorySlot);
  if (!completion.complete && (managedSlotLeaked || activeMemoryEnabled)) {
...
const createTeamyouMemoryActivationService = ({
  ...
  const run = async () => {
    ...
    try {
      ...
      if (
        !result.activated &&
        result.reason !== "memory_slot_already_active" &&
        result.reason !== "teamyou_memory_not_configured"
      ) {
        schedule();
      }
    } catch (error) {
