...
const kTeamyouMemoryAlreadyActiveReason = "teamyou_memory_already_active";
const kTeamyouMemoryPluginMissingReason = "teamyou_memory_plugin_missing";
...
const removeActivationMarker = ({ fsModule, openclawDir }) => {
...
const collectWorkspaceCandidates = ({ cfg = {}, workspaceDir }) => {
  ...
  const defaultAgent =
    configuredAgents.find((agent) => String(agent?.id || "").trim() === defaultAgentId) ||
    configuredAgents[0];
...
const isManagedTeamyouMemoryPluginInstalled = ({
  fsModule,
  openclawDir,
  ...
  return isPlainObject(manifest) && String(manifest.id || "").trim() === pluginId;
};
...
const isTeamyouPluginEntryEnabled = (cfg, pluginId) =>
  isPlainObject(cfg.plugins.entries[pluginId]) &&
  cfg.plugins.entries[pluginId].enabled === true;

const disableTeamyouPluginEntryIfPresent = ({ cfg, pluginId }) => {
  if (!isPlainObject(cfg.plugins.entries[pluginId])) return;
...
const enforceTeamyouBootstrapGateIfPending = ({
  ...
  const teamyouPluginEnabled = isTeamyouPluginEntryEnabled(cfg, pluginId);
  ...
  const teamyouPluginLeaked = memoryConfigured && teamyouPluginEnabled;
  ...
  if (!teamyouPluginLeaked && !activeMemoryLeaked && !teamyouSkillLeaked) {
    return {
      ok: true,
      changed: false,
      reason: completion.reason,
      teamyouPluginEnabled,
      activeMemoryEnabled,
      teamyouSkillEnabled,
      workspaceDir: completion.workspaceDir,
    };
  ...
  if (memoryConfigured) {
    disableTeamyouPluginEntryIfPresent({ cfg, pluginId });
  ...
  return {
    ...
    changed: true,
    reason: completion.reason,
    previousTeamyouPluginEnabled: teamyouPluginEnabled,
    previousActiveMemoryEnabled: activeMemoryEnabled,
    previousTeamyouSkillEnabled: teamyouSkillEnabled,
    teamyouPluginEnabled: memoryConfigured ? false : teamyouPluginEnabled,
...
const activateTeamyouMemoryIfBootstrapComplete = async ({
  ...
  // Every change made here is under plugins.* or skills.*. OpenClaw 2026.9
  // hot-reloads plugins.* in the running Gateway (verified on host 06:
  // enabling the TeamYou plugin loads it without a restart) and needs no
  // action for skills.*. A restart there only interrupts whatever the agent
  // is doing: right after the ritual it cut the agent's last turn (G3 finding
  // #24). Older OpenClaw lines still need the restart.
  const applyConfigChange = async () => {
    if (typeof gatewayHotReloadsConfig === "function" && gatewayHotReloadsConfig()) {
      // The hot reload can leave the Gateway on a revoked plugin instance
      // (G3 finding #42); the caller checks once it has settled.
      if (typeof onHotReloadApplied === "function") onHotReloadApplied();
      return;
    }
  ...
  if (!completion.complete) {
    // plugins.slots.memory is never written here: memory-core owns the slot
    // (clawctl's reconcile keeps it that way), so the pre-bootstrap gate only
    // covers the TeamYou plugin entry, the active-memory eligibility flag, and
    // the TeamYou skill.
    const teamyouPluginLeaked = memoryConfigured && teamyouPluginEnabled;
    const activeMemoryLeaked = memoryConfigured && activeMemoryEnabled;
    const teamyouSkillLeaked = shouldManageSkill && teamyouSkillEnabled;
  ...
  if (
    memoryConfigured &&
    !isManagedTeamyouMemoryPluginInstalled({
      fsModule,
      openclawDir,
      pluginId,
    })
  ) {
    const teamyouPluginLeaked = teamyouPluginEnabled;
    const activeMemoryLeaked = activeMemoryEnabled;
    const teamyouSkillLeaked = shouldManageSkill && teamyouSkillEnabled;
    const markerRemoved = removeActivationMarker({ fsModule, openclawDir });
    const staleMemorySlot =
      String(cfg.plugins.slots.memory || "").trim() === pluginId;
    const configChanged =
      teamyouPluginLeaked ||
      activeMemoryLeaked ||
      teamyouSkillLeaked ||
    ...
    if (configChanged) {
      ...
      await applyConfigChange();
      logger.warn?.(
        `[alphaclaw] TeamYou memory activation deferred because plugin "${pluginId}" is not installed`,
      );
    }
    ...
    return {
      ok: true,
      activated: false,
      repaired: configChanged || markerRemoved,
      reason: kTeamyouMemoryPluginMissingReason,
  ...
  const needsMemoryActivation = memoryConfigured && !teamyouPluginEnabled;
  const needsSkillActivation = shouldManageSkill && !teamyouSkillEnabled;
  if (!needsMemoryActivation && !needsSkillActivation) {
    writeActivationMarker({
      fsModule,
      openclawDir,
      pluginId,
      workspaceDir: completion.workspaceDir,
      reason: kTeamyouMemoryAlreadyActiveReason,
    });
    return {
      ok: true,
      activated: false,
  ...
  return {
    ...
    activated: true,
    reason: completion.reason,
    teamyouPluginEnabled: memoryConfigured ? true : teamyouPluginEnabled,
...
const createTeamyouMemoryActivationService = ({
  ...
  const run = async () => {
    ...
    try {
      ...
      if (!result.activated && result.reason !== kTeamyouMemoryAlreadyActiveReason) {
        schedule();
      }
    } catch (error) {
      logger.warn?.(
...
module.exports = {
  ...
  kTeamyouMemoryAlreadyActiveReason,
  kTeamyouMemoryPluginMissingReason,
  isManagedTeamyouMemoryPluginInstalled,
};
