...
const writeOpenclawConfig = ({
  ...
  // Trailing newline matches OpenClaw's own writes, so the same settings are
  // the same bytes whichever writer saved them last (OpenClaw 2026.9 compares
  // raw config contents across startup; G3 finding #23).
  ...
  try {
    // Owner-only, like OpenClaw's own config writes. A temp file created with
    // the process umask (0644) replaced the config on every write, leaving it
    // group/world readable (G3 finding #25).
    fsModule.writeFileSync(tempPath, content, { encoding: "utf8", mode: 0o600 });
