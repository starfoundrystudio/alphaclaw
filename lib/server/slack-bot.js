...
const inspectSlackCredentials = async (
  ...
  try {
    ...
    if (!String(auth?.bot_id || "").trim()) {
      ...
      throw createSlackInspectionError(
        swapped
          ? "The two Slack tokens are swapped in Agent Vault: SLACK_BOT_TOKEN holds the app-level (xapp-) token and SLACK_APP_TOKEN holds the bot (xoxb-) token. Open Agent Vault, rotate each credential with the correct value, then try again."
