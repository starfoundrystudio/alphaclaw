...
const kBootstrapHandoffReadyPath = "/api/onboard/runtime-ready.svg";
...
const getConfiguredPrivateUiBaseUrl = (env = process.env) =>
  normalizeBaseUrl(
    env.ALPHACLAW_SETUP_URL || env.ALPHACLAW_BASE_URL || env.URL || "",
  );
...
const isLoopbackAddress = (address = "") => {
  ...
  const bare = normalized.startsWith("::ffff:")
...
const isRequestFromSecurityGateway = (req, env = process.env) => {
  ...
  if (remoteAddress === gatewayAddress) return true;

  // Gateway traffic terminates mTLS at the workload's Caddy process before
  // reaching Clawbridge over loopback. Caddy replaces X-Forwarded-For with its
  // immediate peer by default, so the last hop identifies the gateway that
  // connected to the private bridge.
  if (!isLoopbackAddress(remoteAddress)) return false;
  const forwardedFor = String(req?.headers?.["x-forwarded-for"] || "")
    .split(",")
    .map((entry) => normalizeIpAddress(entry))
    .filter(Boolean);
  return forwardedFor.at(-1) === gatewayAddress;
};

const isTrustedBootstrapHandoffRequest = (req = {}, env = process.env) => {
  const method = String(req?.method || "").trim().toUpperCase();
  return (
    (method === "GET" || method === "HEAD") &&
    getRequestPathname(req) === kBootstrapHandoffReadyPath &&
...
// A request from a loopback socket that carries no X-Forwarded-* headers is a
// same-box process talking to us directly (e.g. the gmail-serve sidecar
// delivering to 127.0.0.1:<PORT>/hooks/gmail). Tailscale Serve/Funnel always
// proxy with X-Forwarded-* headers even though their sockets are local, so
// external traffic can never take this branch. Local processes gain nothing
// they don't already have: the guard classifies origins, it is not
// authentication — hook/webhook endpoints still enforce their own tokens.
const isTrustedLoopbackRequest = (req = {}) => {
...
const isRequestAllowedForSurface = (req = {}, env = process.env) => {
  // The public bootstrap hostname is neither the final private UI origin nor
  // the callback origin after onboarding. Admit only the exact, read-only
  // readiness request stamped by the trusted security gateway; normal auth
  // still runs afterward and requires the setup page's existing cookie.
  if (isTrustedBootstrapHandoffRequest(req, env)) return true;
  ...
  // "unknown" origin: still admit direct same-box connections — the Gmail
  // watch pipeline's final hop is the gog sidecar posting to
  // 127.0.0.1:<PORT>/hooks/gmail, which matches neither configured surface.
  return isTrustedLoopbackRequest(req);
};
