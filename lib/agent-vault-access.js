...
const kIncludeServiceModes = new Set(["always", "missing"]);
...
const normalizeAgentVaultAccessRequest = (input) => {
  ...
  // By default a proposal carries the service only when /discover lacks its
  // host. "missing" also re-proposes it whenever a credential is missing and
  // "always" re-proposes it unconditionally: /discover hides substitutions,
  // so a caller that must change the stored substitution list says so.
  const includeService = kIncludeServiceModes.has(input?.includeService)
...
const planAgentVaultAccess = (access, discovered = {}) => {
  ...
  const includeService =
    !serviceAvailable ||
    access.includeService === "always" ||
    (access.includeService === "missing" && credentials.length > 0);
  return {
    status:
      !includeService && credentials.length === 0
        ? "available"
        : "proposal_required",
    serviceAvailable,
    ...
    proposal: {
      services: includeService ? [serializeService(access.service)] : [],
      credentials,
